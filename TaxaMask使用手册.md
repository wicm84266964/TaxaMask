...
> 适用版本：2026-06-02 Agent Center / Ask Agent 路由 / TIF-STL 分流 / 主工作台父部位与子部位标注 / VLM 第一公里预标注 / TIF GPU 体预览 / PDF 文献证据与性状描述闭环版
>
...
这个按钮只训练当前 `父部位 -> 子部位` 路由下的子部位专家。训练参数继续来自 `2D/STL Model Settings` 里的 Blink 默认参数。
