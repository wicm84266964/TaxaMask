...
- context metadata records only message counts, byte counts, compaction counts, and summary byte counts; compacted summary text stays process-local.
...
- `ant-code --resume latest` and `/resume latest` restore the session id and turn index from metadata without restoring raw conversation text.
