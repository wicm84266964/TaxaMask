export const SLASH_COMMANDS = Object.freeze([
  ...
  command("goal", "会话", "Goal", "开启无人值守 Goal：自动续跑直到完成、暂停、失败或预算用尽；完成时显示一行汇总。"),
...
export const KEYBINDINGS = Object.freeze([
  ...
  keybinding("permission-mode", "权限", "Shift+Tab", "切换权限：计划确认 -> 工作区权限 -> 完全访问；后续提示/命令生效，当前运行轮次保留启动时权限。Goal 开启时锁定完全访问，需先 /goal exit。"),
  keybinding("goal", "会话", "/goal", "开启无人值守 Goal；pause/resume/exit 控制循环，完成时在状态栏显示一行汇总。"),
...
export function listSlashCommandGroups() {
  ...
  return CATEGORY_ORDER.map((category) => ({
    category,
    commands: commands.filter((item) => item.category === category)
...
function command(name: string, category: string, title: string, description: string, options: { aliases?: string[]; keybind?: string; disabledReason?: string; action?: string; panel?: unknown } = {}) {
  return {
    id: `slash.${name}`,
    name,
    title,
...
function keybinding(id: string, category: unknown, key: string, description: unknown, condition: unknown = "") {
  return {
    id,
    category,
    key,
