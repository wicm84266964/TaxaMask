...
- [ ] 每个阶段开始前先确认解释器是 `C:\Users\admin\anaconda3\envs\taxamask\python.exe`；不要因为默认环境报缺包就安装 PySide6 或其他依赖。
- [ ] 每个阶段改动前先确认 `git status --short`，阶段内只混入本阶段相关文件；如果发现已有未提交补丁，先判断它属于前一阶段、当前阶段还是用户手工改动。
- [ ] 每次拆 UI helper/controller 后，优先检查“真实用户会点的链路”，而不是只看导入成功：右侧栏滚动、按钮中文、Local Axis 三点、重切片、训练/预测入口都要进入检查表。
- [ ] 每次改 busy lock、后台任务 token、preview controller 或资源策略后，必须确认它没有误挡 Local Axis 三点选择，也没有让旧任务结果刷新到当前 specimen/part/reslice。
- [ ] 每次出现 GUI 卡死、preview 失败、memmap 打不开或 `WinError 1455`，先记录后台训练/预测进程、提交内存、页面文件和显存状态；不要立即把它归类为代码回归或数据损坏。
- [ ] 每次清理测试残留或后台进程时，先区分 `.tmp_validation` / `__pycache__` 这类一次性产物和正在训练的有效任务；不要为了“清爽”误删研究输出或误杀训练。
- [ ] 每次新增测试文件后，同步检查 `scripts/run_validation_suite.py` 和 `tests/test_validation_suite_script.py`；否则新测试可能只在手工命令里跑过，后续全量验证会漏掉。
- [ ] 每次移动 Agent 可读状态、状态摘要或控制器入口后，同步检查 Agent/AntCode 对接输出；不要让询问 Agent 继续读旧字段或旧语义。
- [ ] 每个阶段提交前必须做一次“数据安全清单”：不提交 TIF/CT stack、SQLite 项目库、模型权重、run outputs、API 配置、私有项目 JSON 或一次性验证产物。
- [ ] 自动测试通过后仍要写清楚“还没人工看的链路”；尤其是 GPU 3D preview、真实 TIF 重切片清晰度、训练/预测真实输出导入，这些不能只靠无头测试背书。
