...
> 适用版本：TaxaMask main / v1.2 系列公开稳定线。
>
...
从 TaxaMask 1.2.0 起，新建 2D 形态项目使用 SQLite manifest + `.taxamask.sqlite` 数据库作为主存储。这个改变的目的很实际：大项目在标注、VLM 预标注、候选图导入和模型复核时，不再需要每次把全部项目记录重写成一个巨大的 JSON 文件。

旧版 2D 项目 JSON 仍然可以打开；打开时应优先迁移到 SQLite。legacy JSON 也可以作为兼容导出或审计快照保留，但不建议继续把它当成正式大项目的生产主存储。
...
- 是否记录 profile。
- 是否导出的是确认标签。
- 是否保存了 SQLite manifest、`.taxamask.sqlite` 数据库和对应资源路径。

### 20.6 仓库维护

不要提交：

- 私有项目 JSON、SQLite manifest 或 `.taxamask.sqlite` 数据库。
- PDF 运行输出。
- 提取图像输出。
- 本地数据库。
- API key 或 `.env`。
