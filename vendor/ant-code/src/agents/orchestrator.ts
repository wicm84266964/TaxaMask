...
import { routeAgentTask } from "./router.ts";
import { runSubagent } from "./runner.ts";
import { createAgentTaskStore } from "./task-store.ts";

const MAX_PARALLEL_READONLY = 3;

type RunSubagentOptions = Parameters<typeof runSubagent>[0];
type SuggestedTask = ReturnType<typeof routeAgentTask>["suggestedTasks"][number];
type SubagentRunResult = Awaited<ReturnType<typeof runSubagent>>;

type TaskTreeNode = Record<string, unknown> & {
...
export async function orchestrateAgentTasks(options: {
  ...
  allowCommand?: boolean;
  fullAccess?: boolean;
  workflowState?: RunSubagentOptions["workflowState"];
  approvalCallback?: RunSubagentOptions["approvalCallback"];
  hooksTrusted?: boolean;
  signal?: AbortSignal;
}) {
...
export function buildTaskTree(tasks: unknown = []) {
  const byId = new Map<unknown, TaskTreeNode>();
  ...
  for (const node of byId.values()) {
    const parent = node.parentTaskId ? byId.get(node.parentTaskId) : undefined;
    if (parent) {
...
export function formatTaskTree(tasks: unknown = []) {
...
function collectionLength(value: unknown): unknown {
...
function pushTaskNode(lines: string[], task: TaskTreeNode, depth: number) {
  const indent = "  ".repeat(depth);
  const title = task.title || task.profile || "task";
  const toolLength = collectionLength(task.toolCalls);
  const tools = toolLength ? ` tools=${toolLength}` : "";
...
function summarizeOrchestration(results: SubagentRunResult[], serialTasks: SuggestedTask[]) {
  const lines = [
  ...
  for (const result of results) {
    ...
    const summary = typeof output === "string"
      ? output.split(/\r?\n/).slice(0, 3).join(" ")
      : isRecord(error) ? String(error.message ?? "") : "";
...
function summarizeChildResult(result: SubagentRunResult) {
  return {
    ok: result.ok === true,
...
function compareTaskUpdated(left: Record<string, unknown>, right: Record<string, unknown>) {
