...
import { createMockGatewayServer } from "./mock-gateway.ts";
import { loadConfig, type LabAgentConfig } from "../src/config/load-config.ts";
import { createLabModelGateway } from "../src/model-gateway/client.ts";
import { runGatewayHealth } from "../src/model-gateway/health.ts";
import { GATEWAY_PROTOCOL_VERSION } from "../src/model-gateway/protocol.ts";

type HealthCheck = { name: string; status: string; message: string };
type CompatCheck = { name: string; status: string; message: string };
 ...
 */
async function verifyGateway(options: { mode: string; env: NodeJS.ProcessEnv }): Promise<CompatResult> {
 ...
 */
function summarizeHealth(health: { checks: HealthCheck[] }, issues: HealthCheck[]) {
  if (issues.length === 0) {
...
/**
 * @param {{ mode: string; config: LabAgentConfig; health: Awaited<ReturnType<typeof runGatewayHealth>>; chatOk: boolean }} input
 */
function buildEvidence(input: {
  mode: string;
  config: LabAgentConfig;
  health: Awaited<ReturnType<typeof runGatewayHealth>>;
  chatOk: boolean;
}): GatewayEvidence {
  return {
    mode: input.mode,
 ...
 */
function formatCompatibilityReport(result: CompatResult) {
  return [
 ...
 * @param {string} host
 */
function listen(server: Server, host: string): Promise<Server> {
