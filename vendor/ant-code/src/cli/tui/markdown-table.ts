...
export { displayWidth } from "./input-editor.ts";
...
export type MarkdownBlock = MarkdownParagraphBlock | MarkdownCodeBlock | MarkdownTableBlock;
type WidthFn = (text: string) => number;
type TableLine = { text: string; dim?: boolean; color?: string; noWrap?: boolean };
 ...
 */
export function isLargeTable(block: MarkdownBlock) {
  if (block.type !== "table") {
    return false;
  }
  return (
 ...
 */
export function renderTable(block: MarkdownBlock, availWidth: number, widthFn: WidthFn = displayWidth, options: { summarizeLarge?: boolean; wrapCells?: boolean; minColumnWidth?: number } = {}): TableLine[] {
 ...
 */
function proportionalCompress(widths: number[], maxWidth: number, minWidth: number) {
 ...
 */
function renderFormattedTable(block: MarkdownTableBlock, colWidths: number[], widthFn: WidthFn) {
 ...
 */
function renderWrappedTable(block: MarkdownTableBlock, colWidths: number[], widthFn: WidthFn) {
  const lines: TableLine[] = [];
...
function renderWrappedPhysicalRow(cells: string[], alignments: string[], colWidths: number[], widthFn: WidthFn) {
 ...
 */
function renderRow(cells: string[], alignments: string[], colWidths: number[], widthFn: WidthFn) {
  const parts: string[] = [];
 ...
 */
function renderSeparatorRow(colWidths: number[]) {
  const parts: string[] = [];
  for (let c = 0; c < colWidths.length; c += 1) {
 ...
 */
function padCell(text: string, targetWidth: number, alignment: string, widthFn: WidthFn) {
...
function wrapCellText(text: string, targetWidth: number, widthFn: WidthFn) {
 ...
 */
function truncateToFit(text: string, maxWidth: number, widthFn: WidthFn) {
  if (maxWidth <= 0) {
    return "";
  }
...
function summaryText(text: string, maxWidth: number, widthFn: WidthFn) {
 ...
 */
export function splitBodyBlocks(bodyText: string) {
