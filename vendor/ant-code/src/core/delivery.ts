...
import { buildTaskLifecycle, formatLifecycleLine } from "./task-lifecycle.ts";
import { buildValidationMemory } from "./validation-memory.ts";
 ...
 */
export function buildDeliveryStatus(input: { workflow?: ReturnType<typeof createWorkflowState> | null; sessionInfo?: Record<string, unknown>; validationSuggestions?: Array<{ command: string; reason: string }> } = {}) {
 ...
 */
export function formatDeliveryStatus(status: ReturnType<typeof buildDeliveryStatus>, options: { includeHeader?: boolean; includeCommands?: boolean; includeSuggestions?: boolean } = {}) {
  const lines = [];
 ...
 * @param {ReturnType<typeof buildDeliveryStatus>} status
 */
export function formatTurnFooter(status: ReturnType<typeof buildDeliveryStatus>) {
...
function buildNextActions(
  ...
  const latestCommand = typeof latestValidation?.command === "string" ? latestValidation.command : "";
  if (stage === "repair") {
    const actions = ["Repair the failed validation before reporting completion."];
    if (latestCommand) {
      actions.push(`Rerun with /verify run ${truncate(latestCommand, MAX_HINT_COMMAND_CHARS)}`);
    } else if (suggested) {
 ...
 */
function getUnresolvedFailures(validations: Array<Record<string, unknown>>) {
  let count = 0;
 ...
 * @param {Record<string, any>} summary
 */
function formatStatusCounts(summary: Record<string, unknown>) {
