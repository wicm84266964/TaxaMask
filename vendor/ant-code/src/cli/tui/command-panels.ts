...
import { listSlashCommandGroups } from "../../commands/registry.ts";
...
import { INSPECTOR_FILTERS, inspectorFilterLabel, inspectorPanelLines, type InspectorItem } from "./inspector.ts";
import { displayWidth } from "./input-editor.ts";
import { line, permissionModeDescription, permissionModeLabel, truncate, truncateMiddle, type TuiLine } from "./format.ts";
import { formatTuiGoalFooter, tuiGoalStatusLabel } from "./goal.ts";
import { parseMarkdownBlocks, renderTable } from "./markdown-table.ts";

type ContextSummary = ReturnType<typeof summarizeContextWindow>;
type SlashCommand = ReturnType<typeof listSlashCommandGroups>[number]["commands"][number];
...
type ResumeArchive = {
  ...
  chunks: Array<NonNullable<ReturnType<typeof normalizeResumeChunk>>>;
};
...
type AgentTaskRecord = {
  ...
  budgetProgress?: AgentTaskBudgetProgress | null;
...
type PanelEntry = {
  ...
  task?: Record<string, unknown> | AgentTaskRecord | null;
};
...
type TokenUsageView = {
  ...
  lastCachedPromptTokens?: number | null;
...
type WrapExcerptOptions = {
  prefix?: string;
  color?: string;
};

type PreviewMultilineOptions = {
  fromEnd?: boolean;
};

const EMPTY_ACTIVITY: StatusActivity = {};
const EMPTY_AGENT_TASK: AgentTaskRecord = {};
const EMPTY_PANEL_ENTRY: PanelEntry = {};
const EMPTY_RESUME_CHUNK: ResumeChunk = {};
const EMPTY_BUDGET: AgentTaskBudget = {};
const EMPTY_BUDGET_PROGRESS: AgentTaskBudgetProgress = {};
const EMPTY_USAGE: TokenUsageView = {};
const EMPTY_WRAP_OPTIONS: WrapExcerptOptions = {};
const EMPTY_PREVIEW_OPTIONS: PreviewMultilineOptions = {};
const EMPTY_COMPACT_RESULT: CompactResult = {};
const EMPTY_LOGS_OPTIONS: {
  inspector?: InspectorItem | Record<string, unknown> | null;
  items?: Array<InspectorItem | Record<string, unknown>>;
  index?: number;
  offset?: number;
  filter?: string;
  visibleRows?: number;
...
const EMPTY_QUEUE_OPTIONS: { queuedPrompts?: string[]; selectedIndex?: number; busy?: boolean } = {};
const EMPTY_MESSAGE_ACTIONS_OPTIONS: { entry?: PanelEntry | Record<string, unknown> | null; actionIndex?: number } = {};
const EMPTY_EXCERPT_OPTIONS: { entry?: PanelEntry | Record<string, unknown> | null } = {};
const EMPTY_AGENT_LIVE_OPTIONS: {
  task?: Record<string, unknown> | AgentTaskRecord | null;
  entry?: PanelEntry | Record<string, unknown> | null;
} = {};
const EMPTY_SESSIONS_OPTIONS: { records?: SessionRecord[]; selectedIndex?: number; currentSessionId?: string } = {};
const EMPTY_RESUME_OPTIONS: { session?: AgentSession; selectedIndex?: number } = {};
const EMPTY_RESUME_CHUNK_OPTIONS: {
  session?: AgentSession;
  chunk?: ResumeChunk | Record<string, unknown>;
  messages?: ResumeMessage[];
} = {};
const EMPTY_CLEAR_OPTIONS: { session?: AgentSession } = {};
const EMPTY_UNDO_OPTIONS: { session?: AgentSession; gitAvailable?: boolean; gitStatus?: string } = {};
const EMPTY_HELP_OPTIONS: { tabIndex?: number } = {};
 ...
 */
export function createHelpPanel(options: { tabIndex?: number } = EMPTY_HELP_OPTIONS) {
  ...
  return panel({
    ...
    lines: [
      ...
      ...group.commands.flatMap((command: SlashCommand) => [
 ...
 */
export function createStatusPanel(options: { session: AgentSession; activity?: StatusActivity; trusted?: boolean; cwd?: string }) {
 ...
 */
export function createPermissionsPanel(options: { session: AgentSession; trusted?: boolean }) {
  const session = options.session;
  ...
  return panel({
    ...
    lines: [
      ...
      ...permissionMatrixRows(session).map((row: PermissionMatrixRow) => line(row.text, row.dim, row.color)),
      line(""),
 ...
 */
export function createContextPanel(options: { session: AgentSession; compactResult?: CompactResult | Record<string, unknown> | null; before?: ContextSummary | null; after?: ContextSummary | null }) {
  ...
  return panel({
    ...
    lines: [
      ...
      ...(Number.isFinite(summary.providerPromptTokens)
        ...
        : []),
      line(`模型消息链：${formatTokenCount(summary.messageTokens)} tokens（估算）`),
 ...
 */
export function createCompactPanel(options: { result: CompactResult | Record<string, unknown>; before?: ContextSummary | null; after?: ContextSummary | null; session?: AgentSession }) {
  return panel({
    kind: "compact",
    title: "上下文压缩",
    subtitle: "手动上下文压缩结果。",
 ...
 */
export function createUsagePanel(options: { session: AgentSession; name: "usage" | "cost" }) {
  const session = options.session;
  const workflow = session.workflow;
  const usage = session.usage ?? EMPTY_USAGE;
  const reports = usage.reports;
  const hasProviderUsage = typeof reports === "number" && reports > 0;
  const changes = Array.isArray(workflow.changes) ? workflow.changes : [];
  const validations = (Array.isArray(workflow.validations) ? workflow.validations : []) as WorkflowValidation[];
 ...
 */
export function createModelPanel(options: { session: AgentSession; models?: LabModel[] }) {
  const session = options.session;
 ...
 */
export function createTextOutputPanel(options: { title: string; command: string; output: string; kind?: string }) {
  return panel({
    kind: options.kind ?? "command-output",
    title: options.title,
    subtitle: options.command,
    lines: String(options.output ?? "").split(/\r?\n/).map((text: string, index: number) => line(text || " ", index > 12)),
...
export function createLogsPanel(options: {
  ...
  const tabIndex = Math.max(0, (INSPECTOR_FILTERS as readonly string[]).indexOf(filter));
  return panel({
    kind: "logs",
    title: "运行日志",
 ...
 */
export function createQueuePanel(options: { queuedPrompts?: string[]; selectedIndex?: number; busy?: boolean } = EMPTY_QUEUE_OPTIONS) {
 ...
 */
export function createMessageActionsPanel(options: { entry?: PanelEntry | Record<string, unknown> | null; actionIndex?: number } = EMPTY_MESSAGE_ACTIONS_OPTIONS) {
  const entry = options.entry ? asPanelEntry(options.entry) : null;
 ...
 */
export function createMessageExcerptPanel(options: { entry?: PanelEntry | Record<string, unknown> | null } = EMPTY_EXCERPT_OPTIONS) {
  const entry = options.entry ? asPanelEntry(options.entry) : null;
  ...
  return panel({
    ...
    subtitle: entry
      ? isAgentTask && entry.task
        ? `${truncate(asAgentTask(entry.task).title ?? entry.title ?? "子任务", 48)} · ${truncate(asAgentTask(entry.task).profile ?? entry.profile ?? "unknown", 24)} · ${truncate(asAgentTask(entry.task).status ?? entry.taskStatus ?? "unknown", 24)}`
...
export function createAgentTaskLivePanel(options: {
  task?: Record<string, unknown> | AgentTaskRecord | null;
  entry?: PanelEntry | Record<string, unknown> | null;
} = EMPTY_AGENT_LIVE_OPTIONS) {
  const task = asAgentTask(options.task);
  const entry = asPanelEntry(options.entry);
  const taskId = (task.id ?? entry.taskId ?? "").trim();
  const status = task.status ?? entry.taskStatus ?? "unknown";
  const profile = task.profile ?? entry.profile ?? "unknown";
 ...
 */
export function createSessionsPanel(options: { records?: SessionRecord[]; selectedIndex?: number; currentSessionId?: string } = EMPTY_SESSIONS_OPTIONS) {
    ...
    lines: records.length === 0
      ...
      : [
        ...
        ...records.map((record, index) => {
          ...
          const messageCount = record.transcriptMessages;
          const messages = typeof messageCount === "number" && Number.isFinite(messageCount) && messageCount > 0 ? ` msgs=${messageCount}` : "";
 ...
 */
export function createResumePanel(options: { session?: AgentSession; selectedIndex?: number } = EMPTY_RESUME_OPTIONS) {
  const session = options.session;
  const archive = normalizeResumeArchive(session?.transcriptArchive);
  const chunks = archive.chunks;
  const selectedIndex = clampIndex(Number(options.selectedIndex ?? Math.max(0, chunks.length - 1)) || 0, Math.max(1, chunks.length));
  ...
  return panel({
    ...
    lines: chunks.length === 0
      ...
      : [
        ...
        line(`id: ${truncate(session?.id ?? "unknown", 32)}`),
        ...
        ...chunks.map((chunk, index) => {
          ...
          const bytes = typeof chunk.bytes === "number" && chunk.bytes > 0 ? ` bytes=${chunk.bytes}` : "";
...
export function createResumeChunkPanel(options: {
  ...
  chunk?: ResumeChunk | Record<string, unknown>;
  messages?: ResumeMessage[];
} = EMPTY_RESUME_CHUNK_OPTIONS) {
  const session = options.session;
  const chunk = asResumeChunk(options.chunk);
  const messages = Array.isArray(options.messages) ? options.messages : [];
  return panel({
    kind: "resume-chunk",
    title: `Resume chunk ${chunk.index ?? "?"}`,
    subtitle: `${truncate(session?.id ?? chunk.sessionId ?? "current", 32)} · ${messages.length} 条历史消息`,
    lines: [
 ...
 */
export function createClearConfirmPanel(options: { session?: AgentSession } = EMPTY_CLEAR_OPTIONS) {
 ...
 */
export function createUndoRedoPanel(options: { session?: AgentSession; gitAvailable?: boolean; gitStatus?: string } = EMPTY_UNDO_OPTIONS) {
 ...
 */
function compactResultLines(
  result: CompactResult | Record<string, unknown>,
  before: ContextSummary | null | undefined,
  after: ContextSummary | null | undefined
) {
  const compact = asCompactResult(result);
  const color = compact.compacted ? "green" : "yellow";
  return [
    line(compact.compacted ? "已压缩" : "未压缩", false, color),
    line(`原因：${compact.reason ?? "unknown"}`),
    line(`tokens：${formatTokenCount(compact.beforeTokens ?? before?.messageTokens)} -> ${formatTokenCount(compact.afterTokens ?? after?.messageTokens)}（估算）`),
    line(`消息：${compact.beforeMessages ?? before?.messages ?? "?"} -> ${compact.afterMessages ?? after?.messages ?? "?"}`),
    line(`字节：${compact.beforeBytes ?? before?.messageBytes ?? "?"} -> ${compact.afterBytes ?? after?.messageBytes ?? "?"}`),
    line(`方式：${formatCompactionStrategy(compact.strategy)}`),
    ...(compact.internalAgent ? [line(`内部 agent：${compact.internalAgent}`, true, "cyan")] : []),
    ...(compact.fallbackReason ? [line(`降级原因：${compact.fallbackReason}`, true, "yellow")] : []),
    line(`摘要字节：${compact.summaryBytes ?? after?.summaryBytes ?? "?"}`),
    ...(after ? [
...
function formatCompactionStrategy(strategy: unknown) {
...
function messageActionsForEntry(entry: PanelEntry | null): MessageAction[] {
...
function messageExcerptContentLines(entry: PanelEntry = EMPTY_PANEL_ENTRY): TuiLine[] {
  const rows: TuiLine[] = [
  ...
  if (entry.title && entry.kind !== "user" && entry.kind !== "assistant" && entry.kind !== "agent") {
    rows.push(...wrapExcerptText(entry.title, { prefix: "  ", color: "cyan" }));
  }
  const body = entry.excerptBody ?? entry.body ?? "";
  if (!body) {
...
export function formatMessageBodyForDisplayClipboard(entry: PanelEntry | Record<string, unknown> = EMPTY_PANEL_ENTRY) {
  const view = asPanelEntry(entry);
  const body = view.excerptBody ?? view.body ?? "";
  if (!body || view.kind !== "assistant") {
    return body;
  }
 ...
 */
export function formatAgentTaskExcerptBody(
  task: Record<string, unknown> | AgentTaskRecord = EMPTY_AGENT_TASK,
  entry: Record<string, unknown> | PanelEntry = EMPTY_PANEL_ENTRY
) {
  const agentTask = asAgentTask(task);
  const panelEntry = asPanelEntry(entry);
  const toolCalls = Array.isArray(agentTask.toolCalls) ? agentTask.toolCalls : [];
  const metadata = agentTask.metadata ?? null;
  const parts = [
    `标题：${truncate(agentTask.title ?? panelEntry.title ?? "子智能体任务", 120)}`,
    `task：${agentTask.id ?? panelEntry.taskId ?? "unknown"}`,
    `profile：${agentTask.profile ?? panelEntry.profile ?? "unknown"}`,
    `状态：${agentTask.status ?? panelEntry.taskStatus ?? "unknown"}`,
    agentTask.mode ? `mode：${agentTask.mode}` : null,
    agentTask.model ? `model：${agentTask.model}${agentTask.modelTier ? ` (${agentTask.modelTier})` : ""}` : null,
    agentTask.purpose ? `purpose：${agentTask.purpose}` : null,
    agentTask.difficulty ? `difficulty：${agentTask.difficulty}` : null,
    agentTask.risk ? `risk：${agentTask.risk}` : null,
    agentTask.createdAt ? `createdAt：${agentTask.createdAt}` : null,
    agentTask.startedAt ? `startedAt：${agentTask.startedAt}` : null,
    agentTask.finishedAt ? `finishedAt：${agentTask.finishedAt}` : null,
    metadata?.outputPath ? `outputPath：${metadata.outputPath}` : null,
    typeof metadata?.outputBytes === "number" && Number.isFinite(metadata.outputBytes) ? `outputBytes：${metadata.outputBytes}` : null,
    "",
    "latestProgress：",
    agentTask.latestProgress ? `  ${agentTask.latestProgress}` : "  [空]",
    "",
    "prompt：",
    agentTask.prompt ? `  ${agentTask.prompt}` : "  [空]",
    "",
    "toolCalls：",
    ...(toolCalls.length > 0
    ...
    "",
    "outputSummary：",
    agentTask.outputSummary ? `  ${agentTask.outputSummary}` : "  [空]",
    "",
    "output：",
    agentTask.output ? `  ${agentTask.output}` : "  [空]",
    agentTask.continuationPrompt ? [
      "",
      "continuationPrompt：",
      `  ${agentTask.continuationPrompt}`
    ].join("\n") : null,
    agentTask.budgetExceeded ? [
      "",
      "budgetExceeded：",
      `  ${JSON.stringify(agentTask.budgetExceeded, null, 2)}`
    ].join("\n") : null,
    agentTask.error ? [
      "",
      "error：",
      `  ${JSON.stringify(agentTask.error, null, 2)}`
    ].join("\n") : null
  ].filter((part): part is string => part !== null);
  return parts.join("\n");
}

function agentTaskIdentityLines(task: AgentTaskRecord = EMPTY_AGENT_TASK, entry: PanelEntry = EMPTY_PANEL_ENTRY): TuiLine[] {
  return [
    line("任务", false, "cyan"),
    line(`标题：${truncate(task.title ?? entry.title ?? "子智能体任务", 120)}`),
  ...
  ].filter((item): item is TuiLine => item !== null);
}

function agentTaskProgressLines(task: AgentTaskRecord = EMPTY_AGENT_TASK): TuiLine[] {
  return [
    line("进度", false, "green"),
    task.latestProgress
...
function agentTaskBudgetLines(task: AgentTaskRecord = EMPTY_AGENT_TASK): TuiLine[] {
  const budget = task.budget ?? EMPTY_BUDGET;
  const progress = task.budgetProgress ?? EMPTY_BUDGET_PROGRESS;
  const lines = [
...
function agentTaskToolLines(task: AgentTaskRecord = EMPTY_AGENT_TASK): TuiLine[] {
  ...
  for (const [index, call] of calls.slice(-12).entries()) {
    ...
    const details = [
      call.inputSummary ? truncate(call.inputSummary, 72) : null,
      call.errorCode ? `error=${call.errorCode}` : null,
      call.decision ? `decision=${call.decision}` : null,
...
function agentTaskOutputPreviewLines(task: AgentTaskRecord = EMPTY_AGENT_TASK): TuiLine[] {
...
function agentTaskContinuationLines(task: AgentTaskRecord = EMPTY_AGENT_TASK): TuiLine[] {
  const lines: TuiLine[] = [];
...
function agentTaskStatusLabel(status: string | null | undefined) {
...
function previewMultiline(value: string | null | undefined, maxLines: number = 6, maxWidth: number = 120, options: PreviewMultilineOptions = EMPTY_PREVIEW_OPTIONS): TuiLine[] {
...
function truncateMultiline(value: string | null | undefined, max: number) {
...
function truncateJson(value: unknown, max: number) {
  try {
    ...
    return truncate(value == null ? "" : `${value}`, max);
  }
}

function normalizeResumeArchive(archive: AgentSession["transcriptArchive"] | null | undefined): ResumeArchive {
  const chunkSize = positiveInteger(archive?.chunkSize) ?? 50;
  const chunks = Array.isArray(archive?.chunks)
    ? archive.chunks.map((chunk) => normalizeResumeChunk(chunk)).filter((chunk): chunk is NonNullable<ReturnType<typeof normalizeResumeChunk>> => chunk !== null)
    : [];
  const totalFromChunks = chunks.reduce((sum, chunk) => sum + (chunk.messages ?? 0), 0);
  return {
    chunkSize,
...
function normalizeResumeChunk(chunk: ResumeChunk | Record<string, unknown> | null | undefined = EMPTY_RESUME_CHUNK) {
...
function resumeChunkRange(chunk: { index: number; messages: number }, chunkSize: number) {
  const start = ((chunk.index - 1) * chunkSize) + 1;
  const end = start + Math.max(0, chunk.messages) - 1;
  return chunk.messages > 0 ? `${start}-${end}` : "空";
}

function resumeChunkMessageLines(messages: ResumeMessage[] = []): TuiLine[] {
  if (messages.length === 0) {
    return [line("[空分片]", true)];
  }
  const rows: TuiLine[] = [];
  for (const [index, message] of messages.entries()) {
    const role = message?.role === "user" ? "你" : message?.role === "assistant" ? "Ant Code" : (message?.role ?? "message");
    const color = message?.role === "assistant" ? "green" : message?.role === "user" ? "cyan" : undefined;
    rows.push(line(`${`${index + 1}`.padStart(2, "0")}. ${role}`, false, color));
...
function resumeMessageText(content: unknown) {
  ...
  return content.map((item) => {
    ...
    if (item && typeof item === "object" && "text" in item) {
      return typeof item.text === "string" ? item.text : item.text == null ? "" : `${item.text}`;
    }
    return "";
  }).filter(Boolean).join("\n");
}
...
function assistantExcerptBodyLines(body: string, options: WrapExcerptOptions = EMPTY_WRAP_OPTIONS): TuiLine[] {
  const prefix = options.prefix ?? "";
  const blocks = asMarkdownBlocks(parseMarkdownBlocks(body));
  if (blocks.length === 0) {
    return [];
  }
  const rows: TuiLine[] = [];
...
function renderAssistantBodyForClipboard(body: string) {
  const blocks = asMarkdownBlocks(parseMarkdownBlocks(body));
  if (blocks.length === 0) {
    return "";
  }
...
function renderExcerptTableLines(block: MarkdownTableBlock, options: WrapExcerptOptions = EMPTY_WRAP_OPTIONS): TuiLine[] {
  const prefix = options.prefix ?? "";
...
function wrapExcerptText(value: string, options: WrapExcerptOptions = EMPTY_WRAP_OPTIONS): TuiLine[] {
  const prefix = options.prefix ?? "";
...
function excerptUiLine(text: string, color?: string) {
  return line(padExcerptLine(text), color ? false : true, color);
}

function padExcerptLine(text: string) {
  const value = text ?? "";
...
function messageRoleLabel(kind: string | undefined) {
  if (kind === "user") {
    return "你";
  }
  if (kind === "assistant") {
...
function messageActionPreview(entry: PanelEntry | null) {
  return (entry?.body ?? "").replace(/\s+/g, " ").trim() || "[空消息]";
}

function permissionMatrixRows(session: AgentSession): PermissionMatrixRow[] {
...
function formatContextBrief(summary: ContextSummary) {
  const localLimit = summary.modelMaxTokens ? `, 本地阈值=${formatTokenCount(summary.maxTokens)}` : "";
  const latestInput = Number.isFinite(summary.promptTokens)
    ? `, 最近输入=${formatTokenCount(summary.promptTokens)}`
    : "";
...
function formatProviderUsageBrief(summary: TokenUsageView = EMPTY_USAGE) {
  const parts = [
...
function taskProviderUsagePanelLines(progress: TokenUsageView | AgentTaskBudgetProgress = EMPTY_USAGE): TuiLine[] {
...
function formatCachedPromptUsage(cachedTokens: number | null | undefined, promptTokens: number | null | undefined) {
  const cached = formatTokenCount(cachedTokens);
  if (typeof cachedTokens !== "number" || typeof promptTokens !== "number" || !Number.isFinite(promptTokens) || promptTokens <= 0) {
    return cached;
  }
...
function primaryTokenLimit(summary: ContextSummary) {
  return summary.modelMaxTokens ?? summary.maxTokens;
}

function panel(value: CommandPanel): CommandPanel {
  return {
    tabs: [],
    tabIndex: 0,
    lines: [],
...
function asAgentTask(value: Record<string, unknown> | AgentTaskRecord | null | undefined): AgentTaskRecord {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return EMPTY_AGENT_TASK;
  }
  return value as AgentTaskRecord;
}

function asPanelEntry(value: PanelEntry | Record<string, unknown> | null | undefined): PanelEntry {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return EMPTY_PANEL_ENTRY;
  }
  const record = value as PanelEntry & Record<string, unknown>;
  return {
    ...
    excerptBody: typeof record.excerptBody === "string" ? record.excerptBody : undefined,
    taskId: typeof record.taskId === "string" ? record.taskId : undefined,
    taskStatus: typeof record.taskStatus === "string" ? record.taskStatus : undefined,
    profile: typeof record.profile === "string" ? record.profile : undefined,
    checkpointMessagesLength: typeof record.checkpointMessagesLength === "number" ? record.checkpointMessagesLength : undefined,
    task: record.task && typeof record.task === "object" ? record.task : null
  };
}

function asResumeChunk(value: ResumeChunk | Record<string, unknown> | null | undefined): ResumeChunk {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return EMPTY_RESUME_CHUNK;
  }
  return value as ResumeChunk;
}

function asCompactResult(value: CompactResult | Record<string, unknown> | null | undefined): CompactResult {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return EMPTY_COMPACT_RESULT;
  }
  return value as CompactResult;
}
