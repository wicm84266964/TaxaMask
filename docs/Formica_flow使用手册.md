...
![图 3-1 Formica-Flow 主界面总览：顶部菜单、三个主标签页、左侧图片列表、中间工作台和右侧控制区](manual_images/fig_3_1_main_window_overview.png)
...
![图 5-1 PDF Screener 总览：API 设置、逻辑方案、批量筛选参数与启动按钮](manual_images/fig_5_1_pdf_screener_overview.png)

![图 5-2 PDF Processing 提取页总览：提取输入、数据库输出、多模态验证与数据工具入口](manual_images/fig_5_2_pdf_extractor_overview.png)
...
![图 6-1 数据库浏览器：左侧记录表、右侧图像预览与文本证据详情](manual_images/fig_6_1_database_viewer.png)
...
![图 7-1 标注工作台总览：图片列表、画布、分类单元、结构标签、测量区和 AI 工作流区](manual_images/fig_7_1_labeling_workbench_overview.png)

看这张图时，建议按 **左 → 中 → 右** 的顺序理解：

- **左侧** 是项目图片列表，用来决定当前到底在处理哪张图。
- **中间** 是标注画布，图像、polygon、手工框、自动框都会在这里显示。
- **右侧** 是当前图的工作控制区，包含 `Taxon / 分类单元`、`Structures / 结构标签`、测量区和 AI 工作流按钮。

如果你想从工作台进入 Blink，重点看工作台上方工具条里的 **`Open in Blink Workbench / 在 Blink 工作台中打开`**。而“当前结构标签”并不是由画布自动猜出来的，而是由右侧 `Structures` 列表里你当前选中的标签决定。
...
![图 8-1 Cropper 裁剪器：左侧步骤与裁剪列表，右侧原图与裁剪框预览](manual_images/fig_8_1_cropper_dialog.png)

这张图里要重点看两个地方：

- **左侧面板**：上半部分是裁剪框列表，下面是保存入口。
- **右侧主图**：这里显示原图和你已经画出的裁剪框。

使用时，真正决定“生成哪些新图片”的不是左侧列表本身，而是你在右侧主图上画出来的裁剪框。点击 **`Save & Add to Project`** 后，程序会在原图同目录生成类似 `*_view_1.jpg`、`*_view_2.jpg` 的新图片，并把这些裁剪图自动加入当前项目。
...
![图 9-1 Blink 入口弹窗：明确选择目标部位与进入 ROI](manual_images/fig_9_1_blink_entry_dialog.png)

这张弹窗图请直接按“上框 / 下框”的方式理解：

- **上面的下拉框（Target Part）**：这次真正要精修的部位。
- **下面的下拉框（Entry ROI）**：你通过哪个已有区域进入局部视图，通常来自手工框或自动框。
...
![图 9-2 Blink 工作台：左侧专家分类、中间局部视图、右侧控制区与训练区](manual_images/fig_9_2_blink_workbench_focused_session.png)

看这张图时，建议按 **左 / 中 / 右** 三块理解：

- **左侧**：专家分类、绘制模式和已训练专家模型列表。
- **中间**：局部精修视图。这里会显示通过 Entry ROI 进入后的局部图像，以及当前目标部位的局部标注状态。
- **右侧**：Blink 控制区和训练区，包含 `AUTO-ANNOTATE DRAFT`、`EXECUTE AUTO-SHRINK`、`APPLY TO GLOBAL`、`TRAIN EXPERT MODEL` 等按钮。

这张示例图的阅读逻辑是：先通过较大的 ROI 进入局部区域，再在中间画布里精修当前目标部位，最后根据需要决定是只生成 trajectory，还是正式把结果写回主项目。
...
1. 当前 active expert 先在当前 Entry ROI 里给出一个更准的小框
...
- 当前 target part 还没有训练/激活专家模型

比如你在 Blink 里把目标设为 `Eye`，但 `Eye` 还没有 active expert，这个按钮就不会成功。
...
![图 11-1 导出数据集对话框：导出格式选择与导出目录设置](manual_images/fig_11_1_export_dataset_dialog.png)
