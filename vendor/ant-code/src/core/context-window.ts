import { resolveModelContextTokens } from "../model-gateway/models.ts";
import { createOpenAIChatCompletionRequest } from "../model-gateway/openai-chat.ts";
import { createOpenAIResponsesRequest } from "../model-gateway/openai-responses.ts";
import { createAnthropicMessagesRequest } from "../model-gateway/anthropic-messages.ts";
import { createGatewayRequest } from "../model-gateway/protocol.ts";
import { createInternalAgentRequest } from "../agents/internal.ts";
import { runHooks } from "../hooks/runner.ts";
import { normalizeProviderUsageAggregate } from "./provider-usage.ts";
...
export type ContextWindow = {
  ...
  lastInternalAgent: string | null;
};

type ContextSession = {
  config?: Parameters<typeof createContextWindow>[0];
  id?: string;
  cwd?: string;
  env?: NodeJS.ProcessEnv;
  messages?: Array<Record<string, unknown>>;
  contextWindow?: ContextWindow;
  lastPromptEstimate?: Record<string, unknown> | null;
  usage?: unknown;
 ...
 */
export function compactSessionContext(session: { config?: Record<string, unknown>; messages?: Array<Record<string, unknown>>; contextWindow?: ReturnType<typeof createContextWindow> }, options: { force?: boolean; reason?: string; keepRecentMessages?: number } = {}) {
...
export async function compactSessionContextWithModel(session: { config?: Record<string, unknown>; id?: string; cwd?: string; env?: NodeJS.ProcessEnv; messages?: Array<Record<string, unknown>>; contextWindow?: ReturnType<typeof createContextWindow>; lastPromptEstimate?: Record<string, unknown>; usage?: Record<string, unknown>; goal?: Record<string, unknown> }, options: { force?: boolean; reason?: string; keepRecentMessages?: number; env?: NodeJS.ProcessEnv; hooksTrusted?: boolean; gateway?: { configured?: boolean; sendChat?: (request: {
    messages: Array<Record<string, unknown>>;
  ...
  }) => Promise<unknown> }; signal?: AbortSignal; onBeforeCompact?: (payload: Record<string, unknown>) => void | Promise<void> } = {}): Promise<ContextCompactResult> {
  session.contextWindow ??= createContextWindow(session.config ?? {});
  const window = session.contextWindow;
  const messages = Array.isArray(session.messages) ? session.messages : [];
  const bytes = estimateMessagesBytes(messages);
  ...
  let response: { ok?: boolean; error?: { code?: unknown }; data?: { text?: unknown; model?: unknown } };
  try {
  ...
  if (!response?.ok) {
    const code = typeof response?.error?.code === "string" ? response.error.code : "model_compaction_failed";
  ...
  const result = applyModelCompactionSummary(session, plan, summaryText, {
    reason: options.reason ?? (overBudget ? "budget" : "manual"),
    model: typeof response.data?.model === "string" ? response.data.model : null
  });
  await emitCompactAfterHook(session, options, beforePayload, result);
  return result;
}
 ...
 * @param {{ config?: Record<string, any>; messages?: Array<Record<string, any>>; contextWindow?: ReturnType<typeof createContextWindow> }} session
 */
export function clearSessionContext(session: { config?: Record<string, unknown>; messages?: Array<Record<string, unknown>>; contextWindow?: ReturnType<typeof createContextWindow> }) {
  session.messages = [];
  session.contextWindow = createContextWindow(session.config ?? {});
 ...
 */
export function buildCompactedContextMessage(session: { contextWindow?: ReturnType<typeof createContextWindow> }) {
 ...
 * @param {{ config?: Record<string, any>; messages?: Array<Record<string, any>>; contextWindow?: ReturnType<typeof createContextWindow> }} session
 */
export function summarizeContextWindow(session: { config?: Parameters<typeof createContextWindow>[0]; messages?: unknown[]; contextWindow?: ContextWindow; lastPromptEstimate?: unknown; usage?: unknown }) {
  ...
  return {
    ...
    promptRound: typeof promptEstimate?.round === "number" && Number.isFinite(promptEstimate.round) ? promptEstimate.round : null,
 ...
 * @param {{ config?: Record<string, any>; messages?: Array<Record<string, any>>; contextWindow?: ReturnType<typeof createContextWindow> }} session
 */
export function formatContextWindowStatus(session: { config?: Record<string, unknown>; messages?: Array<Record<string, unknown>>; contextWindow?: ReturnType<typeof createContextWindow> }) {
 ...
 */
function createCompactionPlan(session: { config?: Record<string, unknown>; messages?: Array<Record<string, unknown>>; contextWindow?: ReturnType<typeof createContextWindow> }, options: { keepRecentMessages?: number }, metrics: { bytes: number; tokens: number; keepCount: number; splitAt: number }) {
  const messages = Array.isArray(session.messages) ? session.messages : [];
 ...
 */
function buildModelCompactionRequest(session: { contextWindow?: ContextWindow; goal?: Record<string, unknown>; config?: Record<string, unknown>; cwd?: string }, plan: ReturnType<typeof createCompactionPlan>): {
  messages: Array<Record<string, unknown>>;
  tools?: Array<Record<string, unknown>>;
  toolResults?: Array<Record<string, unknown>>;
} {
  ...
  return internal.ok && internal.request
    ? {
      messages: internal.request.messages as Array<Record<string, unknown>>,
      tools: (internal.request as { tools?: Array<Record<string, unknown>> }).tools ?? [],
      toolResults: (internal.request as { toolResults?: Array<Record<string, unknown>> }).toolResults ?? []
    }
    : {
      messages: [
        {
 ...
 */
function formatMessagesForModelCompaction(messages: Array<Record<string, unknown>>) {
  const lines: string[] = [];
  for (const [index, message] of messages.entries()) {
    ...
    const toolCalls = Array.isArray(message.toolCalls) && message.toolCalls.length > 0
      ? ` toolCalls=${message.toolCalls.map((call) => isRecord(call) ? call.name : undefined).filter(Boolean).join(",") || message.toolCalls.length}`
      : "";
...
function selectRecentConversationContext(messages: Array<Record<string, unknown>>, window: ContextWindow, options: { keepRecentMessages?: number } = {}) {
  const explicit = Number.isInteger(options.keepRecentMessages) ? Number(options.keepRecentMessages) : null;
  if (explicit !== null) {
    return {
 ...
 */
function applyModelCompactionSummary(session: { messages?: Array<Record<string, unknown>>; contextWindow?: ContextWindow }, plan: ReturnType<typeof createCompactionPlan>, summaryText: string, options: { reason: string; model?: string | null }) {
  const window = session.contextWindow as ContextWindow;
 ...
 */
function fallbackLocalCompaction(session: Parameters<typeof compactSessionContext>[0], options: Parameters<typeof compactSessionContext>[1], fallbackReason: string) {
...
async function emitCompactAfterHook(session: { config?: unknown; cwd?: unknown; id?: unknown }, options: { env?: unknown; hooksTrusted?: unknown }, beforePayload: Record<string, unknown>, result: ContextCompactResult) {
  return runHooks({
    config: session.config,
    cwd: session.cwd,
    env: options.env,
 ...
 * @param {unknown} value
 */
function extractMessageText(value: unknown) {
 ...
 */
function mergeSummaries(previous: string, next: string, maxBytes: number) {
  const sections = [];
  if (previous?.trim()) {
 ...
 * @param {Array<Record<string, any>>} messages
 */
function estimateMessagesBytes(messages: unknown) {
 ...
 * @param {Array<Record<string, any>>} messages
 */
function estimateMessagesTokens(messages: unknown) {
 ...
 */
export function estimatePromptPayload(input: { model?: string; messages?: unknown[]; tools?: unknown[]; toolResults?: unknown[]; gatewayProtocol?: string } = {}) {
  ...
  const toolResults = (Array.isArray(input.toolResults) ? input.toolResults : []) as Array<Record<string, unknown>>;
  const messageBytes = estimateMessagesBytes(messages);
...
function toolResultsNotRepresentedInMessages(messages: Array<Record<string, unknown>> = [], toolResults: Array<Record<string, unknown>> = []) {
...
function toolResultId(value: unknown) {
  if (!value || typeof value !== "object") {
    return "";
  }
  const record = value as Record<string, unknown>;
  return String(record.toolCallId ?? record.tool_call_id ?? record.id ?? "").trim();
}
 ...
 * @param {string} value
 */
function redactContextText(value: string) {
  return value
    .replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g, "")
 ...
 * @param {number} maxBytes
 */
function truncateBytesFromStart(value: string, maxBytes: number) {
