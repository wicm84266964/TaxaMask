...
async function getFileContents(args: Record<string, unknown>) {
...
function ownerRepo(args: Record<string, unknown>) {
...
function requiredSlug(value: unknown, name: string) {
...
function perPage(value: unknown) {
...
function boundedPositive(value: unknown, fallback: number, min = 1024, max = 256_000) {
  const number = Number(value);
...
function respond(id: unknown, result: unknown, error: { code: number; message: string } | null = null) {
  const message = error
