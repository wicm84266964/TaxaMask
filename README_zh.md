...
- macOS 可以尝试轻量 CPU 复核和小规模标注，但 Apple Silicon 加速不是当前公开发布版的验证目标。
...
TaxaMask 1.2.0 起，新建 2D 形态项目以一个小型 SQLite manifest 加 `.taxamask.sqlite` 数据库作为主项目记录。这样在日常标注、VLM 预标注、候选图导入和模型复核时，不需要反复整文件覆写一个越来越大的 JSON。旧 2D 项目 JSON 仍可打开并迁移，也可以导出为 legacy JSON 兼容快照；但对正式大项目来说，旧 JSON 应视为交换/审计文件，而不是最稳妥的生产主存储。
