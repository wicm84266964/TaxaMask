 ...
 */
type AntEventPayload = Record<string, unknown>;
type AntEvent = {
...
export function createAntEventNormalizer(options: { sessionId: string; now?: () => string; idFactory?: (sequence: number, type: string) => string }) {
  let sequence = 0;
  ...
  const idFactory = options.idFactory ?? ((nextSequence: unknown, type: string) => `${options.sessionId}:${String(nextSequence).padStart(6, "0")}:${type}`);

  /**
   * @param {Record<string, any>} legacyEvent
   * @returns {Array<Record<string, any>>}
   */
  function normalize(legacyEvent: Record<string, unknown>) {
    ...
    if (type === "gateway_request_start") {
      activeRound = asFiniteNumber(legacyEvent.round) ?? activeRound;
    ...
    if (type === "tool_calls_requested") {
      ...
      for (const rawCall of calls) {
        ...
        events.push(makeEvent("tool_use_start", legacyEvent, {
          round: asFiniteNumber(legacyEvent.round) ?? activeRound,
          toolUseId: asStringOrNull(call.id),
          name: asStringOrNull(call.name),
          inputKeys: Array.isArray(call.inputKeys) ? call.inputKeys : []
        }, {
          source: "gateway",
          parentToolUseId: asStringOrNull(call.id)
        }));
      }
      return events;
    }
    ...
    if (type === "tool_start") {
      events.push(makeEvent("tool_start", legacyEvent, {
        toolUseId: asStringOrNull(legacyEvent.toolCallId),
        name: asStringOrNull(legacyEvent.name),
        inputKeys: Array.isArray(legacyEvent.inputKeys) ? legacyEvent.inputKeys : []
      }, {
        source: "tool",
        parentToolUseId: asStringOrNull(legacyEvent.toolCallId)
      }));
      return events;
    }
    ...
    if (type === "tool_finish") {
      events.push(makeEvent("tool_result", legacyEvent, {
        ...
        errorCode: asStringOrNull(legacyEvent.errorCode),
    ...
    if (type === "delegation_guard") {
      events.push(makeEvent("delegation_guard", legacyEvent, {
        ...
        reason: asStringOrNull(legacyEvent.reason),
    ...
    if (type === "gateway_error" || type === "gateway_not_configured") {
      const error = isRecord(legacyEvent.error) ? legacyEvent.error : {};
      events.push(makeEvent("gateway_error", legacyEvent, {
        code: asStringOrNull(error.code) ?? (type === "gateway_not_configured" ? "GATEWAY_NOT_CONFIGURED" : "GATEWAY_ERROR"),
        message: error.message ? redactText(String(error.message)) : null,
   ...
   * @param {Record<string, any>} legacyEvent
   */
  function ensureMessage(events: AntEvent[], legacyEvent: Record<string, unknown>) {
    if (activeMessage) {
      return;
    }
    activeMessage = true;
   ...
   */
  function ensureText(events: AntEvent[], legacyEvent: Record<string, unknown>, partial: boolean) {
    if (activeText) {
      return;
    }
    activeText = true;
   ...
   * @param {Record<string, any>} legacyEvent
   */
  function closeText(events: AntEvent[], legacyEvent: Record<string, unknown>) {
    if (!activeText) {
      return;
    }
   ...
   * @param {Record<string, any>} legacyEvent
   */
  function closeThinking(events: AntEvent[], legacyEvent: Record<string, unknown>) {
    if (!activeThinking) {
      return;
    }
   ...
   */
  function makeEvent(type: string, legacyEvent: Record<string, unknown>, payload: Record<string, unknown>, overrides: { source?: string; visibility?: string; persistence?: string; redaction?: string; parentToolUseId?: string | null } = {}): AntEvent {
    sequence += 1;
    return {
      ...
      turnId: activeTurnId,
      round: asFiniteNumber(payload.round) ?? activeRound,
      parentId: null,
      parentToolUseId: overrides.parentToolUseId ?? asStringOrNull(legacyEvent.parentToolUseId),
 ...
 */
export function isPrintableAntEvent(event: Record<string, unknown>, options: { includePartialMessages?: boolean } = {}) {
  if (!event || event.persistence === "memory") {
    return false;
  }
  const payload = isRecord(event.payload) ? event.payload : {};
  if (!options.includePartialMessages && PARTIAL_EVENT_TYPES.has(String(event.type ?? "")) && payload.partial === true) {
    return false;
  }
  return true;
}
 ...
 * @param {Record<string, any>} event
 */
export function sanitizeAntEventForPersistence(event: Record<string, unknown>) {
  if (!event || event.persistence === "memory") {
    return null;
  }
 ...
 */
export function createAntJsonOutput(output: { sessionId: string; events: Array<Record<string, unknown>>; result: Record<string, unknown> }) {
  return {
    schemaVersion: ANT_EVENT_SCHEMA_VERSION,
...
function sanitizeChangeStats(value: unknown, options: Record<string, unknown> = {}) {
  if (!isRecord(value)) {
    return null;
  }
