...
> 适用版本：2026-05-22 Agent Center / TIF-STL 分流 / 主工作台 Blink 合并版
>
...
9. [主工作台内 Blink 父子精修：轨迹积累与专家训练](#9-主工作台内-blink-父子精修轨迹积累与专家训练)
...
2. **2D/STL Morphology 工作流**
   ...
   - 进入后主要使用 Labeling Workbench。Blink 父子精修已经合并到主标注工作台右侧，不再需要日常切换到独立 Blink Workbench。
...
3. **2D/STL 形态学标注与训练线**
   ...
   - 在主标注工作台右侧的父子精修 / Blink 区里做局部精修和轨迹积累
...
6. **在对应工作台里做正式人工标注或修订**
7. **需要细部位时，在主标注工作台右侧的父子精修 / Blink 区完成精修**
...
2. **2D/STL 模式**：显示 `Labeling Workbench`，用于普通形态图像和 STL 渲染视角图；子部位的 Blink 父子精修在这个主标注工作台右侧完成。
...
当前 STL 路线不是直接打开一个 3D 模型渲染视图让你在 mesh 上涂色，而是把一批渲染好的视角图注册到现有 Labeling Workbench。这样可以继续复用已经验证过的 polygon 标注、Locator/SAM、父子精修 / Blink 和导出逻辑。

如果你要做小部位精修，重点看右侧 **`Parent-child refinement / Blink`** 小区，而不是再找独立 Blink 工作台入口。当前结构标签由 `Structures` 列表里你选中的标签决定：选中 `Head` 这类父部位时画的是父级上下文框；选中 `Mandible` 这类子部位时，右侧会显示当前父级、父级框和路由专家状态。
...
当你选中子部位时，例如 `Mandible`，`Annotation Box` 默认自由比例，不会继承 `Head` 的固定比例。右侧父子精修区里还可以把框用途切换为：

- `Annotation box / 正式标注框`：保存子部位正式框。
- `Loose shrink box / 收缩松框`：只作为自动收缩起点，不覆盖正式标注框。
...
## 9. 主工作台内 Blink 父子精修：轨迹积累与专家训练

当前版本里，Blink 已经不再作为日常必须进入的独立工作台。它的高频能力被合并到 2D/STL `Labeling Workbench` 右侧的 **`Parent-child refinement / Blink`** 小区。
...
> **大部位先在主工作台解决，细部位也留在主工作台里借助父子精修完成。**
...
### 9.4 框用途：正式标注框 vs 收缩松框

选中子部位时，右侧父子精修区有两个框用途：
...
`Annotation box` 会更新当前子部位的正式框，和 polygon 一起作为项目标注的一部分。

`Loose shrink box` 只保存到 `shrink_loose_boxes`，用于自动收缩 trajectory 的起点。它不会覆盖正式子部位框。

`Box Prompt (SAM)` 仍然只是 SAM 临时提示框，不会写入正式框角色。

### 9.5 Auto-annotate child / 自动标注子部位
...
这个按钮只训练当前父子上下文下的子部位专家。训练参数继续来自 `2D/STL Model Settings` 里的 Blink 默认参数。

它不会把所有部位混在一起训练，也不会把普通 polygon 全部当成 Blink trajectory。真正关键的是：

> **这个子部位是否已经积累了足够多、足够可靠的 trajectory。**

训练完成后，新模型会作为 route candidate 出现在 `2D/STL Model Settings` 的路由专家配置中。是否启用、指定或替换专家，仍然在设置里的路由管理面板完成。
...
5. 如果已有 route expert，点击 `Auto-annotate child` 起草。
6. 手工修准 Mandible polygon。
7. 切到 `Loose shrink box`，画一个覆盖 Mandible 的松框。
...
- 细部位可以后续用 SAM、主工作台父子精修 / Blink 专家或外部脚本后端继续精修
...
这组参数不是主模型训练参数，而是主工作台父子精修 / Blink 区训练细部位专家时的默认值。
...
| Configure Route Expert | 打开当前父子路由专家配置 | 项目 route manifest |
| Auto-annotate child | 路由专家框 -> 基础 SAM 草稿 | 当前子部位草稿 |
| Loose shrink box | 绘制自动收缩起点框 | `shrink_loose_boxes` |
| Run auto-shrink | 生成并保存 trajectory | Blink 训练数据层 |
| Train current child expert | 训练当前父子路由的子部位专家 | Blink 专家候选模型 |
...
普通 2D/STL 标注现在优先使用 14.1 中的主工作台父子精修按钮。这个兼容 Blink 回退界面主要用于旧项目检查、开发验证或特殊排错。
...
因为 Blink 父子精修现在的正确定位不是“从整张图重新开始找”，而是：

> 基于已有大部位或已有 ROI，进入局部精修。

**步骤 2：确认父级上下文和路由**

1. 看右侧 `Parent-child refinement / Blink`
...
- 如果当前 route 已经有专家，用 `Auto-annotate child` 先起草
- 如果你更相信自己的 prompt 框，用 `Box Prompt (SAM)` 先给基础 SAM 起草
- 用 `Draw Polygon` 精修当前目标部位
- 用 `Annotation Box` + `Loose shrink box` 画松框
