...
type DelegationGuardState = {
  config: DelegationGuardConfig;
  sessionId: unknown;
  cwd: unknown;
  prompt: string;
  complexPrompt: boolean;
  broadActions: number;
  networkActions: number;
  repositoryActions: number;
...
type ClassifyContext = {
  cwd?: unknown;
  complexPrompt?: unknown;
  readFiles?: unknown;
  grepPatterns?: unknown;
};

type DelegationReminder = {
  level: unknown;
  text: string;
  reason: unknown;
  broadActions: unknown;
...
export function createDelegationGuard(options: { config?: unknown; cwd?: unknown; sessionId?: unknown; prompt?: unknown } = {}) {
  const config = normalizeDelegationGuardConfig(options.config);
  const state: DelegationGuardState = {
    config,
  ...
  return {
    ...
    observeToolResult(toolName: unknown, input: Record<string, unknown> = {}, execution: unknown = {}) {
      ...
      const weight = observation.weight ?? 0;
      state.broadActions += weight;
      if (observation.category === "network") {
        state.networkActions += weight;
      } else if (observation.category === "repository") {
        state.repositoryActions += weight;
      }
      if (observation.readPath) {
        state.readFiles.add(observation.readPath);
      }
      if (observation.readBytesEstimate && observation.readBytesEstimate > 0) {
...
export function normalizeDelegationGuardConfig(config: unknown = {}) {
  const raw = asRecord(asRecord(asRecord(config).agents).delegationGuard);
...
export function classifyToolUse(toolName: unknown, input: Record<string, unknown> = {}, execution: unknown = {}, context: ClassifyContext = {}): ToolObservation {
  const name = String(toolName ?? "");
  if (name === "agent_run") {
    return { broad: false, delegated: asRecord(execution).ok !== false };
  }
  if (name === "web_search") {
    return {
      broad: true,
...
export function appendDelegationReminderToExecution(execution: Record<string, unknown>, reminder: DelegationReminder | null | undefined) {
...
function classifyWebFetch(input: Record<string, unknown>, context: ClassifyContext): ToolObservation {
...
function classifyListFiles(input: Record<string, unknown>, context: ClassifyContext): ToolObservation {
...
function classifyGlob(input: Record<string, unknown>, context: ClassifyContext): ToolObservation {
...
function classifyGrep(input: Record<string, unknown>, context: ClassifyContext): ToolObservation {
  const target = normalizeRelativePath(input.path ?? ".");
...
function classifyReadFile(input: Record<string, unknown>, execution: unknown, context: ClassifyContext): ToolObservation {
...
function classifyShell(input: Record<string, unknown>, context: ClassifyContext): ToolObservation {
...
function buildReminderIfNeeded(state: DelegationGuardState, observation: ToolObservation) {
  if (state.delegated) {
    return null;
  }
...
function createReminder(level: unknown, state: DelegationGuardState, observation: ToolObservation): DelegationReminder {
...
function suggestedProfilesForState(state: DelegationGuardState) {
...
function isBroadDirectory(value: unknown) {
...
function estimateReadBytes(execution: unknown, input: Record<string, unknown>) {
  const result = asRecord(asRecord(execution).result);
  const bytesRead = result.bytesRead;
  if (typeof bytesRead === "number" && Number.isFinite(bytesRead)) {
    return bytesRead;
  }
