...
Context compaction summaries are process-local model context. Persisted metadata stores only counts, byte totals, compaction counts, and timestamps; it does not store raw prompts, assistant replies, tool outputs, or compacted summary text.
