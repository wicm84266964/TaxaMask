...
type TuiTheme = typeof DEFAULT_TUI_THEME;
type InkWrap = "truncate" | "wrap" | "end" | "middle" | "truncate-end" | "truncate-middle" | "truncate-start";

type TuiComposerSegment = {
  text?: string;
  cursor?: boolean;
  hidden?: boolean;
};
...
type TuiTaskRecord = {
  ...
  budgetProgress?: TuiTaskBudgetProgress;
  budgetExceeded?: { kind?: string } | boolean;
  toolCalls?: TuiTaskToolCall[];
  children?: TuiTaskRecord[];
...
type TuiWorkflowChange = TuiWorkflowItem & {
  created?: boolean;
  edited?: boolean;
...
type TuiWorkflowValidation = TuiWorkflowItem & {
  passed?: boolean;
  command?: string;
  toolName?: string;
};
...
type TuiWorkflow = {
  todos: TuiWorkflowItem[];
  plan: {
    explanation: string;
    steps: TuiWorkflowItem[];
  };
  changes: TuiWorkflowChange[];
  validations: TuiWorkflowValidation[];
};

type ContextWindowSummary = ReturnType<typeof summarizeContextWindow>;

type FooterHintOptions = {
  sideView?: string;
  wide?: boolean;
  detailMode?: string;
  thinkingVisible?: boolean;
...
type TuiViewProps = {
  ...
  visibleRows?: number;
  entry?: TuiEntry;
  palette?: SlashPaletteState | null;
  index?: number;
  state?: FileMentionState | null;
  candidates?: FileMentionCandidate[];
  queuedPrompts?: string[];
  models?: LabModel[];
  currentModel?: string;
  panel?: TuiCommandPanel | null;
  offset?: number;
  visible?: TuiLine[];
  lines?: TuiLine[];
  firstRow?: number;
  ...
  wide?: boolean;
  trusted?: boolean;
  workspaceDiagnostic?: { warning?: string } | null;
  status?: string;
  label?: string;
};

const EMPTY_STREAM: TuiStream = {};
const EMPTY_WORKFLOW: TuiWorkflow = {
  todos: [],
  plan: { explanation: "", steps: [] },
  changes: [],
  validations: []
};
const EMPTY_FOOTER_HINT: FooterHintOptions = {};
const EMPTY_ACTIVITY: TuiActivity = {
  status: "idle",
  gateway: "",
  lastGateway: "",
  lastTool: "",
...
function isStaticActivityStatus(status: string) {
        ...
        h(Box, { flexDirection: "column", width: Math.max(1, (width ?? 100) - 6), flexShrink: 1 },
          paddedLines.map((item, index) => {
            ...
            return h(Text, {
              ...
              dimColor: selected ? false : Boolean(item.dim),
...
export function resolveLogPaneLayout({ height, streamActive = false, scrollbackMode = false, transcriptTotalRows = 0 }: { height?: number; streamActive?: boolean; scrollbackMode?: boolean; transcriptTotalRows?: number } = {}) {
...
export function transcriptScrollBar(viewport: { totalRows?: number; lastRow?: number; firstRow?: number } = {}, rows: number = 1) {
  ...
  return Array.from({ length: height }, (_value, index: number) => {
    const active = index >= thumbStart && index < thumbStart + thumbRows;
    return { char: active ? "█" : "│", active };
  });
}

export function SidePanel({ view, session, activity, sidePanelOffset = 0, taskRecords, taskGroupRecords, workflowFilter = "incomplete", taskFilter = "active", visibleRows, width, height, theme = DEFAULT_TUI_THEME }: TuiViewProps & { session: AgentSession }) {
...
export function sidePanelTabsText(activeView: string = "") {
  return SIDE_VIEWS.map((view) => view === activeView
    ? `[${panelTitle(view)}]`
    : panelTitle(view)).join(" ");
}

export function sidePanelViewport(lines: TuiLine[] | unknown = [], visibleRows: number = 12, offset: number = 0) {
...
export function CompactSideSummary({ session, activity }: TuiViewProps & { session: AgentSession }) {
  const context = summarizeContextWindow(session);
  const inputBrief = Number.isFinite(context.promptTokens)
    ? ` | 最近输入=${formatTokenCount(context.promptTokens)}`
    : "";
  const providerBrief = Number.isFinite(context.providerPromptTokens)
...
export function StartupSplash({ session, theme = DEFAULT_TUI_THEME }: TuiViewProps & { session: AgentSession }) {
  const lines = startupBannerLines(session);
  return h(Box, { flexDirection: "column", paddingX: 1, paddingY: 1, width: "100%" },
    ...lines.map((item: string, index: number) => h(Text, {
      key: index,
...
function LogEntry({ entry, theme = DEFAULT_TUI_THEME }: TuiViewProps & { entry: TuiEntry }) {
...
function StartupEntry({ entry, theme = DEFAULT_TUI_THEME }: TuiViewProps & { entry: TuiEntry }) {
  const body = visibleBodyLines(entry.body);
  return h(Box, { flexDirection: "column", marginBottom: 1, paddingY: 1 },
    ...body.map((item: string, index: number) => h(Text, {
      key: index,
      color: index < 5 ? themeColor(theme, "identity", "cyan") : index === 6 ? themeColor(theme, "success", "green") : undefined,
      dimColor: index > 6,
      wrap: "wrap"
...
function ConversationEntry({ entry, theme = DEFAULT_TUI_THEME }: TuiViewProps & { entry: TuiEntry }) {
  ...
  return h(Box, { flexDirection: "column", marginBottom: 1 },
    ...
    ...(body.length === 0
      ...
      : body.map((item: string, index: number) => h(Text, { key: index, wrap: "wrap" }, `  ${item}`)))
  );
}

function LiveStreamEntry({ stream = EMPTY_STREAM, pulse = 0, detailMode = "compact", width = 80, visibleRows = 5, scrollOffset = 0, scrollbackMode = false, theme = DEFAULT_TUI_THEME }: TuiViewProps) {
  const rowBudget = Math.max(1, Number(visibleRows ?? 5) - 2);
...
function TranscriptEntry({ entry, theme = DEFAULT_TUI_THEME }: TuiViewProps & { entry: TuiEntry }) {
...
function OutputEntry({ entry, theme = DEFAULT_TUI_THEME }: TuiViewProps & { entry: TuiEntry }) {
  const body = visibleBodyLines(entry.body);
  return h(Box, { flexDirection: "column", marginBottom: 1 },
...
function MetaEntry({ entry, theme = DEFAULT_TUI_THEME }: TuiViewProps & { entry: TuiEntry }) {
...
export function SlashPalette({ palette, index = 0, width, height, visibleRows, theme = DEFAULT_TUI_THEME }: TuiViewProps) {
...
export function FileMentionPalette({ state, candidates, index = 0, width, height, visibleRows, theme = DEFAULT_TUI_THEME }: TuiViewProps) {
  if (!state) {
    return null;
  }
...
export function QueuedPromptLine({ queuedPrompts, theme = DEFAULT_TUI_THEME }: TuiViewProps) {
...
export function ModelPicker({ models, currentModel, index = 0, width, height, visibleRows, theme = DEFAULT_TUI_THEME }: TuiViewProps) {
  const list = models ?? [];
  if (list.length === 0) {
    return null;
  }
...
export function CommandPanel({ panel, offset = 0, visibleRows = 12, width, height, theme = DEFAULT_TUI_THEME }: TuiViewProps) {
  if (!panel) {
    return null;
  }
  ...
  const bodyWrap: InkWrap = panel.wrap === "wrap" ? "wrap" : "truncate";
  return h(Box, {
    flexDirection: "column",
    width,
    height: rowBudget,
    ...
    tabs.length > 0 ? h(Text, { dimColor: true, wrap: "truncate" },
      tabs.map((tab: unknown, index: number) => index === panel.tabIndex ? `[${tab}]` : ` ${tab} `).join("  ")
    ) : null,
...
function BorderlessCommandPanel({ panel, visible = [], lines = [], firstRow, lastRow, totalRows, width, height, theme = DEFAULT_TUI_THEME }: TuiViewProps & { panel: TuiCommandPanel }) {
  const rowBudget = Number(height) || undefined;
  const footerText = commandPanelFooterText(panel.footer);
...
export function commandPanelViewport(panel: TuiCommandPanel | null | undefined, offset: number = 0, visibleRows: number = 12) {
...
export function PromptBox({ mode = "input", busy, inputBuffer, inputCursor, inputVisibleStart, questionBuffer, questionCursor, questionVisibleStart, queuedPrompts, pendingApproval, pendingQuestion, pulse = 0, width, height, theme = DEFAULT_TUI_THEME }: TuiViewProps) {
...
export function PermissionModal({ pendingApproval, focusedIndex = 0, width, height, theme = DEFAULT_TUI_THEME }: TuiViewProps) {
  if (!pendingApproval) {
    return null;
  }
...
export function ExitConfirmNotice({ active, busy = false, theme = DEFAULT_TUI_THEME }: TuiViewProps) {
  if (!active) {
    return null;
  }
...
export function InterruptConfirmNotice({ active, theme = DEFAULT_TUI_THEME }: TuiViewProps) {
  if (!active) {
    return null;
  }
...
export function FooterBar({ sideView, wide, detailMode = "compact", thinkingVisible = false }: TuiViewProps) {
...
export function PermissionFooter({ session, width = 100, theme = DEFAULT_TUI_THEME }: TuiViewProps & { session: AgentSession }) {
...
function truncateSingleLine(value: unknown, max: unknown) {
...
function permissionFooterColor(mode: string, theme: TuiTheme) {
  if (mode === "fullAccess") {
...
export function footerHintText({ sideView, wide, detailMode = "compact", thinkingVisible = false }: FooterHintOptions = EMPTY_FOOTER_HINT) {
...
function sideFooterHint(sideView: string = "") {
...
export function StartupConfirmDialog({ cwd = "", trusted = false, workspaceDiagnostic = null, theme = DEFAULT_TUI_THEME }: TuiViewProps) {
...
export function TrustDialog({ cwd = "", status = "needed", theme = DEFAULT_TUI_THEME }: TuiViewProps) {
...
function statusPanelLines(session: AgentSession, activity: TuiActivity = EMPTY_ACTIVITY) {
  const context = summarizeContextWindow(session);
  const workflow: TuiWorkflow = session.workflow ?? EMPTY_WORKFLOW;
  return [
    line("会话", true),
    ...
    line(`模型消息链：${formatTokenCount(context.messageTokens)} tokens`, true),
...
function workflowPanelLines(session: AgentSession, filter: string = "incomplete") {
  const workflow: TuiWorkflow = session.workflow ?? EMPTY_WORKFLOW;
...
function workflowItemText(item: TuiWorkflowItem) {
  return String(item?.content ?? item?.title ?? item?.text ?? "");
}

function sideSubtabsText(filters: readonly string[], labels: Record<string, string>, active: string) {
  return filters.map((filter) => filter === active
    ? `[${labels[filter] ?? filter}]`
    : labels[filter] ?? filter).join(" ");
}

function normalizeWorkflowFilter(value: string) {
  return (WORKFLOW_FILTERS as readonly string[]).includes(value) ? value : WORKFLOW_FILTERS[0];
}

function filterWorkflowItems(items: TuiWorkflowItem[] | unknown, filter: string): TuiWorkflowItem[] {
  const list: TuiWorkflowItem[] = Array.isArray(items) ? items : [];
  if (filter === "all") {
    return list;
  }
  if (filter === "completed") {
...
function taskPanelLines(taskRecords: TuiTaskRecord[] | unknown = [], filter: string = "active", taskGroupRecords: TuiTaskGroupRecord[] | unknown = []) {
  ...
  const tree = buildTaskTree(visibleTasks) as TuiTaskRecord[];
  const rows = [
...
function pushTaskGroupPanelNode(rows: TuiLine[], group: TuiTaskGroupRecord) {
...
function pushTaskPanelNode(rows: TuiLine[], task: TuiTaskRecord, depth: number) {
  ...
  if (task.status === "partial" && task.budgetExceeded && typeof task.budgetExceeded === "object") {
...
function pushTaskTokenPanelRows(rows: TuiLine[], progress: TuiTaskBudgetProgress | undefined = {}, indent: string = "") {
  ...
  for (const item of taskProviderUsageLines(progress, indent)) {
    if (item) {
      rows.push(item);
    }
  }
...
function taskProviderUsageLines(progress: TuiTaskBudgetProgress | undefined = {}, indent: string = "") {
  if (!progress || typeof progress !== "object") {
    return [];
  }
...
function latestTaskTool(task: TuiTaskRecord) {
...
function normalizeTaskFilter(value: string) {
  return (TASK_FILTERS as readonly string[]).includes(value) ? value : TASK_FILTERS[0];
}

function filterTasks(tasks: TuiTaskRecord[] | unknown, filter: string): TuiTaskRecord[] {
  const list: TuiTaskRecord[] = Array.isArray(tasks) ? tasks : [];
  if (filter === "all") {
    return list;
  }
  if (filter === "completed") {
...
function filterGroups(groups: TuiTaskGroupRecord[] | unknown, filter: string): TuiTaskGroupRecord[] {
  const list: TuiTaskGroupRecord[] = Array.isArray(groups) ? groups : [];
  if (filter === "all") {
    return list;
  }
  if (filter === "completed") {
...
function sortTasksForPanel(tasks: TuiTaskRecord[]) {
...
function taskStatusPriority(task: TuiTaskRecord) {
...
function isIssueTask(task: TuiTaskRecord) {
  const status = normalizeTaskStatus(task?.status ?? "");
...
function taskStatusMark(status: string) {
  if (status === "completed") {
    return "[✓]";
  }
  if (status === "partial") {
...
function visibleBodyLines(value: unknown) {
  return String(value ?? "").split(/\r?\n/).filter((item) => item.length > 0);
}

function metaSummary(entry: TuiEntry) {
...
function renderSegments(segments: TuiComposerSegment[], theme: TuiTheme, promptLineColor: string | undefined) {
...
function themedSideColor(theme: TuiTheme, view: string | undefined) {
  if (view === "tasks") {
...
function themedPanelKindColor(theme: TuiTheme, kind: string) {
  if (kind === "logs") {
...
function themedPromptColor(theme: TuiTheme, colorToken: string | undefined) {
...
function themedKindColor(theme: TuiTheme, kind: string) {
  if (kind === "error") {
    return themeColor(theme, "danger", "red");
  }
  if (kind === "approval") {
...
function themedLineColor(theme: TuiTheme, color: string | undefined) {
  if (color === "cyan") {
    return themeColor(theme, "identity", "cyan");
  }
  if (color === "green") {
...
function EscapeAction({ theme = DEFAULT_TUI_THEME, label = "Esc 关闭" }: TuiViewProps) {
  return h(Text, {
...
function commandPanelEscLabel(footer: unknown) {
  return /Esc\s*取消/i.test(String(footer ?? "")) ? "Esc 取消" : "Esc 关闭";
}

function commandPanelFooterText(footer: unknown) {
...
function providerStatusLines(context: ContextWindowSummary) {
  ...
  if (Number.isFinite(context.providerUsageReports) && (context.providerUsageReports ?? 0) > 1) {
...
function formatProviderUsageBrief(context: ContextWindowSummary) {
  const parts = [
...
function formatCachedPromptUsage(cachedTokens: number | null | undefined, promptTokens: number | null | undefined) {
  ...
  if (!Number.isFinite(promptTokens) || (promptTokens ?? 0) <= 0) {
    return `${cached} tokens`;
  }
  const percent = Math.round((Number(cachedTokens) / Number(promptTokens)) * 100);
  return `${cached}/${formatTokenCount(promptTokens)} tokens (${percent}%)`;
}

function primaryTokenLimit(context: ContextWindowSummary) {
