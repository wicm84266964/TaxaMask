...
TRANSLATIONS = {
    "zh": {
        ...
        "VLM Pre-Label": "VLM 预标注",
        "VLM Batch Pre-Label": "VLM 批量预标注",
    ...
    "External backend needs at least a train command or a predict command.": "父部位拓展至少需要填写训练命令或推理命令。",
        ...
        "Locator checkpoint could not be loaded: {0}. Prediction and training were stopped.": "定位器检查点无法加载：{0}。预测和训练已停止。",
        ...
        "Model selection locked": "模型选择已锁定",
        "Locator and Segmenter selection is locked while training or training input verification is running.": "训练或训练输入校验运行期间，定位器和分割器选择已锁定。",
        "Model settings are locked while training or training input verification is running.": "训练或训练输入校验运行期间，模型设置已锁定。",
        "Browse": "浏览",
        ...
        "Panel splitting is already running.": "批量拼图切分正在运行。",
        "{0} is still running. Wait for it to finish before starting panel splitting.": "{0}仍在运行，请等待任务完成后再开始批量拼图切分。",
        "All original images have already been checked for panel splitting. Clear the split status on specific images before running them again.": "所有原图都已经完成拼图切分检查。如需重新处理，请先在图片列表中清除对应图片的切分状态。",
        "Preparing panel splitting...": "正在准备批量拼图切分...",
        "Processing panel images: {0}/{1}\n{2}": "正在切分拼图：{0}/{1}\n{2}",
        "Stopping after the current image...": "正在停止；当前图片完成后不会继续处理...",
        ...
        "Panel splitting finished: {0} crop(s) from {1} image(s); hard-joined candidate plates needing review: {2}; no split detected: {3}; retryable failures: {4}.": "拼图切分完成：从 {1} 张图片生成 {0} 张裁剪图；硬拼接候选需复核 {2} 张；未检测到切分 {3} 张；可重试失败 {4} 张。",
        "Failed files remain eligible for the next run: {0}": "失败图片不会标为已完成，下次运行仍会继续处理：{0}",
        "Panel splitting stopped after {0}/{1} image(s).": "已在完成 {0}/{1} 张图片后停止；已完成结果均已保留。",
        "Panel splitting stopped because project results could not be saved: {0}": "由于项目结果无法保存，批量拼图切分已停止：{0}",
        ...
        "Note": "备注",
        "Delete": "删除",
        "VLM preannotation could not start: {0}": "VLM 预标注无法启动：{0}",
        "Could not find the selected image file on disk.": "找不到当前选中的图片文件。",
        "VLM preannotation stopped before any draft boxes were saved.": "VLM 预标注在保存任何草稿框之前就停止了。",
        "VLM preannotation did not save any draft boxes.\n\n{0}": "VLM 预标注没有保存任何草稿框。\n\n{0}",
        "VLM preannotation finished without draft boxes. The model returned no usable boxes, or existing labels were kept. Details are in the log panel.": "VLM 预标注结束了，但没有留下草稿框。模型没有返回可用框，或原有标注被保留。细节请看日志面板。",
        "No Locators Found": "未找到定位器",
        "Untrained locator (no matching weights)": "未训练定位器（没有匹配的权重）",
        "Skipped locator {0}: it was trained for {1} part(s), but this project locator scope is {2} ({3} parts). Selected matching locator {4} instead.": "已跳过定位器 {0}：它是按 {1} 个部位训练的，而当前项目定位器范围是 {2}（{3} 个部位）。已改为选择匹配的定位器 {4}。",
        "Skipped locator {0}: it was trained for {1} part(s), but this project locator scope is {2} ({3} parts). No matching locator was found, so this project starts with an untrained locator. Train a matching model, or manually select a locator trained for these parts.": "已跳过定位器 {0}：它是按 {1} 个部位训练的，而当前项目定位器范围是 {2}（{3} 个部位）。没有找到匹配的定位器，因此新项目先使用未训练的定位器。请训练匹配的模型，或手动选择按这些部位训练的定位器。",
        "This locator was trained for {0} part(s), but the current project locator scope has {1} part(s) ({2}). Choose a matching locator or train a new one.": "这个定位器是按 {0} 个部位训练的，但当前项目定位器范围有 {1} 个部位（{2}）。请选择匹配的定位器，或重新训练一个。",
        "This locator's trained part order does not match the current project locator scope ({0}). Choose a matching locator or train a new one.": "这个定位器训练时的部位顺序与当前项目定位器范围（{0}）不一致。请选择匹配的定位器，或重新训练一个。",
        ...
        "Starting model verification": "起始模型复验",
        ...
        "Recovered training inputs verified. Retrying with the previous configuration.": "训练文件已恢复并通过复验，正在沿用原配置重新开始训练。",
        "Cancelling training input verification...": "正在安全取消训练文件复验...",
        "Training did not start because the registered files or data version could not be verified: {0}": "训练未启动，因为登记文件或数据版本未能通过复验：{0}",
        "Stop Training": "停止训练",
        ...
        "The selected locator checkpoint has no verifiable ordered locator scope or training resolution. Continuing assumes its output channels follow the current project order ({0}) and uses 512px inference. Continue only after verifying both assumptions.": "当前选中的定位器检查点没有可验证的有序部位范围和训练分辨率。继续操作将假定其输出通道遵循当前项目的部位顺序（{0}），并按 512px 执行推理。请仅在核实这两项假设后继续。",
        "The selected locator checkpoint has no verifiable ordered locator scope. Continuing assumes its output channels follow the current project order ({0}). Continue only after verifying this assumption.": "当前选中的定位器检查点没有可验证的有序部位范围。继续操作将假定其输出通道遵循当前项目的部位顺序（{0}）。请仅在核实该假设后继续。",
        "The selected locator checkpoint has no verifiable training resolution. Continuing uses 512px inference. Continue only after verifying this assumption.": "当前选中的定位器检查点没有可验证的训练分辨率。继续操作将按 512px 执行推理。请仅在核实该假设后继续。",
