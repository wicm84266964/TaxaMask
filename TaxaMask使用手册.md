...
## 当前 main / v2.x 先读：TIF/CT 与局部轴重切片
...
- 记录人工 ROI、part mask、local frame、输出轴和参照点，为未来 TIF/CT 模型训练准备素材。
...
10. [TIF 体数据工作台：切片、部位体与局部轴重切片](#10-tif-体数据工作台切片部位体与局部轴重切片)
...
>
> 当前 TIF 路线已经合入 `main`，灵感来自 AntScan 一类体数据集需求。它已经具备项目结构、导入、切片查看、材料表、part ROI、part volume、3D 体预览、局部轴重切片和导出/后端契约，但稳定性和跨数据验证程度仍低于 2D/STL 主标注路线。
...
普通 TIF stack 导入或 materialize 后不会自动拥有 `manual_truth`，也不会自动标记为可训练。`working_edit` 是按需创建的：当你真正开始 brush/rectangle/lasso 编辑、或从 `manual_truth` / `model_draft` 复制到当前编辑层时，程序才会创建 `labels/working_edit.ome.zarr/`。这样可以避免一批大体数据刚导入时就额外写出大量空标签体。
...
显示模式切到 `三维体预览` 后，可以用 3D 视图检查整体形态、part volume 和内部结构。它适合在真正切片编辑前寻找脑部、腺体或其他内部区域的大致空间位置，也适合在 Local Axis Reslice 时观察 source Z-axis、editable output Z-axis 和 roll reference points 的空间关系。
...
当前 TIF/CT 自动化方向先以“把人工操作记录成可训练素材”为主，不直接内置一个成熟的自动脑部定位模型。
...
这些材料未来可用于训练局部结构定位、部位提取和 local frame 预测模型。当前阶段仍以人工确认结果为可信数据来源。

### 10.10 Brush 编辑与提升为 manual_truth
...
它负责的是 `prepare_dataset / train / predict` 这种 material-ID label volume 体分割任务。预测导回时只能进入 `model_draft`，不能直接覆盖 `manual_truth`。
...
### 15.5 标准流程模板 E：当项目搬盘、路径变化或运行中断时，应该怎么稳妥处理
