...
import { listAgentProfiles } from "../agents/profiles.ts";
...
import { formatTaskTree, orchestrateAgentTasks } from "../agents/orchestrator.ts";
import { formatAgentRoute, routeAgentTask } from "../agents/router.ts";
...
import { formatValidationSuggestions, selectValidationSuggestion, suggestValidationCommands } from "../core/validation-suggestions.ts";
import { runDoctor, formatDoctorReport } from "../diagnostics/doctor.ts";
import { formatHooksReport } from "../hooks/report.ts";
import { loadProjectMemoryManifest } from "../memory/loader.ts";
import { appendProjectMemory } from "../memory/store.ts";
import { createMcpRuntime } from "../mcp/runtime.ts";
import { runGatewayHealth, formatGatewayHealthReport } from "../model-gateway/health.ts";
import { formatModelOptions, listConfiguredModels, resolveModelSelection } from "../model-gateway/models.ts";
import { decidePermission, isInside } from "../permissions/policy-engine.ts";
import { listTrustedWorkspaces, revokeWorkspaceTrust } from "../permissions/workspace-trust.ts";
import { formatSkillDetail, formatSkillsList, runSkill } from "../skills/registry.ts";
...
import { addTodo, clearPlan, clearTodos, setPlanSteps, updateTodoStatus, type WorkflowItem } from "../tools/workflow-tools.ts";
import type { AgentSession } from "../core/session.ts";
import type { ProviderUsageAggregate } from "../core/provider-usage.ts";
import type { LabModel } from "../model-gateway/models.ts";
import { createWorkflowState } from "../tools/workflow-tools.ts";
import type { ToolExecutionResult } from "../tools/runtime.ts";
import { listKeybindingGroups, listSlashCommandGroups } from "./registry.ts";
 ...
 *   fullAccess?: boolean;
 *   workflowState?: ReturnType<typeof import("../tools/workflow-tools.ts").createWorkflowState>;
...
type SlashCommandConfig = LabAgentConfig;
...
export type SlashCommandOptions = {
  ...
  approvalCallback?: Parameters<typeof createToolRuntime>[0]["approve"];
  setModelCallback?: (model: LabModel) => void | Promise<void>;
  signal?: AbortSignal;
  trusted?: boolean;
};

export async function runSlashCommand(options: SlashCommandOptions) {
...
function formatCommandList() {
  ...
  for (const group of listSlashCommandGroups()) {
    ...
    lines.push(...group.commands.map((item) => `- /${item.name} - ${item.description}`));
  }
 ...
 */
async function runFilesCommand(options: Parameters<typeof runSlashCommand>[0], config: SlashCommandConfig) {
 ...
 * @param {Record<string, any>} config
 */
async function runDiffCommand(options: Parameters<typeof runSlashCommand>[0], config: SlashCommandConfig) {
 ...
 * @param {Record<string, any>} config
 */
async function runShellSlashCommand(options: Parameters<typeof runSlashCommand>[0], config: SlashCommandConfig) {
 ...
 * @param {Record<string, any>} config
 */
async function runEditCommand(options: Parameters<typeof runSlashCommand>[0], config: SlashCommandConfig) {
  ...
  const editResult = execution.result && typeof execution.result === "object" && !Array.isArray(execution.result)
    ? execution.result as Record<string, unknown>
    : undefined;
  if (execution.ok && editResult?.dryRun && editResult.wouldEdit) {
    return (typeof editResult.diff === "string" ? editResult.diff : "") || `Edit preview for ${editResult.path}`;
  }
  if (!execution.ok || editResult?.edited === false) {
    return formatObject(execution);
  }

  return (typeof editResult?.diff === "string" ? editResult.diff : "") || `Edited ${editResult?.path}`;
}
 ...
 * @param {Record<string, any>} config
 */
async function runReviewCommand(options: Parameters<typeof runSlashCommand>[0], config: SlashCommandConfig) {
 ...
 * @param {Record<string, any>} config
 */
async function runVerifyCommand(options: Parameters<typeof runSlashCommand>[0], config: SlashCommandConfig) {
 ...
 * @param {Record<string, any>} config
 */
async function runReportCommand(options: Parameters<typeof runSlashCommand>[0], config: SlashCommandConfig) {
  ...
  return [
    ...
    `context: ${formatContextSummary(session?.context as Record<string, unknown> | undefined)}`,
    "",
    "Delivery status",
 ...
 * @param {Parameters<typeof runSlashCommand>[0]} options
 */
async function runNextCommand(options: Parameters<typeof runSlashCommand>[0]) {
  const validationSuggestions = await suggestValidationCommands(options.cwd);
  const validationMemory = buildValidationMemory({ workflow: options.workflowState, suggestions: validationSuggestions });
  const repoMap = await buildRepoMap(options.cwd);
  const delivery = buildDeliveryStatus({
 ...
 */
function formatStatusReport(status: Record<string, unknown>) {
  return [
    "Ant Code status",
    "",
    "Session",
    formatKeyValues([
      ...
      ["context", formatContextSummary(status.context && typeof status.context === "object" ? status.context as Record<string, unknown> : undefined)]
    ]),
    "",
    "Repository map",
    formatRepoMap(status.repoMap as Parameters<typeof formatRepoMap>[0], { includeHeader: false }),
    "",
    "Delivery status",
    formatDeliveryStatus(status.delivery as Parameters<typeof formatDeliveryStatus>[0], { includeHeader: false, includeCommands: true, includeSuggestions: false }),
    "",
    "Suggested validation",
    formatValidationSuggestions(status.validationSuggestions as Parameters<typeof formatValidationSuggestions>[0])
  ].join("\n");
}
 ...
 * @param {string} name
 */
function runUsageCommand(options: Parameters<typeof runSlashCommand>[0], config: SlashCommandConfig, name: string) {
  const workflow = options.workflowState;
  const session = options.sessionInfo;
  const usage = session?.usage;
  const usageRecord = usage && typeof usage === "object" ? usage as { reports?: number } : undefined;
  const hasProviderUsage = Number.isFinite(usageRecord?.reports) && (usageRecord?.reports ?? 0) > 0;
  ...
  const validationsPassed = validations.filter((validation) => Boolean((validation as Record<string, unknown>).passed)).length;
  const validationsFailed = validations.filter((validation) => (validation as Record<string, unknown>).passed === false).length;
 ...
 * @param {Record<string, any>} config
 */
async function runModelCommand(options: Parameters<typeof runSlashCommand>[0], config: SlashCommandConfig) {
 ...
 */
function formatModelCommand(config: SlashCommandConfig, options: { note?: string } = {}) {
 ...
 * @param {string} name
 */
function formatDisabledLocalCommand(name: string) {
 ...
 */
function formatKeyValues(entries: Array<[string, unknown]>) {
 ...
 */
async function executeBuiltInTool(options: Parameters<typeof runSlashCommand>[0], config: SlashCommandConfig, toolName: string, input: Record<string, unknown>): Promise<ToolExecutionResult> {
  ...
  return await runtime.execute(toolName, input) as ToolExecutionResult;
}
 ...
 */
function readonlyGitDecision(options: Parameters<typeof runSlashCommand>[0], config: SlashCommandConfig, command: string) {
 ...
 * @param {Parameters<typeof runSlashCommand>[0]} options
 */
function runTodoCommand(options: Parameters<typeof runSlashCommand>[0]) {
  const workflow = options.workflowState;
  if (!workflow) {
 ...
 * @param {Parameters<typeof runSlashCommand>[0]} options
 */
function runPlanCommand(options: Parameters<typeof runSlashCommand>[0]) {
  const workflow = options.workflowState;
  if (!workflow) {
 ...
 */
function formatTodos(todos: WorkflowItem[] | undefined) {
 ...
 */
function formatPlan(plan?: { explanation?: string; steps?: WorkflowItem[] }) {
  const lines = [];
  if (plan?.explanation) {
 ...
 */
function formatRecordedChanges(changes: Array<Record<string, unknown> | WorkflowItem>) {
  ...
  return changes.map((change, index) => {
    ...
    const flags = [
      ...
      record.edited ? "edited" : null,
      record.diffTruncated ? "diff-truncated" : null
    ].filter(Boolean).join(", ") || "changed";
    return `${index + 1}. ${record.path} (${record.toolName}, ${flags}, ${record.diffBytes ?? 0} diff bytes)`;
  }).join("\n");
}
 ...
 */
function formatValidations(validations: Array<Record<string, unknown> | WorkflowItem>) {
  ...
  return validations.map((validation, index) => {
    ...
    const state = record.passed ? "passed" : "failed";
    const timeout = record.timedOut ? ", timed out" : "";
    return `${index + 1}. [${state}] ${record.command} (exit=${record.exitCode ?? "null"}${timeout}, ${record.durationMs ?? "unknown"}ms)`;
  }).join("\n");
}
 ...
 */
function formatContextSummary(context: Record<string, unknown> | undefined | null) {
  if (!context) {
    return "not-attached";
  }
...
function formatProviderUsageCommandLines(usage: ProviderUsageAggregate | undefined) {
  const record = usage ?? {} as ProviderUsageAggregate;
  return [
    ...formatKeyValues([
      ...
      ["last", formatProviderUsageBrief({
        promptTokens: record.lastPromptTokens,
        completionTokens: record.lastCompletionTokens,
        totalTokens: record.lastTotalTokens
      })],
      ["last round", record.lastRound ?? "unknown"],
      ["last model", record.lastModel ?? "unknown"]
    ]).split("\n"),
    record.last ? `last raw usage: ${truncatePlain(JSON.stringify(record.last), 220)}` : null
  ].filter(Boolean).join("\n");
}

function formatProviderUsageBrief(usage: { promptTokens?: unknown; completionTokens?: unknown; totalTokens?: unknown } = {}) {
  const parts = [
...
function truncatePlain(value: unknown, max: number = 80) {
 ...
 */
function inferPermissionMode(options: { readonly?: boolean; allowWrite?: boolean; allowCommand?: boolean; fullAccess?: boolean; sessionInfo?: AgentSession }) {
  const session = options.sessionInfo;
 ...
 * @param {Parameters<typeof runSlashCommand>[0]} options
 */
function resolvePermissionState(options: Parameters<typeof runSlashCommand>[0]) {
  const session = options.sessionInfo;
...
function normalizePermissionModeValue(value: unknown) {
 ...
 * @param {string[]} parts
 */
function splitPlanSteps(parts: string[]) {
  return parts.join(" ")
    .split("|")
    .map((step) => step.trim())
    .filter(Boolean);
 ...
 * @param {Record<string, any>} config
 */
function createPolicy(options: Parameters<typeof runSlashCommand>[0], config: SlashCommandConfig) {
  const permission = resolvePermissionState(options);
  return {
    workspace: options.cwd,
    permissionMode: permission.permissionMode,
    networkMode: String(config.networkMode ?? ""),
    allowedHosts: Array.isArray(config.allowedHosts) ? config.allowedHosts.map(String) : [],
    readonly: permission.readonly,
    fullAccess: permission.fullAccess,
    approvals: {
...
function createPermissionOptions(options: SlashCommandOptions) {
 ...
 */
function formatFileEntry(entry: Record<string, string>) {
 ...
 * @param {string} cwd
 */
function parseDiffArgs(args: string[], cwd: string): { ok: false; message: string } | { ok: true; stat: boolean; pathspecs: string[] } {
 ...
 */
function parseRunArgs(command: { name: string; args: string[]; raw?: string }) {
 ...
 */
function formatShellExecution(execution: ToolExecutionResult) {
  if (!execution.ok) {
    return formatObject(execution);
  }

  const result = execution.result;
  if (!result || typeof result !== "object" || Array.isArray(result) || !("exitCode" in result)) {
    return formatObject(execution);
  }
  return formatProcessResult(result as ProcessResult, "Command completed without output.");
}
 ...
 */
function formatProcessResult(result: { exitCode: number | null; signal?: string | null; timedOut?: boolean; durationMs?: number; stdout?: string; stderr?: string; stdoutTruncated?: boolean; stderrTruncated?: boolean; error?: Record<string, unknown> }, emptyMessage: string) {
  const lines = [
 ...
 */
function runProcess(executable: string, args: string[], cwd: string, env: NodeJS.ProcessEnv | undefined, signal: AbortSignal | undefined = undefined): Promise<ProcessResult> {
  ...
  return new Promise((resolve) => {
    ...
    function requestProcessTermination(target: ChildProcess) {
...
function interruptedProcessResult(startedAt: number, message: string): ProcessResult {
  return {
    exitCode: null,
    signal: null,
    timedOut: false,
...
function interruptedProcessFields(message: string) {
  return {
    interrupted: true,
 ...
 * @param {Record<string, any>} config
 */
async function runMcpCommand(options: Parameters<typeof runSlashCommand>[0], config: SlashCommandConfig) {
  const runtime = createMcpRuntime({
    cwd: options.cwd,
    config,
    policy: createPolicy(options, config),
    approve: options.approvalCallback as NonNullable<Parameters<typeof createMcpRuntime>[0]>["approve"]
  });
...
async function runMcpDoctor(config: SlashCommandConfig, runtime: ReturnType<typeof createMcpRuntime>, options: { live?: boolean } = {}) {
  const rawServers = Array.isArray(config.mcp?.servers) ? config.mcp.servers : [];
  const visible = runtime.listServers();
  type McpDoctorCheck = { name: string; status: string; command: string; message: string; tools: string[] };
  const checks: McpDoctorCheck[] = [];
  for (const server of visible) {
    const raw = rawServers.find((item) => item.name === server.name) ?? server;
    const enabled = raw.enabled !== false && !("disabled" in raw && raw.disabled === true);
    if (!enabled) {
      checks.push({
        name: server.name,
        status: "disabled",
        command: String(raw.command ?? ""),
        message: String(raw.description ?? "MCP server is disabled"),
        tools: []
      });
      continue;
    }
    const commandCheck = await checkCommandAvailable(String(raw.command ?? ""));
    if (!commandCheck.ok) {
      checks.push({
        name: server.name,
        status: "missing",
    ...
    const listedTools = "tools" in tools && Array.isArray(tools.tools) ? tools.tools : [];
    const listedError = "error" in tools ? tools.error : undefined;
    const listedErrorMessage = listedError && typeof listedError === "object" && "message" in listedError
      && typeof listedError.message === "string"
      ? listedError.message
    ...
    checks.push({
      ...
      command: String(raw.command ?? ""),
      message: tools.ok ? `${listedTools.length} tool(s) discovered` : listedErrorMessage,
      tools: tools.ok
        ? listedTools
            .map((tool) => (tool && typeof tool === "object" && "name" in tool && typeof tool.name === "string" ? tool.name : ""))
            .slice(0, options.live ? 80 : 12)
        : []
    });
  }
  return {
...
function formatMcpDoctor(report: { servers: Array<{ status: string; name: string; message: string; tools: string[] }>; note?: string }) {
  return [
    "Ant Code MCP doctor",
    "",
...
async function checkCommandAvailable(command: string) {
 ...
 * @param {Parameters<typeof runSlashCommand>[0]} options
 */
async function runMemoryCommand(options: Parameters<typeof runSlashCommand>[0], config: SlashCommandConfig) {
  const [subcommand, ...rest] = options.command.args;
  if (subcommand === "add") {
    const input = { text: rest.join(" ") };
    const definition = {
      name: "add_project_memory",
      risk: "memory" as const,
    ...
    const decision = decidePermission(
      {
        ...
        risk: definition.risk as "read" | "write" | "execute" | "network" | "browser" | "document" | "mcp" | "memory",
        cwd: options.cwd,
 ...
 * @param {Record<string, any>} config
 */
async function runPermissionsCommand(options: Parameters<typeof runSlashCommand>[0], config: SlashCommandConfig) {
 ...
 * @param {Parameters<typeof runSlashCommand>[0]} options
 */
async function runAgentsCommand(options: Parameters<typeof runSlashCommand>[0], config: SlashCommandConfig) {
  ...
  if ((subcommand === "continue" || subcommand === "续跑") && profileName) {
    ...
    const result = await runSubagent({
      ...
      parentSessionId: options.sessionInfo?.id ?? (typeof task.parentSessionId === "string" ? task.parentSessionId : undefined),
      hooksTrusted: options.trusted === true,
      parentTaskId: task.parentTaskId ?? task.id,
      profileName: task.profile,
      title: `${task.title || task.profile} - 续跑`,
      routeDecision: task.routeDecision && typeof task.routeDecision === "object" ? task.routeDecision as Record<string, unknown> : undefined,
      contextPack: task.contextPack && typeof task.contextPack === "object" ? task.contextPack as Record<string, unknown> : undefined,
  ...
  if (subcommand === "run" && profileName) {
    ...
    const result = await runSubagent({
      ...
      workflowState: options.workflowState,
      approvalCallback: options.approvalCallback,
      parentSessionId: options.sessionInfo?.id,
      hooksTrusted: options.trusted === true,
      profileName,
...
function parseAgentRunQuery(parts: string[]) {
  const raw = parts.join(" ").trim();
  const writeScope: string[] = [];
  const acceptance: string[] = [];
  const cleaned = raw.replace(/--write-scope\s+([^\s]+)/g, (_match: string, value: string) => {
    writeScope.push(value);
    return "";
  }).replace(/--accept\s+("[^"]+"|'[^']+'|[^\s]+)/g, (_match: string, value: string) => {
    acceptance.push(value.replace(/^['"]|['"]$/g, ""));
    return "";
  }).trim();
  return {
    query: cleaned,
...
function formatAgentTasks(tasks: unknown) {
...
function formatAgentGroups(groups: unknown) {
...
function formatAgentRecordStatus(status: string) {
 ...
 * @param {Parameters<typeof runSlashCommand>[0]} options
 */
async function runBackgroundCommand(options: Parameters<typeof runSlashCommand>[0]) {
  ...
  if (subcommand === "worktree" && taskId) {
    ...
    if (decision.decision === "ask" && options.approvalCallback) {
      const approved = await options.approvalCallback({
        ...
        input: { taskId },
        decision,
        definition: { name: "background_worktree", risk: "execute" as const, description: "Create a local git worktree for a background task", inputSchema: {} }
      });
      if (!approved) {
        return formatObject({ ok: false, blocked: true, decision });
      }
  ...
  if (subcommand === "cleanup-worktree" && taskId) {
    ...
    if (decision.decision === "ask" && options.approvalCallback) {
      const approved = await options.approvalCallback({
        ...
        input: { taskId },
        decision,
        definition: { name: "background_cleanup_worktree", risk: "execute" as const, description: "Remove a local git worktree for a background task", inputSchema: {} }
      });
      if (!approved) {
        return formatObject({ ok: false, blocked: true, decision });
      }
 ...
 * @param {Record<string, any>} config
 */
async function runSkillsCommand(options: Parameters<typeof runSlashCommand>[0], config: SlashCommandConfig) {
  ...
  if (subcommand === "run" && name) {
    ...
    return [
      ...
      "",
      `模式：${"execution" in result ? result.execution : ""}`,
      `说明：${"note" in result ? result.note : ""}`,
      "userMessage" in result && result.userMessage ? `任务：${result.userMessage}` : null,
      "",
...
async function runSkillsDoctor(options: SlashCommandOptions, config: SlashCommandConfig) {
  const capabilities = await listCapabilities({
    cwd: options.cwd,
    config,
    env: options.env
...
function formatSkillsDoctor(report: { skills: number; checks: Array<{ status: string; name: string; message: string }> }) {
  return [
 ...
 * @param {Record<string, any>} config
 */
async function runSessionsCommand(options: Parameters<typeof runSlashCommand>[0], config: SlashCommandConfig) {
 ...
 * @param {Record<string, any>} config
 */
function redactConfig(config: SlashCommandConfig) {
  return JSON.parse(JSON.stringify(config, (key: string, value: unknown) => {
