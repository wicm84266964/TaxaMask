...
import { RECOMMENDED_MCP_SERVERS } from "../mcp/recommended.ts";
...
function mcpServersFromConfig(config: Record<string, unknown>): McpServerView[] {
  const mcp = asRecord(config.mcp);
  return Array.isArray(mcp.servers) ? mcp.servers as McpServerView[] : [];
}

export async function listCapabilities(options: { cwd: string; config: Record<string, unknown>; env?: NodeJS.ProcessEnv }) {
  const skills = await loadSkills(options);
  const configuredMcp = mcpServersFromConfig(options.config);
  ...
  return {
    builtInTools: BUILT_IN_TOOLS.map((tool: CapabilityTool) => ({
      name: tool.name,
      risk: tool.risk,
 ...
 */
export function formatCapabilities(capabilities: Awaited<ReturnType<typeof listCapabilities>>) {
  const lines = [
    "Ant Code 能力清单",
    "",
    "内置工具",
...
function formatMcp(servers: Awaited<ReturnType<typeof listCapabilities>>["mcpServers"]) {
  if (servers.length === 0) {
...
function formatSkills(skills: Awaited<ReturnType<typeof listCapabilities>>["skills"]) {
  if (skills.length === 0) {
