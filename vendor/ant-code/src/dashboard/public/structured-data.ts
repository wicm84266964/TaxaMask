...
type StructuredDataSuccess = {
  ok: true;
  summary: unknown;
  html: unknown;
  expandTreeNode?: (id: string) => string;
  tsv?: string;
};
...
type StructuredDataResult = StructuredDataSuccess | StructuredDataFailure;

type DeferredTreeEntry = {
  value: unknown;
  depth: number;
...
export function renderStructuredData(kind: string, source: string, vendor: unknown = {}): StructuredDataResult {
...
function renderJson(source: string) {
  try {
...
function renderYaml(source: string, vendor: unknown) {
  try {
    const parseYaml = (vendor as { parseYaml?: unknown }).parseYaml;
...
function renderDelimited(source: string, delimiter: string, label: string) {
...
function createTreeRenderer(root: unknown) {
  const deferred = new Map<string, DeferredTreeEntry>();
  let nextDeferredId = 1;
  let renderedNodes = 0;

  function renderValue(value: unknown, depth: number, eagerUntil: number, ancestors: Set<object> = new Set()): string {
  ...
  function renderNode(value: unknown, depth: number, eagerUntil: number, ancestors: Set<object>, open: boolean): string {
...
function parseDelimited(source: string, delimiter: string): DelimitedParseResult {
...
function boundedTsv(rows: string[][]) {
  const lines: string[] = [];
  let bytes = 0;
  let truncated = false;
...
function truncateCell(value: unknown) {
  const text = String(value ?? "");
  return text.length > MAX_CELL_CHARS ? `${text.slice(0, MAX_CELL_CHARS)}...` : text;
}

function success(summary: unknown, html: unknown, extra: { expandTreeNode?: (id: string) => string; tsv?: string } = {}): StructuredDataSuccess {
  return { ok: true, summary, html, ...extra };
}

function failure(error: unknown): StructuredDataFailure {
