...
`multimodal_configs/` 里的 profile 负责 figure 阶段：决定怎样提取图版、怎样组装 caption 和附近文本材料，以及启用多模态验证时怎样判断候选图是否合格。
...
`extraction_rules.core_section_hints`：核心章节提示。

通常放 diagnosis、description、type material、material examined 等。

`extraction_rules.extended_section_hints`：扩展章节提示。
...
9. 检查 accepted、needs_review、rejected 结果和模型理由。
10. 稳定后再批量处理。

## 6. 结果可信度边界

如果没有启用真实多模态验证，或者模型配置不可用，程序会走默认复核路径。这类结果应进入 Review，而不是直接作为可信 accepted。

即使启用了真实多模态验证，新类群 profile 也需要小批量人工验收。profile 提供的是适配接口，不代表其他类群已经被同等验证。
