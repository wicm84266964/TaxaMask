...
> 适用范围：新版 PDF Processing / V2 screening
>
> 目标：让用户可以把默认的蚂蚁新种筛选逻辑，改成其他生物类群的分类学文献筛选逻辑。

---

## 当前结论

新版 PDF 文献处理已经保留了可改筛选逻辑的接口。真正控制筛选目标的地方是：
...
```text
PDF Processing -> Select Logic Profile -> Advanced Logic Settings
```
...
它在做什么：标记这个 profile 属于新版 V2 筛选流程。

为什么重要：当前用户界面只开放 V2 筛选逻辑。旧的规则预筛回退入口已经移除，避免把用户带回蚂蚁历史逻辑。

建议：新建跨物种筛选 profile 时使用：
...
为什么重要：这是跨物种适配中最该改的字段。蚂蚁 profile 里是 Formicidae、ant、ants；如果筛植物或甲虫，必须换成对应类群。
