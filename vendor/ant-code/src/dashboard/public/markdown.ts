type MarkdownBlock = {
  ...
  level?: number;
  ordered?: boolean;
  items?: Array<{ task: boolean; checked: boolean; text: string }>;
  language?: string;
  headers?: string[];
  alignments?: string[];
  rows?: string[][];
...
type MarkdownListBlock = MarkdownBlock & {
  type: "list";
  ordered: boolean;
  items: Array<{ task: boolean; checked: boolean; text: string }>;
};

type MarkdownCodeLike = MarkdownBlock & { text: string[] };
...
export function renderMarkdown(value: unknown, options: Record<string, unknown> = {}): string {
...
export function parseMarkdownBlocks(value: unknown): MarkdownBlock[] {
  ...
  let list: MarkdownListBlock | null = null;
  let code: MarkdownCodeLike | null = null;
  let math: MarkdownCodeLike | null = null;
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (code) {
      if (fence) {
        ...
        (code.text as string[]).push(line);
      }
      continue;
    }
    if (mathFence) {
      flushOpenText();
      math = { type: "math", display: true, text: [] as string[] };
      continue;
    }
    if (fence) {
      flushOpenText();
      code = { type: "code", language: normalizeCodeLanguage(fence[1]), text: [] as string[] };
      continue;
    }
    if (/^\s*$/.test(line)) {
      flushOpenText();
    ...
    if (listItem) {
      ...
      if (!list || list.ordered) {
        flushList();
        list = { type: "list", ordered: false, items: [] as Array<{ task: boolean; checked: boolean; text: string }> };
      }
    ...
    if (orderedItem) {
      ...
      if (!list || !list.ordered) {
        flushList();
        list = { type: "list", ordered: true, items: [] as Array<{ task: boolean; checked: boolean; text: string }> };
      }
...
function createRenderContext(blocks: MarkdownBlock[], source: unknown, options: Record<string, unknown> = {}) {
...
function renderBlock(block: MarkdownBlock, context: Record<string, unknown>): string {
  if (block.type === "heading") {
...
function textLines(text: string | string[] | undefined): string[] {
  if (Array.isArray(text)) return text;
  return text ? [text] : [];
}

function renderCodeBlock(block: MarkdownBlock, context: Record<string, unknown> = {}) {
...
function renderMathBlock(block: MarkdownBlock) {
  const text = textLines(block.text).join("\n").trim();
...
function renderMermaidBlock(text: string) {
...
function renderDataBlock(language: unknown, text: string) {
...
function renderPlainDraftBlock(text: string, label: string) {
...
function renderDiffCode(text: string) {
  return text.split("\n").map((line) => {
...
function renderListItem(item: { task: boolean; checked: boolean; text: string }, context: Record<string, unknown>) {
  if (item.task) {
...
function renderTable(block: MarkdownBlock, context: Record<string, unknown>) {
...
function tableBlockToMarkdown(block: MarkdownBlock) {
...
function alignmentAttribute(value: unknown) {
...
function readBlockquote(lines: string[], startIndex: number) {
...
function parseListItem(value: unknown) {
...
function parseTableRow(line: unknown) {
...
function parseTableSeparator(line: unknown) {
...
function renderInline(value: unknown, context: Record<string, unknown>) {
...
function renderInlineMath(value: unknown, context: Record<string, unknown>) {
  if (context.lightweight) {
...
function renderInlineMarkdown(value: unknown, context: Record<string, unknown>) {
...
function renderLink(label: string, href: unknown, context: Record<string, unknown>) {
...
function renderImage(alt: unknown, src: unknown, context: Record<string, unknown>) {
...
function safeUrl(value: unknown, options: Record<string, unknown> = {}) {
  const text = String(value ?? "").trim();
...
function safeDataImageUrl(value: unknown) {
...
function matchesBitmapSignature(kind: string, bytes: number[]) {
  if (kind === "png") {
...
function startsWithBytes(value: number[], expected: number[]) {
  return expected.every((byte, index) => value[index] === byte);
}

function remoteImageHost(value: string) {
  try {
...
function isLikelyLocalFileUrl(value: string) {
...
function resolveRelativeUrl(url: string, basePath: unknown = "") {
...
function isLocalWorkspaceUrl(value: unknown) {
  const text = String(value ?? "").trim();
...
function isWorkspaceRelativeToBase(value: unknown, basePath: unknown = "") {
