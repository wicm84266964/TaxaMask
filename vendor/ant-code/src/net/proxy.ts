...
type WindowsProxySelection = {
  http?: string | null;
  https?: string | null;
};

let cachedWindowsProxy: WindowsProxySelection | null | undefined;

/**
 * @param {string} targetUrl
 * @param {{ env?: NodeJS.ProcessEnv; config?: Record<string, any> }} [options]
 */
export function resolveProxyForUrl(targetUrl: string, options: { env?: NodeJS.ProcessEnv; config?: Record<string, unknown> } = {}) {
  let target;
  try {
  ...
  const configProxy = normalizeProxyUrl(asRecord(config.web).proxyUrl ?? config.proxyUrl);
  if (configProxy) {
    return configProxy;
  }
 ...
 */
export function withProxyEnvironment(env: NodeJS.ProcessEnv = process.env, options: { config?: Record<string, unknown> } = {}) {
  const next = { ...env };
 ...
 * @param {NodeJS.ProcessEnv} env
 */
function proxyFromEnv(protocol: string, env: NodeJS.ProcessEnv) {
...
function selectProxyForProtocol(protocol: string, proxy: WindowsProxySelection | null | undefined) {
  if (!proxy) {
    return null;
  }
 ...
 * @param {unknown} value
 */
function matchesNoProxy(hostname: string, value: unknown) {
  const text = String(value ?? "").trim();
  if (!text) {
    return false;
  }
