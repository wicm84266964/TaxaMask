...
import { runInteractiveChat } from "./interactive.ts";
import { createPrintEventCollector } from "./print-output.ts";
import { runTui } from "./tui.ts";
import { parseSlashCommand } from "../commands/parser.ts";
import { runSlashCommand } from "../commands/runtime.ts";
import { runPrintTurn } from "../core/session.ts";
import { startDashboard } from "../dashboard/server.ts";
import { ANT_EVENT_SCHEMA_VERSION, createAntJsonOutput, redactText } from "../core/events.ts";
...
import { helpText } from "../ui/output.ts";
...
async function main() {
  ...
  if (args.print) {
    ...
    const collector = createPrintEventCollector({
      format: args.outputFormat === "json" || args.outputFormat === "stream-json" ? args.outputFormat : "text",
 ...
 */
function writeTurnPrintOutput(input: { args: ReturnType<typeof parseArgs>; collector: ReturnType<typeof createPrintEventCollector>; result: Awaited<ReturnType<typeof runPrintTurn>> }) {
 ...
 */
function writeCommandPrintOutput(input: { args: ReturnType<typeof parseArgs>; slashCommand: ReturnType<typeof parseSlashCommand>; text: string }) {
  if (input.args.outputFormat === "text") {
