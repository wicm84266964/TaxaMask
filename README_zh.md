...
减少重复工作还体现在另一件事上：我彻底摒弃了旧时代专业软件那种让人看到就想吐的超长使用说明。TaxaMask 直接内嵌了一个类似 Codex、OpenCode 这类工具的 Agent Center。你对 TaxaMask 有任何不懂的问题，尽管去问他就好，他手上有整个 TaxaMask 的源码，你想让他怎么改就怎么改。你可以根据自己研究的类群改配置，修改想要使用的预测模型，甚至把 TaxaMask 在你的本地仓库改成少女粉色都没问题。如果你发现改着改着 TaxaMask 无法启动了，也无所谓，Ant-Code 还有一个兜底 Dashboard，可以在这里管理历史会话，继续让智能体帮你排查和修复；它不受 TaxaMask 主窗口是否能启动的影响。

至于 micro-CT，也就是页面里的 TIF 入口，这条路线的灵感来自我看到 AntScan 这类很棒的数据集发布。那时候我在想，这种超高精度的 micro-CT 数据，不管是体表结构还是体内结构，都是极好的研究素材。我既然已经做了体表结构的自动标注系统，为什么不试试把体内结构也做了呢？于是便有了 TIF 这条与三维体渲染相关的路径：用来查看体数据、定位部位、绘制更精细的体数据 mask，导出局部轴重切片，并把复核后的体标注继续送回训练和推理链路。
...
TIF / CT 内部结构
  ...
  -> 部位体与部位 mask 生成
  -> 3D 预览与局部轴重切片导出
  -> 基于标签方案的 part/reslice 标注
  -> 核验 manual_truth 并标记 train-ready
  -> 调用 TIF 后端准备数据、训练和推理
  -> 预测结果以待复核标签层导入并保留 raw_ai_prediction_backup
...
- 在需要处理 TIFF stack 或 CT 派生体数据时，把工作流扩展到内部形态三维检查、部位体提取、局部轴重切片，以及体分割模型的训练和推理。
...
- 记录人工部位提取和局部轴确认过程。
- 维护、导入、导出和绑定标签方案，让多个个体共享同一套数字标签。
- 在 part volume 和 local-axis reslice 上用画笔、橡皮和自动填充辅助进行体标注。
- 将当前编辑标注显式核验为 `manual_truth`；AI 预测默认只是草稿，不会自动变成训练真值。
- 按项目收集 train-ready 的 part/reslice 样本；如果没有部位级真值，可回退使用整只体真值。
- 通过 TIF 后端契约运行 `prepare_dataset`、`train` 和 `predict`。内置预设面向 nnU-Net v2，也可以修改命令接入 MONAI 或其他自定义 3D 分割模型。
- 训练完成后登记到 TIF 模型库，后续推理可选择已有 trained-model manifest。
- 推理结果导入为待复核标签层：part/reslice 预测进入 `editable_ai_result`，整只体预测进入待复核顶层编辑层，并保留 `raw_ai_prediction_backup`，便于复核、回滚和追溯。
...
docs/contracts/             公开后端契约、TIF 体分割契约和 TIF local-axis 契约
...
8. 为当前 part 或 reslice 绑定标签方案。
9. 用画笔、橡皮或自动填充完成标注，保存后核验为 `manual_truth`。
10. 把已核验样本标记为 train-ready。
11. 通过 nnU-Net v2 预设或自定义 TIF 后端准备训练数据并训练模型。
12. 选择训练好的 model manifest 运行推理，先复核导入的预测标签层，确认后再接受为真值。
...
- [TIF 体分割后端契约](docs/contracts/ant3d_tif_backend_contract_v1.md)
- [TIF local-axis 后端契约](docs/contracts/tif_local_axis_backend_contract_v1.md)

外部后端预测都是待复核候选结果。研究者确认前，不应把它们当作训练真值。

TIF 体分割后端契约负责标签体数据的准备、训练和推理；TIF local-axis 后端契约是另一条路线，负责重切片方向相关的 ROI/frame 建议。
