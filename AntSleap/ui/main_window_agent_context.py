...
class MainWindowAgentContextMixin:
    ...
    def _pdf_agent_prompt(self):
        ...
        return (
            ...
            "0. 文献来源与发现方式：用户已有 PDF，还是需要用 taxonomy-paper-finder 做每日推荐、专题检索、精选文献获取或批量采集。\n"
            "1. key/model 就绪检查。\n"
            "2. 目标类群与 PDF 筛选条件适配。\n"
            "3. figure/caption 数据处理与图文复核条件适配。\n"
            "4. 跑通流程、说明产物位置和复核/导入边界。\n\n"
            "如果用户还没有 PDF，请先停在第 0 阶段确认目标：每日追踪、专题检索、下载已筛选文献，还是批量采集。"
            "发现与推荐默认先看元数据和证据，不自动下载全部候选；下载前必须确认，精选衔接默认只处理 deep reads。"
            "每日推荐通常约 5 篇主要文献、最多 8 篇候选和 3 篇深读，不用弱结果凑数。"
            "只使用开放元数据和合法暴露的 PDF 链接，不使用 Sci-Hub、LibGen、付费墙绕过、验证码绕过或登录态抓取。"
            "第 0 阶段可产生 paper records、screening report、digest、records.csv、doi_list.txt、summary.json、"
            "download_manifest.csv 和 pdfs/；这些是文献发现、判断和来源审计材料，不是训练真值。\n\n"
            "请优先使用需求确认式交互：给我简短选项或短问题让我确认，不要用长段说明替代确认。"
            "现在先停在第 0 阶段：只确认我是否已经有待筛选 PDF 文件夹；如果没有，再确认是每日推荐、专题检索、精选获取还是批量采集；"
