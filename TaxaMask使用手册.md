...
> 适用版本：2026-06-04 Agent Center / Ask Agent 路由 / TIF-STL 分流 / 主工作台父部位与子部位标注 / 2D-STL 模型方案与 Blink 后端切换 / VLM 无网格第一公里预标注 / TIF 离屏 GPU 体预览 / PDF 文献证据与性状描述闭环收尾版
>
...
如果项目还没有任何训练素材，可以先用 **`VLM Pre-Annotate`** 做第一公里预标注。它会让多模态大模型在无网格输入图上提出像素候选框，再用 SAM 生成草稿 polygon。注意：这些结果只是 AI 草稿，必须经过空格确认、当前图像一键通过或人工重画后，才能成为训练材料。
...
5. 程序会生成无网格 VLM 输入图、调用多模态模型、写入橙色 `aibox`，并尽量用 SAM 生成 `Auto-Annotated` 草稿 polygon。若输入图被缩放，程序会把模型给出的输入图像素坐标映射回原始图片坐标。
