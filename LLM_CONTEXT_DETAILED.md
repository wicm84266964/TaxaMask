...
- Large full volumes now follow the same background-first preview rule. For full-volume sources of at least 512 MiB, the initial overview is capped at maximum dimension 512 on OpenGL renderers or 128 on the CPU fallback. ROI/detail preview requests remain separate and may use higher local resolution. Do not restore synchronous full-volume texture construction on the GUI thread, including for integrated-GPU/OpenGL systems.
- Z/Y/X slice extraction, percentile normalization, and label compositing move to `TifSliceRenderWorker` when the source is at least 32 MiB. Repeated navigation cancels the old worker result and coalesces to the latest requested slice. Project/specimen release must wait for both slice and 3D preview workers before closing memory maps.
- A completed import lands on Z-axis slice review even if the workbench was previously in 3D mode. This prevents an automatic full-volume render from immediately following a long import; the researcher can enter 3D explicitly after the first slice is available.
