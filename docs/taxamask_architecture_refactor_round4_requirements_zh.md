...
| 顶层类 | 22 |
| 顶层函数 | 35 |
| 全文件方法 | 592（Stage 0 AST 正式口径；需求研究期文本缩进估算为 594） |
| 全文件 `.connect(...)` | 194 |
| 私有方法 | 413（Stage 0 AST 正式口径；需求研究期估算为 414） |
| `self.<field> = ...` 状态赋值行 | 658（Stage 0 正式正则口径；需求研究期估算为 675） |
...
| 同期累计增删 | 18,711 行 |
| 五个关键测试文件直接引用 main/MainWindow/Dialog 的行 | 324（Stage 0 完整引用模式；需求研究期窄模式估算为 192） |
