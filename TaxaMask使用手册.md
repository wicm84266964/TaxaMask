...
在当前 Windows 测试环境中，可以直接双击仓库根目录的 `启动TaxaMask.bat`。这个脚本直接调用 conda 环境 `antsleap` 里的 Python 启动 `AntSleap/main.py`，避免通过 `conda activate` 生成临时 `.bat` 时偶发的“找不到临时 bat”提示。
...
4. 如果批量范围选择按分组处理，可以在 `VLM Image Group` 里选择 `Split Crops / Original Images / Manual Split Needed`。通常从 PDF 版面图裁剪后，建议选择 `Split Crops`，这样原始整版图可以保留在项目里，但不会被批量送给 VLM。
5. 在 PDF Evidence Tools 里配置 Multimodal LLM API。缺少 API 时，VLM 弹窗里的按钮会直接跳到 API 设置区。
6. 回到标注工作台，点击 `VLM预标注` 只跑当前图，或点击 `VLM批量预标` 按设置的批量范围运行。
7. 程序会生成自适应轻量网格 VLM 输入图、调用多模态模型、写入橙色 `aibox`，并尽量用 SAM 生成 `Auto-Annotated` 草稿 polygon。程序会把模型给出的网格坐标映射回原始图片坐标；像素框解析只作为兼容旧响应的 fallback。
