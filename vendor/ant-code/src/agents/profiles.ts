...
import { normalizeOutputContract } from "./contracts.ts";
...
export const AGENT_PROFILES = Object.freeze([
  {
    ...
    aliases: ["default"],
    hidden: true,
    tools: ["read_file", "list_files", "glob", "grep", "git_status", "git_diff", "web_fetch", "web_search", "document_intake", "write_file", "edit_file", "powershell", "bash", "background_shell", "background_terminal_list", "background_terminal_cancel", "mcp_list", "mcp_call", "skill_list", "skill_read", "skill_run", "agent_run", "todo_read", "todo_write", "plan_update", "ask_user"],
  ...
  },
  {
    name: "verifier",
    mode: "execute",
    tools: ["read_file", "list_files", "glob", "grep", "git_status", "git_diff", "web_fetch", "document_intake", "powershell", "bash", "background_shell", "background_terminal_list", "background_terminal_cancel", "mcp_list", "mcp_call", "skill_list", "skill_read"],
  ...
  },
  {
    ...
    tools: ["read_file", "list_files", "glob", "grep", "git_status", "git_diff", "web_fetch", "document_intake", "write_file", "edit_file", "powershell", "bash", "background_shell", "background_terminal_list", "background_terminal_cancel", "mcp_list", "mcp_call", "skill_list", "skill_read", "skill_run", "todo_read", "todo_write", "plan_update"],
  ...
  },
  {
    ...
    tools: ["read_file", "list_files", "glob", "grep", "web_fetch", "powershell", "bash", "background_shell", "background_terminal_list", "background_terminal_cancel", "mcp_list", "mcp_call", "skill_list", "skill_read", "skill_run"],
    role: "verifier",
    purpose: "browser",
    modelTier: "default",
    canSpawnAgents: false,
  ...
  },
  {
    ...
    tools: ["read_file", "list_files", "glob", "grep", "git_status", "git_diff", "web_fetch", "document_intake", "powershell", "bash", "background_shell", "background_terminal_list", "background_terminal_cancel", "mcp_list", "mcp_call", "skill_list", "skill_read", "skill_run"],
    role: "verifier",
    purpose: "visual",
    modelTier: "vision",
    canSpawnAgents: false,
 ...
 */
export function listAgentProfiles(config?: Record<string, unknown>, options: Record<string, unknown> = {}) {
 ...
 * @param {Record<string, any>} [config]
 */
export function listAgentProfileLabels(config: Record<string, unknown> = {}, options: Record<string, unknown> = {}) {
 ...
 * @param {Record<string, any>} [config]
 */
export function getAgentProfile(name: string, config: Record<string, unknown> = {}, options: { cwd?: string; includeHidden?: boolean } = {}): AgentProfile | null {
 ...
 * @param {Record<string, any>} [config]
 */
function mergeConfiguredProfiles(config: Record<string, unknown> = {}, options: Record<string, unknown> = {}) {
  ...
  const configuredProfiles = Array.isArray(agents.profiles) ? agents.profiles : [];
  for (const custom of configuredProfiles) {
    const source = isRecord(custom) ? custom : {};
    const normalized = normalizeCustomProfile({ ...source, source: source.source ?? "config" });
    if (!normalized) {
      continue;
    }
    upsertProfile(profiles, normalized);
 ...
 * @param {unknown} value
 */
function normalizeCustomProfile(value: unknown): AgentProfile | null {
  if (!isRecord(value)) {
    return null;
  }
  const item = value;
...
function normalizeBudget(value: unknown): Record<string, number | null> | null {
  if (!isRecord(value)) {
    return null;
  }
...
function loadAgentMarkdownProfiles(config: Record<string, unknown>, options: Record<string, unknown> = {}) {
  const cwd = typeof options.cwd === "string" && options.cwd ? options.cwd : inferCwd(config);
  const roots = [
...
function markdownProfile(filePath: string, cwd: string): Record<string, unknown> {
...
function parseMarkdownFrontmatter(raw: unknown): { frontmatter: Record<string, unknown>; body: string } {
...
function parseFrontmatterValue(value: unknown) {
...
function cleanQuoted(value: unknown) {
  return String(value ?? "").replace(/^['"]|['"]$/g, "");
}

function toCamelKey(value: unknown) {
  return String(value).replace(/-([a-z])/g, (_match, char: string) => char.toUpperCase());
}
...
function upsertProfile(profiles: AgentProfile[], normalized: AgentProfile) {
...
function inferCwd(config: Record<string, unknown>) {
  if (typeof config.projectConfigPath === "string" && config.projectConfigPath) {
...
function cloneProfile(profile: AgentProfile | Record<string, unknown>): AgentProfile {
  const tools = Array.isArray(profile.tools)
    ? profile.source === "builtin" ? expandLocalCodingTools(profile.tools) : [...profile.tools.map(String)]
    : [];
  return {
    ...profile,
    ...
    tools,
    disallowedTools: Array.isArray(profile.disallowedTools) ? [...profile.disallowedTools.map(String)] : [],
    skills: Array.isArray(profile.skills) ? [...profile.skills.map(String)] : [],
    mcpServers: Array.isArray(profile.mcpServers) ? [...profile.mcpServers.map(String)] : [],
    triggerHints: Array.isArray(profile.triggerHints) ? [...profile.triggerHints.map(String)] : [],
    outputContract: cloneOutputContract(profile.outputContract)
  };
}

function expandLocalCodingTools(tools: unknown): string[] {
...
function cloneOutputContract(value: unknown) {
  if (!isRecord(value)) {
    return null;
  }
  return {
