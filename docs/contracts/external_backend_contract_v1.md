...
本契约用于把你自己的训练或推理脚本接入 TaxaMask。你可以把这份契约直接交给大模型，并要求它把你的 YOLO、Detectron、Mask R-CNN、SAM2 或其他模型脚本改成符合这个接口。
...
```bash
python your_script.py --contract C:/path/to/contract.json
```
...
```json
{
  ...
  "locator_scope": ["Leaf"],
  "run_dir": "C:/path/external_runs/20260503_103000_train_custom_external_backend",
  "dataset_dir": "C:/path/external_runs/.../dataset",
  "model_dir": "C:/path/external_runs/.../model",
  "image_path": "",
  "prediction_json": "",
  "model_manifest": "C:/path/external_runs/.../model/taxamask_model_manifest.json",
  "log_dir": "C:/path/external_runs/.../logs"
}
```
...
- 外部脚本报错时，TaxaMask 会保留 `logs/*stderr.log` 和 `contract.json`，用于你或大模型继续修改脚本。
