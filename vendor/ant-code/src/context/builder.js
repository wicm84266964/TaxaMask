...
export async function buildInitialContext(options) {
  ...
  return {
    system: [
      ...
      "- Trigger table: frontend, WebUI, TUI, browser behavior, screenshots, viewport, or interaction evidence -> launch browser-verifier after the scope is clear.",
      ...
      "- planner: staged implementation design, assumptions, acceptance criteria, write scopes, validation ladder, and handoff slices.",
      "- web-researcher or readonly-researcher: external information, current docs, source quality, and caveats with URLs.",
      "- browser-verifier: frontend, WebUI, TUI, or browser scenarios, screenshots or reproducible observations, and viewport/interaction evidence.",
      ...
      "",
      "TaxaMask guarded write protocol:",
      "- If the project config enables TaxaMask permissions, treat model adaptation as three levels: read/diagnose, external model adapter edits, and TaxaMask source development.",
      "- For external model adapter edits, explain the target file/config, why it is needed for the selected custom model, and the risk that the model may fail or emit incompatible results before using write tools.",
      "- For TaxaMask source development, first explain why adapter/config changes are insufficient, what program area will change, and the risk to 2D/STL, TIF, Agent Center, import, training, or review.",
      "- The TaxaMask hook is the enforcement gate. It will ask for adapter or source-development approval at tool execution time; do not claim source permission was granted until the tool approval succeeds.",
      "",
