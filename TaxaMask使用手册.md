...
> 适用版本：当前 `main` / v2.x 维护线：Agent Center / PDF 文献证据闭环 / 2D-STL 大项目标注 / SAM 与 VLM 草稿复核 / 父部位与子部位标注 / 模型方案与 Blink 后端切换 / TIF-CT 体预览 / part volume 提取 / Local Axis Reslice / 数据集导出
>
...
## 当前 main / v2.x 先读：TIF/CT、局部轴重切片与标注训练回环

如果你从 GitHub 获取当前维护版，可以按下面方式安装或下载：
...
当前 `main` 在原有 PDF、2D/STL 和 Agent Center 之外，已经合入 TIF/CT 内部结构路线。它当前主要用 AntScan 蚂蚁 CT 数据验证，目标是处理原始切片方向、标本体态和目标结构方向差异很大的体数据。程序结构没有把 TIF 工作流写死为蚂蚁专用，但目前还不能宣称已经完成广泛跨类群验证。

TIF/CT 当前最重要的新能力是：

- 导入 TIFF stack 作为 specimen。
- 在整只体数据中用 ROI 关键切片矩形做粗切定位，例如 head 的大致范围。
- 在整只体数据中用手绘轮廓关键切片做精切 mask；确认 ROI 后同时生成 part volume 和 part mask。
- 在 full volume 或 part volume 中使用 GPU 三维体预览、mask boundary、masked image、Z/Y/X 多方向切片和观察侧截面。
- 在 part volume 下使用 **Local Axis Reslice / 局部轴重切片**。
- 显示锁定的 source Z-axis，并从它复制 editable output Z-axis。
- 在 3D 体预览中移动输出轴端点，确定重切片推进方向。
- 设置 roll reference point pair，例如脑部模板中的左右参照点，用于方向标准化复核。
- 导出重切片后的 `image.tif` 和 `metadata.json`；如果 part 已有 mask，可额外导出 `mask.tif`。
- 在 `标注与训练` 区域维护通用标签方案，把同一套数字标签绑定到多个个体/部位。
- 将复核过的 `editable_ai_result` 或人工编辑结果显式核验为 `manual_truth`，再标记为 train-ready。
- `Prepare dataset / Train` 默认收集项目内所有已核验、train-ready 的 part/reslice 样本；如果项目里没有可训练部位样本，才退回使用 train-ready 的整只体数据。
- 内置真实 nnU-Net v2 后端预设，同时保留可编辑命令，方便 Agent 或用户替换成 MONAI、自定义 3D U-Net 或其他体分割脚本。
- 训练完成后登记到 TIF 模型库，后续推理可以选择已有模型 manifest，不需要每次重新训练。
- 预测结果导回为待复核标签层：part/reslice 预测进入 `editable_ai_result`；整只体预测进入待复核的顶层 `working_edit`，并保留 `raw_ai_prediction_backup` 和 legacy `model_draft` 审计副本。它们都不会自动覆盖 `manual_truth`。

重切片方向严格由 editable output Z-axis 决定。roll reference point pair 只用于方向标准化和复核，不改变“沿这根主轴切片”的基本逻辑。原始 TIFF stack 不会被修改。

TIF/CT 体预览最好使用 NVIDIA 独显。如果机器同时有核显和 NVIDIA 显卡，请在 Windows 图形设置或 NVIDIA 控制面板中，把当前环境的 `python.exe` 设为高性能 GPU。`启动TaxaMask.bat` 会优先寻找 `taxamask` Conda 环境，再兜底旧的 `antsleap` 环境；如果你曾设置过 `TAXAMASK_PYTHON_EXE`，环境改名后要同步指向新的 `python.exe`。打开 TIF 项目后，看体预览状态栏里的 OpenGL renderer，确认是否真的使用 NVIDIA 显卡。
...
10. [TIF 体数据工作台：标注与训练、部位体与局部轴重切片](#10-tif-体数据工作台标注与训练部位体与局部轴重切片)
...
3. **TIF Volume 工作流**
   ...
   - 它有独立的 TIF 项目、材料表、标签方案、slice 拖条、`working_edit`、`manual_truth`、`editable_ai_result` 和 `raw_ai_prediction_backup` 标签层。
   - 它不使用 2D/STL 的 Locator/SAM/Blink 训练路线，而是通过独立 TIF 后端契约连接真实 nnU-Net v2、MONAI 或自定义体分割脚本。
   - 训练和推理不是只看当前选中的切片。`Prepare dataset / Train` 会优先收集项目内所有 train-ready 的 part/reslice `manual_truth` 样本；如果没有可训练的 part/reslice，才退回整只体 train-ready 样本。预测 part/reslice 时使用预测目标表中勾选的目标；在整只体视图中预测时使用当前整只体目标，并使用当前选择的模型 manifest。
...
4. **TIF 体数据标注与训练线**
   ...
   - 绑定标签方案、核验 `manual_truth`、准备训练数据、调用独立体分割后端并复核预测结果
...
| 你手里的数据 | 推荐入口 | 后续动作 |
|---|---|---|
| 已有 PDF 或需要先找开放 PDF 文献 | Agent Center 或 PDF Evidence Tools | 合法采集/整理 PDF、筛选、提取 figure、生成 evidence index |
...
| TIF stack / AMIRA 数据 | TIF Volume | 导入体数据、检查材料表/标签方案、切部位/重切片、标注与核验训练真值、训练或推理体分割模型 |
| 不知道怎么配置或报错 | Agent Center | 让 Agent 带上下文检查项目和设置 |

如果你是从零开始整理一批材料，可以按下面顺序：

0. **如果还没有 PDF，先让 Agent 按目标类群合法采集开放 PDF 文献**
1. **准备 API 设置与筛选逻辑方案**
...
4. TIF 的 `manual_truth`、`working_edit`、`editable_ai_result` 和 `raw_ai_prediction_backup` 不会和 2D polygon 标注混在一起
...
- TIF 工作台里的当前 volume/part 范围、slice 位置、3D 预览模式、GPU/CPU 渲染器和性能诊断摘要
- TIF 标注与训练里的当前标签方案、train-ready part/reslice 样本数、顶层整只样本数、训练样本选择范围、已登记模型数、已选模型 manifest、后端 Python/命令填写状态、后台 run 目录和 result JSON
- PDF 工具里的筛选 profile、图文复核 profile、部位描述 profile、输入目录和数据库位置
- 最近日志摘要
- 诊断路线，例如 2D/STL 外部后端、TIF 后端、主标注工作台、Blink 精修或 PDF evidence
- 建议 Agent 阅读的公开契约、README、源码入口或当前工作台上下文
- 相关源码、契约文档和可能需要检查的运行产物
- 安全边界，例如不要自动把 PDF 候选或 TIF `editable_ai_result` 当成训练真值
...
对自定义模型来说，2D/STL 和 TIF 是两条不同线路。2D/STL 外部后端负责输出可复核的候选框或 polygon；TIF 外部后端负责把体分割预测写回待复核标签层，并保留原始预测备份。part/reslice 预测进入 `editable_ai_result`，整只体预测进入顶层 `working_edit`。Agent 在申请源码开发前，应该先说明为什么仅改外部后端脚本或配置不够。
...
当前新建项目主要使用 SQLite 项目索引，并通过轻量 manifest 入口文件打开；旧项目仍可能是单个 JSON 项目文件。

所以排错时要分清：

- 如果新项目的 manifest、SQLite 数据库或旧项目 JSON 本身损坏，即使图片路径正确，GUI 也**打不开**。
- 如果只是图片路径搬家，项目入口和索引仍然能读，只是图像文件找不到。

这件事和“图片搬盘”是两回事。不要混淆：

- **路径问题** = 项目入口/索引能读，但原图找不到
- **JSONDecodeError** = 旧 JSON 项目文件本身语法损坏
- **SQLite / manifest 错误** = 新项目入口或 SQLite 索引需要先修复或从备份恢复
...
这一页负责的不是正式标注，而是“文献来源、发现与初筛”。

### 5.0 第 0 步：文献/PDF 从哪里来
...
安全边界也要固定：TaxaMask 的这个前置 skill 只使用开放元数据和合法暴露的 PDF 链接，不使用 Sci-Hub、LibGen、付费墙绕过、验证码绕过或登录态抓取。下载到的 PDF 和运行输出是研究材料和本地产物，不应该提交进 Git。
...
如果一次保存了很多裁剪图，加入项目的步骤也会走图片导入进度窗口；裁剪文件已经生成在磁盘上，进度窗口显示的是这些新图正在被登记到当前项目索引中。
...
## 10. TIF 体数据工作台：标注与训练、部位体与局部轴重切片

TIF Volume Workbench 是独立于 2D/STL 的体数据工作流。它适合连续切片、AMIRA/Avizo 标注数据、micro-CT、confocal 或其他能表示为 stack 的体数据。当前 `main` 已经加入 AntScan / Local Axis Reslice 路线，用于先提取目标 part volume，再对该部位体进行局部坐标系重切片。

最重要的一点是：

> TIF 的训练单位不是单张 polygon 图片，而是一个带 material-ID label volume 的 specimen 体数据。
>
> 当前 TIF 路线已经合入 `main`，灵感来自 AntScan 一类体数据集需求。它已经具备项目结构、导入、切片查看、材料表、标签方案、part ROI、part volume、3D 体预览、局部轴重切片、标注核验、nnU-Net v2 后端预设、训练结果模型库和预测复核闭环，但稳定性和跨数据验证程度仍低于 2D/STL 主标注路线。
...
- `File -> Open Project` 打开 TIF 项目入口文件

当前新建的 TIF 项目默认使用 SQLite 作为主索引，项目目录里通常会看到：

- `project.tif_sqlite_manifest.json`：轻量入口文件，用来告诉程序 SQLite 数据库和资产根目录在哪里。
- `project.taxamask_tif.sqlite`：TIF 项目的 specimen、ROI、part、reslice、模型运行记录等索引数据。
- `specimens/`：真正的大体数据、part volume、mask、导出结果和日志 sidecar。

旧版 TIF JSON 项目仍可以打开并迁移，但它不再是正常的新项目主存储。这个设计的好处是：项目索引可以稳定增量保存，大体数据仍留在普通文件夹中，不会被塞进 SQLite 或 JSON。
...
当前批量导入多个 TIF stack 时，默认是 **metadata-only 登记**。也就是说，程序先记录每个 TIF 的路径、shape、dtype、spacing、material map 和 import report，不会立刻把所有个体都读成完整 working volume。单个 TIF 导入仍可能直接生成 working image，但也不会自动生成 `manual_truth`，并且当前界面不会在导入时额外创建空的 `working_edit`。

导入后通常先创建或记录：

- specimen 记录和 source TIF 路径
- `material_map.json`：材料 ID、名称、颜色和是否可训练
- `working/import_report.json`：导入报告
- metadata-only 状态下的 `working_volume` 元信息

当你选中某个 metadata-only specimen 进入切片复核，或切到三维体预览时，程序才会为当前 specimen 启动 `Build working volume / 生成 working volume` 后台任务，生成：

- `working/image.ome.zarr/`：当前用于显示、部位提取和训练准备的工作体数据

这个 materialize 过程一次只针对当前选中的个体，不是批量导入后把所有个体都在后台慢慢处理完。完成前，切片区会显示“正在生成 working volume”，slice 轴和滑条暂时不可用；完成后才能正常切换 Z/Y/X 切片和 3D 预览。

普通 TIF stack 导入或 materialize 后不会自动拥有 `manual_truth`，也不会自动标记为可训练。`working_edit` 或部位级 `editable_ai_result` 是按需创建的：当你真正开始 brush/rectangle/lasso 编辑、导入预测结果，或从已有真值/预测复核层继续修订时，程序才会写出对应 label sidecar。这样可以避免一批大体数据刚导入时就额外写出大量空标签体。
...
| `editable_ai_result` | 模型预测或外部标签导入后的可编辑复核层 | 不能直接训练，需核验为 `manual_truth` |
| `raw_ai_prediction_backup` | 模型原始预测备份 | 不能训练，只用于追溯 |

这对研究可信度很关键。模型预测、PDF 候选或临时 brush 修改都不能绕过人工确认直接变成训练真值。旧项目或顶层整只体预测里可能还会看到 `model_draft` 字段；当前部位/重切片训练闭环使用的是 `editable_ai_result` 和 `raw_ai_prediction_backup`。

### 10.5 Slice 浏览、材料表与标签方案
...
- **材料表 / Material map** 更接近当前体数据里已经存在的 material ID 显示与训练开关，常用于查看 AMIRA 导入或现有标签体里的材料语义。
- **标签方案 / Region label schema** 是训练前必须统一的数字标签计划。它定义“1、2、3 分别代表什么结构”，并可绑定到多个个体、部位或重切片上。

实际标注顺序建议是：

1. 先导入或新建标签方案。
2. 如果当前是在 part/reslice 上做默认训练闭环，把标签方案绑定到当前 part/reslice；如果当前是在整只体上复核 AMIRA 或整只体预测，则先确认 specimen 的材料表能解释当前标签 ID。
3. 再选择具体标签 ID。
4. 最后使用画笔、橡皮或自动填充工具修改标签体。

这样做的意义是：不同个体使用同一套数字标签，后端训练才不会把“1 在 A 个体代表甲结构、在 B 个体代表乙结构”混在一起。

### 10.6 3D 体预览、剖切与 GPU 清晰模式

显示模式切到 `三维体预览` 后，可以用 3D 视图检查整体形态、part volume 和内部结构。它适合在真正切片编辑前寻找目标区域、腺体或其他内部结构的大致空间位置，也适合在 Local Axis Reslice 时观察 source Z-axis、editable output Z-axis 和 roll reference points 的空间关系。

基本操作：

- 左键拖动：旋转体数据。
- 右键拖动：平移画面，方向类似“抓住图像移动”。
- 滚轮：缩放，适合放大到头部、局部部位或目标内部区域。
...
观察侧剖切和清晰截面只改变屏幕显示，不会修改保存的 TIFF、mask、part volume 或训练数据。这个 3D 视图不能直接改 material ID。需要精确修订 material ID 时，仍然要回到 `切片复核`，在 `working_edit` 上用 brush 编辑。

如果右上角或体预览状态栏显示的 renderer 不是 NVIDIA 显卡，通常需要在 Windows 图形设置或 NVIDIA 控制面板里把当前环境的 `python.exe` 指定为高性能显卡。环境改名后也要同步更新 `TAXAMASK_PYTHON_EXE` 或 NVIDIA 控制面板里的 python 路径。

### 10.7 Part ROI、粗切、精切与 part mask

TIF/CT 路线现在保持两级结构：`整只体数据 -> 部位`。不再设计 `部位 -> 内部小部位` 的第三级切割；如果需要更小的内部结构，直接在整只体数据中放大并把它作为另一个 part 切出来。

整只体数据里有两套互补工具：

| 工具 | 你在哪里操作 | 作用 | 结果 |
|---|---|---|---|
| 粗切 ROI | `Full volume` 的切片复核 | 用矩形快速确定大致部位范围 | 更新 ROI 草稿和规则 bbox |
| 精切轮廓 | `Full volume` 的切片复核 | 用手绘轮廓贴合真实结构边界 | 生成可插值的 part mask 草稿 |

`Confirm ROI / 确认 ROI` 是落盘动作。它会把当前草稿切成一个 part，并在 part 下写入 `image.ome.zarr`、`mask.ome.zarr`、`contours.json` 和 `extraction.json`。

#### 10.7.1 在整只体数据里做粗切 ROI

粗切 ROI 适合快速定位，例如先把 head 或某个内部结构的大致空间范围框出来：

1. 选中 specimen 的 `Full volume`。
2. 回到 `切片复核` 模式。
3. 点击 `手动框选 ROI / Draw ROI`。
4. 在同一个观察方向的多张关键切片上画矩形。
5. 程序会把这些矩形合并成一个父级 bbox 容器。

粗切 ROI 是辅助范围，不是最终形状。它适合减少搜索范围、方便 3D 预览和后续确认，但不要把它当作真实部位边界。

当前同一个 ROI 外壳只使用一个观察方向。如果你已经在 Z 方向画了关键切片，后来切到 Y 或 X 方向继续画，程序会提示你切回原方向，或另存一个独立 ROI。

#### 10.7.2 在整只体数据里做精切 part mask

精切轮廓才用于表达真实结构形状：

1. 仍然选中 `Full volume`，保持 `切片复核` 模式。
2. 切回 Z 方向。
3. 点击 `手绘轮廓 / Draw contour`。
4. 在当前切片上沿真实结构边界画闭合轮廓。
5. 在结构开始、形态明显变化、结构结束等关键切片上重复手绘。
6. 点击 `预览自动填充 / Preview auto fill`，程序会在这些手绘关键切片之间生成 mask 预览。
7. 预览合理后可以点 `接受部位 mask / Accept part mask`，这一步只是接受当前预览状态；真正写出 part 文件仍发生在 `Confirm ROI`。

如果你同时画了粗切 ROI 和精切轮廓，确认时会优先使用手绘轮廓的 bbox 来裁出 part，并把手绘轮廓转换成 part 内部坐标写入新 part 的 `contours.json`。也就是说，part image 仍是规则数组容器，但 part mask 可以随切片收缩或扩张。

#### 10.7.3 Save ROI draft 和 Confirm ROI 的区别

`保存 ROI 草稿 / Save ROI draft` 是暂存点。它会保存当前这个待切部位的粗切矩形、手绘轮廓关键切片和 bbox，防止中途退出或继续调整时丢失。

保存草稿后继续画 ROI 或轮廓，会自动更新同一个草稿；最后 `Confirm ROI` 使用的是当前最新草稿状态。

`确认 ROI / Confirm ROI` 不是保存草稿。它会：

- 裁出规则的 part image 容器。
- 如果有整只体数据手绘轮廓，用这些轮廓生成 part mask，并写入 part 的 `mask.ome.zarr` 和 `contours.json`。
- 如果没有手绘轮廓但有 ROI 矩形关键切片，则用 ROI 外壳初始化一个粗略 mask 草稿。
- 写入 `extraction.json`，记录这个 part 是从父级整只体数据哪个 bbox 切出来的。
- 把 ROI 记录标记为已经关联到新 part。

旧代码里仍保留了 `Create part / 新建部位` 的后端函数，但当前界面定位流程里这个按钮不作为推荐入口，按钮也保持禁用。正常流程是：必要时 `Draw ROI` 粗切，使用 `Draw contour` 精切，需要防丢时 `Save ROI draft`，最后 `Confirm ROI`。

#### 10.7.4 进入 part volume 后做什么

确认 ROI 后，左侧 specimen 树会出现新的 part。选中这个 part 后，主要用途是：

- 检查裁出的 part image 和 part mask 是否正确。
- 必要时继续修订 part mask。
- 切到 3D 体预览观察 mask 边界和局部结构。
- 设置 Local Axis Reslice 并导出重切片。

这个 part 仍然是第二级节点，不会再往下生成“内部小部位”。如果你发现需要另一个更小结构，回到 `Full volume`，放大后重新按粗切/精切流程创建一个新的 part。

旧版里曾经有用于快速测试的 `Add rectangular key slice / 添加矩形关键切片` 入口；它不能理解真实部位边界，当前主界面不再把它作为推荐标注方式。

#### 10.7.5 旧项目里残留的 ROI 外壳怎么处理

如果某个 part 是旧逻辑生成的，`contours.json` 里可能残留 `taxamask_roi_shell` 这类定位 ROI 外壳关键帧。当前程序会做两件事：

- 切片覆盖层不再显示这些旧 ROI 外壳框。
- `预览自动填充` 会忽略这些旧 ROI 外壳关键帧，并在状态区提示已经忽略多少个。

如果你想把旧残留永久清掉，可以在该 part 中点击 `清空关键切片 / Clear key slices`。它会清空当前 part 保存的 mask 关键切片、当前自动填充预览，并把 part mask 草稿重置为空；不会删除裁出来的 part image，也不会修改父级整只 TIF 体数据。

part volume、mask、contours 和 extraction metadata 都挂在当前 specimen 的 part 节点下。原始 TIFF stack 仍然保持只读，不会被裁剪、重采样或覆盖。

### 10.8 Local Axis Reslice / 局部轴重切片

Local Axis Reslice 用于对某个 part volume 生成统一方向的局部重切片序列。它不是 Brain Reslice，脑部只是第一个模板。

最小操作闭环：

1. 先创建并选中一个 part volume。
2. 在 3D 体预览中显示 source Z-axis。这个轴是原始 TIFF 切片方向参考，始终锁定。
3. 从 source Z-axis 复制 editable output Z-axis。
4. 在 3D 体预览中拖动输出轴两端，确定重切片推进方向。
5. 设置 roll reference point pair。脑部模板可以使用左右眼一类解剖参照点，但底层字段应理解为通用 roll reference。
6. 检查右侧栏中的轴方向、参照点关系和 local frame 信息。
7. 导出 `image.tif` 和 `metadata.json`；如果 part 已有 mask，可同时导出 `mask.tif`。

重切片方向严格由 editable output Z-axis 决定。roll reference point pair 用来帮助不同个体之间的方向标准化复核，不改变“沿这根主轴切片”的基本逻辑。
...
metadata 应记录 source volume、part volume、source Z-axis、editable output Z-axis、local frame、roll reference point pair、spacing、interpolation 和导出路径。灰度图像重切片使用 linear interpolation；mask / label 重切片使用 nearest-neighbor interpolation。

#### 10.8.1 高风险样本优先复核

Local Axis 批量复核继续使用原来的队列。排序选择“高风险样本优先复核”后，程序会综合建议状态、困难标记、缺失参照点、置信度不确定性和模型版本不一致，把更值得先看的建议排到前面，并在表格中列出原因。

这里的风险分数只是复核顺序，不是“错误概率”。切换排序不会接受建议，也不会改变标签。只有你明确选中一条并点击“接受”，该 Local Axis 建议才会变为已接受；“批量导出已接受项”也只处理已经接受的记录。未接受的 Local Axis 建议和未核验的 TIF 预测都不能进入训练。

### 10.9 TIF 训练素材与训练样本规则

TIF/CT 自动化分成两类材料，不要混在一起：

第一类是 Local Axis 和部位切分素材，主要记录“怎么找到和标准化某个部位”：

- specimen 与 part volume 身份。
...
这些材料可用于训练局部结构定位、部位提取和 local frame 预测模型。

第二类是真正进入体分割训练的标签体样本。当前 `Prepare dataset / Train` 的默认规则是：

- 优先收集整个 TIF 项目里所有已经核验、train-ready 的 part/reslice `manual_truth` 样本。
- 如果项目里没有任何 train-ready part/reslice 样本，才退回使用 train-ready 的整只 specimen 体数据。
- 标签方案本身不是训练样本。必须有 `manual_truth`、图像和标签 shape 匹配、标签 ID 能被当前标签方案解释，并且人工标记为 train-ready。
- 真实 nnU-Net v2 训练至少需要 2 个导出的训练样本；`Prepare dataset` 可以用 1 个样本先检查导出布局。

这个规则的实际含义是：你不需要每次只训练当前打开的一个部位。只要多个个体/部位已经按同一标签方案核验并标记 train-ready，训练按钮会把它们合成训练集。

### 10.10 Brush 编辑与核验为 manual_truth

TIF 的第一阶段编辑方式以 brush 为主。推荐顺序是：

1. 确认当前 part/reslice 已绑定标签方案；如果是在整只体上编辑，先确认材料表里的 material ID 和当前研究标签一致。
2. 选择要画的标签 ID。
3. 再选择画笔、橡皮或其他编辑工具。
4. 在当前 slice 上涂改 `working_edit` 或 `editable_ai_result`。
5. Ctrl 或对应擦除操作用于快速擦除。
6. 支持 slice 级 undo/redo。
7. 点击 `Save current labels` 保存当前可编辑标签层。
8. 人工确认后，在 `Label review / 标注核验` 中点击 `Accept as training truth / 核验为训练真值`。

这里和 2D polygon 工作台不同。TIF 不需要用 Blink 训练局部专家，也不会走 2D 的 SAM 魔棒逻辑。`editable_ai_result` 是可写复核层，不是训练真值；只有核验后的 `manual_truth` 才会被训练导出使用。

如果你只是粗标注，有两种容易混淆的自动填充：

- `Preview auto fill / 预览自动填充` 用在 part mask 生成流程里，依赖手绘轮廓关键切片，结果先是 part mask 预览。
- `Interpolate fill / 插值自动填充` 用在当前标签层里，依赖当前标签 ID 至少出现在两个关键切片上，然后补齐中间切片。

两者都适合快速覆盖较连续的内部结构，但仍然需要人工复核。只有保存并核验为 `manual_truth` 后，才会成为训练真值。

#### 10.10.1 从 manual_truth 导出 Blender 5.0 STL

1. 选中已经有 `manual_truth` 的整只体、part 或 reslice。
2. 点击 `Export reviewed label STL / 导出审核标签 STL`。
3. 勾选要导出的标签并选择目录；默认目录是当前项目的 `exports/meshes/`。
4. 有可信物理单位时，原始毫米测量网格一定单独导出；单位未知时会导出明确标记的 `unitless` 观察网格。只有需要展示时才勾选“生成平滑展示副本”，平滑副本不会覆盖原始网格，也不能用于精确测量。
5. 点击“导出所选 STL”。完成提示必须写明已经与 SQLite 记录复验一致。

STL 使用 XYZ 方向，可直接导入 Blender 5.0。只有 spacing 的物理单位可信且数值有限、为正时，程序才换算为毫米测量网格；单位未知时保留原数值尺度，并在文件名、界面和 SQLite 中标为 `unitless` 观察网格，不能把它当毫米测量。每个标签独立成网格；原始网格保留小结构，不自动平滑、补洞或删除组件。SQLite 是导出状态、来源哈希、尺度、用途、处理参数和 STL 哈希的唯一权威记录，默认不生成网格 JSON sidecar。

导出前会按当前 data version/revision 复验实际读取的 `manual_truth`。如果导出中断或文件后来对不上，在同一窗口的“导出历史”里选择记录，再使用“复验文件”“新建重试导出”或“安全清理未完成文件”。程序不会把半写入文件或被修改的 STL 当作成功结果，也不会自动删除未列入该记录的其他文件。若提示路径经过 junction/symlink/reparse point，先把文件恢复到普通目录，或使用正式重新定位/登记流程；不要用路径重定向绕过复验。
...
它负责的是 `prepare_dataset / train / predict` 这种 material-ID label volume 体分割任务。part/reslice 预测导回时进入 `editable_ai_result`；整只体预测导回时进入待复核的顶层 `working_edit`，同时保留 `raw_ai_prediction_backup`，不能直接覆盖 `manual_truth`。

不要把它和 Local Axis 后端混淆。Local Axis 使用另一份契约：

`docs/contracts/tif_local_axis_backend_contract_v1.md`

Local Axis 后端只提出 global ROI 或 local frame proposal，供你复核后再导出局部轴重切片；它不是普通 TIF material-ID label volume 的预测导入接口。
...
- `train` command
- `predict` command
- materialization policy / required formats，也就是由通用导出器预先生成交换格式，还是由后端只按需要生成工作格式
- model manifest，也就是后续预测使用哪一个已训练模型
...
这样 TaxaMask 才能把本次任务的项目路径、训练体、输出目录、label role、标签方案和 material map 以 contract JSON 形式交给外部脚本。

训练页里的模型库用于管理项目内已经登记的 TIF 体分割模型。训练成功后会登记模型 manifest、样本数、标签方案和备注；后续预测时可以选择已有模型并点击 `Use selected model`，不需要每次重新训练。删除模型登记只会从项目索引里移除这条记录，不会删除磁盘上的权重、manifest 或 run 文件夹。

### 10.12 nnU-Net、MONAI 和自定义脚本怎么理解

TIF 后端不是被写死成 nnU-Net。当前设计是：

- nnU-Net v2 是当前默认预设后端
- MONAI 可以作为候选后端
- 你自己的体分割脚本也可以作为后端

TaxaMask 负责把 train-ready 的体数据导出成后端能读取的布局，例如 nnU-Net v2 的 `nnUNet_raw` 数据集、NIfTI label、manifest 和 label-ID 映射；真正训练和预测由你配置的外部命令完成。

内置 nnU-Net v2 适配器会调用：

- `nnUNetv2_plan_and_preprocess`
- `nnUNetv2_train`
- `nnUNetv2_predict`

这些命令必须存在于你设置的 TIF 后端 Python 环境里。当前项目常用的是 `taxamask` conda 环境，但是否装了 nnU-Net v2，要看该环境的 `Scripts/` 下是否能找到这些命令。

预测结果导回时不会覆盖 `manual_truth`。part/reslice 预测进入 `editable_ai_result`；整只体预测进入待复核的顶层 `working_edit`。如果你觉得预测质量可以用，也需要先人工检查，再决定是否把修订后的结果核验为训练真值。

### 10.13 TIF train-ready 检查

一个 part/reslice 要进入当前默认训练集，至少应满足：

- part 记录和 part volume 存在
- 已导出或选定 reslice 图像；当前默认 part/reslice 训练导出读取的是 reslice 图像，而不是只读取未重切片的 part image
- 已绑定非空标签方案
- `manual_truth` sidecar 存在
- 图像和标签 shape 匹配
- 标签 ID 能被标签方案解释
- 人工标记为已核验 train-ready

如果没有任何 train-ready part/reslice，程序才会尝试使用 train-ready 的整只 specimen。整只 specimen 至少需要：
...
### 10.14 TIF 存储管理与智能体清理

入口位于 TIF 工作台右侧的存储管理区域，只保留四个操作：

- `分析占用`：只读扫描当前项目，统计源数据、权威工作数据、可重建缓存和清理中的内容；不会移动或删除文件。
- `生成清理计划`：自动重新扫描一次，并生成 dry-run 计划。它只列入已经登记、身份核验通过且可以重新生成的缓存；历史遗留但未登记的文件继续受保护。
- `交给内嵌智能体`：把最新占用报告和清理计划交给 Agent。Agent 会说明能释放多少、哪些内容受保护，并在获得研究者明确授权后执行隔离或最终清理；不要求研究者照着报告手工删目录。
- `打开最新报告`：打开最新盘点或计划文件，供研究者查看项目位置、预计释放量、保护原因和审计状态。

存储层级按恢复价值区分：L0 包含源 TIF、`manual_truth` 和未知/未登记文件，L1 是当前项目仍在使用的工作与训练数据，这两层都受保护；L2 是已登记的可重建物化、预览缓存和 staging，L3 是事务临时文件或未完成下载。当前自动清理计划只纳入身份与哈希均通过安全门的 L2 可重建缓存；模型权重、训练/预测合同、标签方案和未登记历史文件不因体积大而自动进入删除计划。

清理采用“计划 -> 隔离 -> 宽限期 -> 最终删除”的生命周期。隔离阶段仍可恢复；最终删除后不可从 TaxaMask 恢复。只有精确确认本次 plan ID 才能执行最终删除，宽限期未到时还需要显式提前删除授权，并会留下审计记录。因此在研究数据仍需复核时，优先让 Agent 先隔离、实际运行项目和模型预测，确认无误后再完成最终删除。

新逻辑还减少了未来重复占用：标签数组按实际标签 ID 使用最小安全 dtype；nnU-Net 只在训练或预测需要时物化 NIfTI，不再默认同时预生成多套通用格式；相同输入可复用内容寻址缓存或硬链接。训练仍然需要必要的工作集和模型输出，所以它减少的是无意义的长期副本，不是宣称整个训练过程零重复。
...
模型方案是保存在当前项目索引里的 2D/STL 模型配置快照。它记录：
...
批量运行期间可以从进度窗口停止任务。停止不是强行杀掉已经发出的网络请求，而是停止继续派发剩余图片，并等待已发出的请求收尾。这样既能避免一个错误设置跑完整个几千张任务，也能尽量保护项目索引、报告文件和 API 响应记录不留下半写入状态。
...
第一版重点支持 `predict_child` 推理。外部脚本不应该直接修改项目索引，也不应该把结果写成人工真值。
...
TIF 后端不使用这份 2D 契约。普通 TIF 体分割后端使用独立的 `ant3d_tif_backend_contract_v1`，公开说明在：

`docs/contracts/ant3d_tif_backend_contract_v1.md`

配置入口在 `TIF Volume Model Settings`。Local Axis proposal 后端则使用：
...
2D Locator/SAM、Blink、TIF、nnU-Net 和 Local Axis 训练入口使用同一套运行记录。项目 SQLite 保存实际生效的参数、输入文件及哈希、训练/验证实际拆分、起始和输出权重、后端与代码版本、开始结束时间、成功/失败/取消状态和产物索引。训练报告和模型文件仍保存在各自 run/report 目录中，可从“训练结果”打开；SQLite 负责事实与对账，不把 JSON 当作平级项目后端。

2D Locator/SAM 和 Blink 在开始训练前会在后台做两轮完整内容复验，界面显示当前文件、完成比例、读取速度和预计剩余时间。第一轮建立只供本次 run 使用的验证凭据，第二轮在真正启动训练前再次读取内容，防止两次检查之间文件被替换。选择某个图片组训练时，只检查该组实际 source/人工标签以及共享 schema、本次配置和选中的起始权重，不再扫描未参与训练的图片。可以安全取消；取消后不会启动训练，已经创建的 run 会记为 `cancelled`。这里不使用只看文件大小和修改时间的缓存替代第二轮复验，因为同长度文件可以在恢复时间戳后骗过这类缓存。

训练结束后、SQLite 准备记为成功并激活新权重之前，程序会再复验一次本 run 绑定的输入。如果训练期间文件发生了持续的外部变化，本次 run 会失败，待发布权重不会变成当前可用模型。这个收尾检查提高了可信度，但仍不要在训练期间搬动或修改源图和标签；未来如采用不可变输入快照，才能进一步消除“修改后又恢复原字节”这类极端时间窗口。

程序不区分“正式训练”和“临时训练”。你可以在训练结果中用 `Edit Run Note / 编辑运行备注` 写明目的、重要性、结论和后续用途。备注可以修改，但不能改写当时的参数、样本拆分、哈希和运行状态。

#### 文件不一致时怎么善后

训练发现源图、`manual_truth`、配置快照或权重缺失、被修改或与记录不一致时，会停止并打开“训练文件完整性恢复”。根据真实情况选择：

- 文件没有变，只是搬了位置或从备份恢复：选中记录后点“重新定位 / 恢复副本”，程序会先复验内容哈希。
- 文件是你有意修改的新内容：点“登记为新版本”，填写原因；旧版本历史保留，新版本重新建立基线。
- 不确定发生了什么：先点“重新复验”，必要时“导出脱敏诊断”再交给 Agent 排查。
- 文件确实损坏或找不到：先从可信备份恢复，不能跳过门禁继续训练。

只有所有必需文件重新复验通过后，才应重试训练。这个过程不会把未核验 AI 草稿提升为真值。
...
当前推理日志还会按 `input -> locator -> crop -> expert_route -> sam -> assemble` 记录阶段、决定原因和耗时，写入 `TaxaMask_outputs/runtime_logs/` 下现有运行日志。它用于回答“是 Locator 没找到、路由没命中，还是 SAM 没生成 polygon”，不是把更细的模型输出偷偷加入训练。默认不会写原图、完整 mask、大数组、API 密钥或私人绝对路径。
...
当前 TIF 体分割后端里，`Prepare dataset` 和 `Train backend` 会使用同一套 train-ready 样本选择规则：

- 有 train-ready part/reslice 时，导出项目内所有可训练 part/reslice。
- 没有 train-ready part/reslice 时，才导出 train-ready 的整只 specimen。
- 导出使用已核验的 `manual_truth`，不会直接拿 `editable_ai_result` 或未保存编辑层训练。
...
- 哪个 specimen 被导出
- 哪个 part/reslice 被导出；如果走整只体兜底路径，则记录整只 specimen 的导出信息
- 图像和标签 shape 是什么
- label role 是不是 `manual_truth`
- 绑定的标签方案或材料表是什么
- TaxaMask 原始标签 ID 如何映射到 nnU-Net 的紧凑类别 ID
- 输出文件分别在哪里

真实 nnU-Net v2 训练至少需要 2 个导出的训练样本。如果你只有 1 个样本，可以先运行 `Prepare dataset` 检查导出布局，但 `Train backend` 会在真正调用 nnU-Net 前提示样本数不足。
...
### 13.6 最常见误区 6：以为 TIF 的 editable_ai_result 可以直接训练

不是。

`editable_ai_result` 是 part/reslice 模型预测或外部标签导入后的可编辑复核层。整只体预测当前会进入待复核的顶层 `working_edit`，同时保留 `raw_ai_prediction_backup` 和 legacy `model_draft` 审计副本。它们都可以帮助你加速检查和修订，但不能直接作为训练真值。

稳妥流程是：

1. 导入预测为待复核标签层，同时保留 `raw_ai_prediction_backup`
2. 人工检查和修订该复核层
3. 在 `Label review / 标注核验` 中明确核验为 `manual_truth`
4. 标记为 train-ready
5. 再参与 `Prepare dataset / Train backend`

### 13.7 最常见误区 7：以为标签方案等于训练数据

不是。

标签方案只是数字标签的说明书。它告诉程序“1、2、3 各代表什么结构”，让多个个体使用同一套语义。真正训练还需要对应图像、`manual_truth` 标签体、shape 匹配、标签 ID 检查通过，以及人工 train-ready 状态。
...
5. 搬盘后先用 `Check / Relocate Project Images` 检查图片路径，再判断是不是项目入口或索引本身损坏
6. TIF 预测结果先当作待复核标签层，不要直接当成 `manual_truth`；part/reslice 预测对应 `editable_ai_result`，整只体预测对应顶层 `working_edit`
7. TIF 训练前先确认可训练样本数；真实 nnU-Net v2 训练至少需要 2 个样本
...
| 按钮 / 控件 | 作用 | 影响层 |
|---|---|---|
| Import TIF stack | 导入连续 TIF stack | 批量时先登记 metadata-only specimen |
| Build working volume | 为当前 metadata-only specimen 生成工作体数据 | 当前 specimen 的 `working/image.ome.zarr` |
...
| Draw ROI | 在整只体数据切片上画粗切定位矩形 | 当前 ROI 草稿和 bbox |
| Save ROI draft | 暂存当前粗切 ROI、手绘轮廓和 bbox | specimen 的 part ROI 记录，不生成 part |
| Confirm ROI | 用当前草稿生成 part volume 和 part mask | part image / part mask / contours / extraction 记录 |
| Cancel ROI | 取消未关联 part 的 ROI 草稿 | ROI 草稿状态 |
| Draw contour | 在整只体数据或 part volume 当前 Z 切片手绘 mask 轮廓 | 整只体数据草稿，或 part 的 `contours.json` |
| Preview auto fill | 根据手绘轮廓关键切片生成自动填充预览 | 当前 part mask 预览 |
| Accept part mask | 接受当前 mask 预览 | 整只体数据中等待 `Confirm ROI` 落盘；part volume 中写入 `mask.ome.zarr` |
| Clear key slices | 清空当前手绘 mask 关键切片和预览 | 草稿轮廓或 part contours，不删 part image |
| Delete part volume | 删除当前 part volume | 裁出的 part 文件，不删父级 TIF |
| Material map | 编辑材料 ID、名称、颜色和 trainable 状态 | TIF 标签语义层 |
| Region label schema | 统一多个个体/部位的数字标签方案 | TIF 训练标签语义层 |
| Bind to current part | 把当前标签方案绑定到当前 part/reslice | 当前 part 的训练配置 |
| Save current labels | 保存当前可编辑标签层 | `working_edit` 或 `editable_ai_result` |
| Accept as training truth | 将人工确认后的当前标签核验为训练真值 | `manual_truth` |
| Prepare dataset | 导出项目内 train-ready 样本给后端 | 后端训练交接 |
| Train backend | 调用 TIF 训练后端 | external TIF run；成功后登记模型库 |
| Trained model / Use selected model | 选择已有训练模型用于预测 | 当前模型 manifest |
| Import prediction | 运行预测并导入待核验结果 | part/reslice: `editable_ai_result` + `raw_ai_prediction_backup`; top-level: `working_edit` + `raw_ai_prediction_backup` |
...
| Profile / 方案 | 新建、复制、删除或切换当前模型方案 | 当前项目索引的 `model_profiles` |
...
| Ask Agent | 发送当前设置页的短诊断索引卡片 | Agent 根据路由表定位大模型对接文件、源码和契约；TIF 会区分体分割后端与 Local Axis proposal 后端 |
...
- 你手里已经有一批原始 PDF，或者需要先根据目标类群合法采集开放 PDF
- 你想先快速找出可能真正值得标注的图
- 你不想一开始就把全部图片扔进项目里
...
TaxaMask 后面的 PDF evidence 链路会告诉你候选图来自哪篇 PDF、哪一页、哪段 caption 或附近文字。但如果最开始没有记录“这批 PDF 是怎么来的”，来源链就会缺一截。`download_manifest.csv` 可以帮助你以后回看哪些文献被检索过、哪些 PDF 是合法开放链接下载的、哪些没有拿到 PDF。
...
### 15.4 标准流程模板 D：TIF/CT 中从整只体数据粗切、精切并生成 part mask

这个模板适合：

- 你手里是一批 TIF stack 或 CT 派生体数据
- 你要从整只个体中切出 head、brain 或其他内部/局部结构
- 你希望先快速定位范围，再用手绘关键切片减少无关结构干扰

#### 推荐步骤

**步骤 1：新建或打开 TIF 项目**

1. 从启动中心进入 `TIF Volume`。
2. 使用 `File -> New TIF Volume Project` 新建项目，或 `File -> Open Project` 打开已有 TIF 项目入口文件。
3. 新项目默认会生成 SQLite 主索引和 `project.tif_sqlite_manifest.json` 入口文件；大体数据仍保存在 `specimens/` sidecar 目录里。

**为什么先确认项目入口**

TIF 项目不是把整只体数据塞进一个 JSON 文件里。入口文件、SQLite 索引和 sidecar 文件夹共同构成一个项目。搬盘或备份时，不能只拿走一个数据库文件，也不能只拿走 `specimens/`。

**步骤 2：导入 TIF stack**

1. 在 TIF 工作台内点击 `Import TIF stack`。
2. 如果一次选择多个 TIF，程序会先把它们登记为 metadata-only specimen。
3. 批量导入完成后，不要以为每个个体都已经在后台生成完整 working volume；它们只是先进入项目索引，等待你选中后再 materialize。

**你会看到什么**

- 列表里出现新的 specimen。
- metadata-only 个体还不能立刻切换 Z/Y/X 切片。
- 当你选中某个 specimen 进入切片复核或三维体预览时，程序会为当前个体启动 `Build working volume / 生成 working volume`。

**步骤 3：让当前 specimen 生成 working volume**

1. 在左侧树中选中目标 specimen 的 `Full volume`。
2. 如果它还是 metadata-only，等待 `Build working volume` 完成。
3. 完成后检查切片滑条、Z/Y/X 方向切换和三维体预览是否可用。

**这里最重要的判断点**

只有当前 specimen materialize 后，你才能在切片复核中画 ROI、切换轴向或进入 3D 预览。批量导入本身不是“把所有体数据都处理好了”。
...
2. 回到 `切片复核` 模式。
3. 选择一个观察方向，例如 Z。
4. 点击 `Draw ROI / 手动框选 ROI`。
5. 在同一方向的多张关键切片上，根据目标部位的大致范围画矩形。
6. 如果中途怕丢，可以点 `Save ROI draft / 保存 ROI 草稿`。
7. 继续画框时，同一个 ROI 草稿会随最新矩形更新。

**不要这样做**

- 不要在一个 ROI 里混用 Z、Y、X 三个方向画矩形。
- 不要把 `Save ROI draft` 理解成最终导出。
- 不要把粗切矩形当成真实部位边界。

**步骤 5：在整只体数据中做精切手绘轮廓**

1. 仍然选中 `Full volume`，保持 `切片复核`。
2. 切回 Z 方向。
3. 点击 `Draw contour / 手绘轮廓`。
4. 在结构开始、形态变化明显、结构结束等关键切片上画闭合轮廓。
5. 点击 `Preview auto fill / 预览自动填充`，检查插值生成的 mask 是否合理。
6. 需要暂存时点击 `Save ROI draft`，它会保存当前粗切 ROI 和手绘轮廓关键切片。

**步骤 6：确认 ROI 并生成 part volume 和 part mask**

1. 检查粗切范围和手绘 mask 预览是否覆盖目标结构。
2. 点击 `Confirm ROI / 确认 ROI`。
3. 输入 part ID 和显示名，例如 `head` / `Head`。
4. 程序会裁出 part volume，并在左侧树下出现新的 part。

确认 ROI 后得到的是：

- 一个规则数组容器，也就是 part image。
- 一个由整只体数据手绘轮廓生成的 part mask，前提是你画了轮廓关键切片。
- 如果你只画了粗切 ROI、没有手绘轮廓，程序会退回到 ROI 外壳初始化的粗略 mask 草稿。
- ROI 与 part 的关联记录。
- 后续 Local Axis Reslice 可使用的 part 节点。

**步骤 7：进入 part volume 后复核和导出**

1. 在左侧树中选中新生成的 part。
2. 回到 `切片复核` 或切到三维体预览。
3. 检查 part image 和 part mask 是否合理。
4. 必要时继续修订 part mask。
5. 需要统一方向时进入 Local Axis Reslice。

这里仍然是两级结构：`Full volume -> part`。程序不会在 part 下面再自动切一个 subpart；如果需要另一个更小结构，回到 `Full volume` 放大后新建另一个 part。

**步骤 8：如果旧项目里有残留 ROI 外壳**

如果 `Preview auto fill` 提示忽略了旧版 ROI 外壳关键切片，说明这个 part 可能来自旧逻辑。

推荐处理：

1. 如果当前 part mask 是你还想保留的结果，先确认是否已经导出或备份。
2. 如果旧框确实干扰复核，点击 `Clear key slices`。
3. 它会清空当前 part 的 mask 关键切片、自动填充预览和 mask 草稿，但保留裁出来的 part image。
4. 然后重新在 part volume 里添加真正要参与 mask 插值的关键切片。

**步骤 9：需要统一方向时再做 Local Axis Reslice**

1. 选中已经生成并复核过的 part volume。
2. 切到三维体预览。
3. 显示 source Z-axis。
4. 复制出 editable output Z-axis。
5. 拖动 output axis 两端，确定局部重切片方向。
6. 打开观察侧剖切面，点选 roll reference pair。
7. 确认 local frame 后导出 `image.tif`、`metadata.json`，如果已有 part mask 也可导出 `mask.tif`。

#### 这条流程最后得到的是什么

你会得到三类材料：

1. **part volume**：从整只体数据中裁出的局部结构体。
2. **part mask**：由整只体数据手绘轮廓生成，并可在 part 内继续复核的目标结构 mask。
3. **Local Axis reslice**：在统一局部坐标系下导出的重切片序列和 metadata。

这三者不要混成一个动作。粗切 ROI 负责快速定位，手绘轮廓负责精确分割，Local Axis 负责方向标准化。

---

### 15.5 标准流程模板 E：TIF/CT 从标签方案到训练、推理和复核

这个模板适合：

- 你已经有整只 TIF、AMIRA 标注、part volume 或 Local Axis reslice
- 你想把多个个体/部位统一成同一套数字标签
- 你想用 nnU-Net v2、MONAI 或自定义体分割后端训练，并把预测结果导回 TaxaMask 复核

#### 推荐步骤

**步骤 1：先决定训练单位**

当前默认训练闭环优先使用 part/reslice 样本。也就是说，最稳妥的路线是：

1. 先按模板 D 从整只体数据切出 part。
2. 如果不同个体方向差异大，再做 Local Axis Reslice 并导出 `image.tif`。
3. 在 part/reslice 上做标签方案绑定、标注、核验和训练。

如果项目里没有任何 train-ready 的 part/reslice，程序才会退回整只体 train-ready 样本。整只体路线更适合 AMIRA/Avizo 已有整只体人工标签，或你确实想训练整只体分割。

**步骤 2：先统一标签方案，再选择标签**

在 `标注与训练` 区域，先处理 `Region label schema / 结构区域标签表`：

1. 如果已有同项目导出的标签表，点击 `Import schema` 导入 TaxaMask 标签表 JSON。
2. 如果是新方案，点击 `New schema`，填写 `Schema ID` 和用户自定义部位名。
3. 用 `Add region label` 增加标签，检查每个标签 ID、名称和颜色。
4. 点击 `Save schema` 保存。
5. 选中当前 part/reslice 后，点击 `Bind to current part`。

这里的标签方案不是“某一张图的标注结果”，而是多个个体共用的编号说明书。`Part group tags / 部位批次标签` 只用于把部位分组给预测或复核批次，不是训练类别，也不是标签方案。

**步骤 3：再做切片标注**

推荐顺序是：

1. 在 `Current labels / 当前标签` 里先选要画的标签 ID。
2. 再到 `Annotation tools / 标注工具` 里选画笔、橡皮、矩形、套索或拾色。
3. 在切片上涂改当前可编辑标签层。
4. 如果只是粗标注，可以让当前标签 ID 至少出现在两个关键切片上，再点 `Interpolate fill / 插值自动填充` 补齐中间切片。
5. 点 `Save current labels / 保存当前标注`。

part/reslice 上的预测或外部导入结果通常编辑在 `editable_ai_result`；整只体上的当前编辑通常是 `working_edit`。它们都还不是训练真值。

**步骤 4：核验为训练真值**

确认标注质量可以作为训练真值后：

1. 在 `Label review / 标注核验` 中点击 `Accept as training truth / 核验为训练真值`。
2. 程序会把当前人工确认过的可编辑层提升为 `manual_truth`。
3. 对 part/reslice，程序会把对应样本标记为 train-ready。
4. 对整只体，需要确认 specimen 已有 `manual_truth`、工作图像、材料表和 train-ready 状态。

这一步是研究可信度边界：未保存编辑层、`editable_ai_result`、`working_edit` 和 `raw_ai_prediction_backup` 都不能直接训练。

**步骤 5：准备训练数据**

在 `Model training / 模型训练` 中点击 `Prepare dataset`。

程序会：

- 优先收集项目内所有 train-ready part/reslice 的 `manual_truth`。
- 如果没有可训练 part/reslice，才收集 train-ready 整只体。
- 导出后端需要的体数据、标签体、manifest、标签 ID 映射和日志。

如果这里提示没有数据，通常不是“当前切片没有显示标签”，而是还没完成保存、核验、标签方案绑定、reslice 导出或 train-ready 检查。

**步骤 6：训练后端**

确认 TIF 后端设置里有 Python、`prepare_dataset`、`train`、`predict` 命令，并且命令里保留 `{contract}` 或 `{contract_json}` 后，点击 `Train backend`。
...
这些命令必须在你配置的后端 Python 环境里存在。训练成功后，模型会登记到 `Trained models / 训练模型`，包括 manifest、样本数、标签方案和备注。

**步骤 7：选择已有模型再预测**

后续推理不需要每次重新训练：

1. 在 `Trained model` 下拉框选择已有模型。
2. 必要时写备注，方便以后知道这次模型用的是哪批样本。
3. 点击 `Use selected model`，把该模型 manifest 填入预测配置。
4. 在 `Batch prediction targets` 中勾选要预测的 part/reslice；如果当前是在整只体视图，预测会使用当前整只体目标。
5. 点击 `Import prediction`。

预测结果不会覆盖 `manual_truth`。part/reslice 预测进入 `editable_ai_result` 并保留 `raw_ai_prediction_backup`；整只体预测进入顶层 `working_edit` 并保留原始备份和旧版审计副本。
...
1. 打开待复核标签层。
2. 用画笔、橡皮、插值填充等工具修正错误。
3. 点 `Save current labels`。
4. 质量可用时，再点 `Accept as training truth`。
5. 下一轮 `Prepare dataset / Train backend` 才会使用这些新核验的 `manual_truth`。

#### 这条流程最后得到的是什么

你会得到四类可追溯材料：

1. **标签方案**：多个个体共用的数字标签说明。
2. **manual_truth 标签体**：人工核验后的训练真值。
3. **后端训练 run**：导出数据、contract、日志、result JSON 和模型 manifest。
4. **待复核预测层**：模型预测的可编辑版本和原始备份。

最重要的是：训练集不是当前屏幕上的一张切片，而是项目内所有已经核验、train-ready、标签方案一致且 shape 检查通过的体数据样本。

---

### 15.6 标准流程模板 F：当项目搬盘、路径变化或运行中断时，应该怎么稳妥处理
