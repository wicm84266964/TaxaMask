...
4. 原始测量网格一定单独导出。只有需要展示时才勾选“生成平滑展示副本”，平滑副本不会覆盖原始网格。
5. 点击“导出所选 STL”。完成提示必须写明已经与 SQLite 记录复验一致。

STL 使用 XYZ 方向和毫米单位，可直接导入 Blender 5.0。每个标签独立成网格；原始网格保留小结构，不自动平滑、补洞或删除组件。SQLite 是导出状态、来源哈希、尺度、处理参数和 STL 哈希的唯一权威记录，默认不生成网格 JSON sidecar。

如果导出中断或文件后来对不上，在同一窗口的“导出历史”里选择记录，再使用“复验文件”“新建重试导出”或“安全清理未完成文件”。程序不会把半写入文件或被修改的 STL 当作成功结果，也不会自动删除未列入该记录的其他文件。
