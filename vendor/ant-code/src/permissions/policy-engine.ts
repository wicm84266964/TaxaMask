...
import { decideNetworkAccess } from "./network-policy.ts";
import { normalizeToolPath, resolveToolPath } from "./path-utils.ts";
 ...
 */
export function decidePermission(request: PermissionRequest, policy: PermissionPolicy = {}) {
...
function isShellCommandRequest(request: PermissionRequest) {
  return request.risk === "execute"
    && (request.toolName === "powershell" || request.toolName === "bash" || typeof request.command === "string");
}

function decideBrowserRequest(request: PermissionRequest, policy: PermissionPolicy) {
...
function decideDocumentRequest(request: PermissionRequest, policy: PermissionPolicy) {
  if (policy.readonly) {
...
function decideMemoryRequest(request: PermissionRequest, policy: PermissionPolicy) {
 ...
 */
function decidePathAccess(request: PermissionRequest, workspace: string, policy: PermissionPolicy = {}) {
...
function isPathReadRisk(risk: unknown) {
 ...
 */
function decideCommand(request: PermissionRequest, policy: PermissionPolicy) {
 ...
 */
function decideCommandPathAccess(request: PermissionRequest, policy: PermissionPolicy) {
 ...
 */
function extractCommandTargetPaths(command: string) {
...
function addPathMatch(paths: Set<string>, match: RegExpMatchArray, options: { allowRelative?: boolean } = {}) {
...
function addPathTokens(paths: Set<string>, segment: unknown) {
 ...
 */
function isCommandPathCandidate(value: string, options: { allowRelative?: boolean } = {}) {
 ...
 * @param {string} value
 */
function cleanCommandPath(value: string) {
 ...
 */
function decideNetworkRequest(request: PermissionRequest, policy: { networkMode?: string; allowedHosts?: string[] }) {
 ...
 */
export function isInside(workspace: string, candidate: string) {
