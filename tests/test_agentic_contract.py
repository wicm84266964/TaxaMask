...
class AgenticContractTests(unittest.TestCase):
    ...
    def test_ant_code_source_write_hook_requests_dashboard_approval(self):
        script = textwrap.dedent(
            ...
            import {{ createToolRuntime }} from {json.dumps((PROJECT_ROOT / "vendor" / "ant-code" / "src" / "tools" / "runtime.ts").resolve().as_uri())};
    ...
    def test_ant_code_truncates_oversized_tool_results_before_model_context(self):
        result_module = (PROJECT_ROOT / "vendor" / "ant-code" / "src" / "tools" / "result.ts").resolve().as_uri()
        self._run_node_contract(
            f"""
            import assert from "node:assert/strict";
            import {{ DEFAULT_TOOL_RESULT_MAX_BYTES, serializeToolResult }} from {json.dumps(result_module)};

            const small = serializeToolResult({{ ok: true, result: {{ value: "ok" }} }});
            assert.equal(small.truncated, false);

            const large = serializeToolResult(
              {{ ok: true, result: {{ entries: "x".repeat(600_000) }} }},
              {{ maxBytes: DEFAULT_TOOL_RESULT_MAX_BYTES }}
            );
            assert.equal(large.truncated, true);
            assert.ok(large.bytes <= DEFAULT_TOOL_RESULT_MAX_BYTES);
            assert.match(large.content, /truncated/i);

            const tiny = serializeToolResult({{ ok: true, result: "x".repeat(1000) }}, {{ maxBytes: 64 }});
            assert.equal(tiny.truncated, true);
            assert.ok(tiny.bytes < 1000);
            """
        )

    def test_ant_code_retries_a_gateway_response_without_visible_content(self):
        session_module = (PROJECT_ROOT / "vendor" / "ant-code" / "src" / "core" / "session.ts").resolve().as_uri()
        self._run_node_contract(
            f"""
            import assert from "node:assert/strict";
            ...
            assert.equal(empty.mustRetry, false);

            const lengthStop = analyzeAssistantOutputHealth(
              {{ text: "partial", content: [], toolCalls: [], stopReason: "length", raw: {{}} }},
              "partial",
              null
            );
            assert.equal(lengthStop.ok, false);
            assert.equal(lengthStop.reasons.includes("stop_reason:length"), true);
            ...
            const toolSummary = summarizeToolCalls(
              ...
              [{{ content: {json.dumps(json.dumps({"ok": True}))}, ok: true, blocked: false, interrupted: false, decision: null, truncated: true }}]
            );
    ...
    def test_ant_code_uses_explicit_config_without_user_home_defaults(self):
        config_module = (PROJECT_ROOT / "vendor" / "ant-code" / "src" / "config" / "load-config.ts").resolve().as_uri()
        self._run_node_contract(
            f"""
            import assert from "node:assert/strict";
            import fs from "node:fs/promises";
            ...
            assert.notEqual(explicit.modelAlias, "user-default-model");

            const projectOverExplicit = await loadConfig({{
              cwd,
              env: {{ HOME: home, USERPROFILE: home, LAB_AGENT_CONFIG: explicitPath }}
            }});
    ...
    def test_ant_code_project_config_writes_are_atomic(self):
        config_store_module = (PROJECT_ROOT / "vendor" / "ant-code" / "src" / "dashboard" / "config-store.ts").resolve().as_uri()
        self._run_node_contract(
            f"""
            import assert from "node:assert/strict";
            import fs from "node:fs/promises";
    ...
    def test_ant_code_project_gateway_change_does_not_inherit_environment_key(self):
        sessions_module = (PROJECT_ROOT / "vendor" / "ant-code" / "src" / "dashboard" / "sessions.ts").resolve().as_uri()
        self._run_node_contract(
            f"""
            import assert from "node:assert/strict";
            import fs from "node:fs/promises";
    ...
    def test_ant_code_file_preview_blocks_link_escape(self):
        files_module = (PROJECT_ROOT / "vendor" / "ant-code" / "src" / "dashboard" / "files.ts").resolve().as_uri()
        self._run_node_contract(
            f"""
            import assert from "node:assert/strict";
            import fs from "node:fs/promises";
    ...
    def test_ant_code_separates_visible_transcript_from_model_context(self):
        store_module = (PROJECT_ROOT / "vendor" / "ant-code" / "src" / "storage" / "session-store.ts").resolve().as_uri()
        self._run_node_contract(
            f"""
            import assert from "node:assert/strict";
            import fs from "node:fs/promises";
    ...
    def test_ant_code_does_not_compact_without_prior_conversation(self):
        context_module = (PROJECT_ROOT / "vendor" / "ant-code" / "src" / "core" / "context-window.ts").resolve().as_uri()
        self._run_node_contract(
            f"""
            import assert from "node:assert/strict";
