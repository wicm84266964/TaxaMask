...
import { recordHookAudit, updateHookAudit } from "./audit-store.ts";
import { createHookPayload, eventMayBlock, summarizeHookPayload, truncateText } from "./events.ts";
import { formatHookType, matchHooks } from "./registry.ts";
import { runBuiltinHook } from "./builtins.ts";

type MatchedHook = ReturnType<typeof matchHooks>[number];
...
type RunHookOptions = {
  ...
  onRecord?: ((record: unknown) => unknown) | undefined;
  auditRecordId?: unknown;
  background?: boolean;
};
...
function hookTypeLabel(hook: MatchedHook) {
...
function unrefResource(value: Unrefable | null | undefined) {
  value?.unref?.();
}

let runningDepth = 0;

export async function runHooks(options: Record<string, unknown> = {}) {
  const event = options.event;
  if (!event || runningDepth > 0 || options.skipHooks === true) {
    return { ok: true, blocked: false, results: [] as HookRunResult[] };
  }
  const config = isRecord(options.config) ? options.config : {};
  const incomingPayload = isRecord(options.payload) ? options.payload : {};
  const cwd = typeof options.cwd === "string"
    ? options.cwd
    : typeof incomingPayload.cwd === "string"
      ? incomingPayload.cwd
      : process.cwd();
  const payload = createHookPayload(event, incomingPayload, {
    cwd,
  ...
  try {
    for (const hook of hooks) {
      const hookOptions: RunHookOptions = {
        hook,
        payload,
        cwd,
        env: isRecord(options.env) ? options.env as NodeJS.ProcessEnv : undefined,
        config,
        hooksTrusted: options.hooksTrusted === true,
        onRecord: typeof options.onRecord === "function"
          ? options.onRecord as (record: unknown) => unknown
          : undefined
      };
      ...
      if (!shouldAwait) {
        ...
        void runOneHook({ ...hookOptions, auditRecordId: scheduled.id, background: true }).catch((error: unknown) => {
          updateHookAudit(String(scheduled.id), {
            ok: false,
            status: "failed",
      ...
      if (result.blocked && hook.blocking && eventMayBlock(event)) {
        return {
          ...
          blockingError: result.error ?? { code: "HOOK_BLOCKED", message: result.message || `${hook.name} blocked ${String(event)}` },
          results
        };
      }
    }
...
async function runOneHook(options: RunHookOptions): Promise<HookRunResult> {
  ...
  try {
    if (hook.type === "command") {
      ...
      result = await runBuiltinHook(String(hook.builtin ?? ""), {
        hook,
        payload,
        cwd: options.cwd,
        env: options.env,
  ...
  const record = options.auditRecordId
    ? updateHookAudit(String(options.auditRecordId), auditRecord)
...
function shouldAwaitHook(hook: MatchedHook, options: Record<string, unknown> = {}) {
  if (hook.type !== "command") {
    return true;
  }
...
async function runCommandHook(hook: MatchedHook, options: RunHookOptions): Promise<HookRunResult> {
  ...
  env.ANT_CODE_HOOK_EVENT = String(hook.event ?? "");
  env.ANT_CODE_HOOK_NAME = String(hook.name ?? "");
  ...
  return new Promise((resolve) => {
    ...
    if (options.background === true) {
      timeout.unref?.();
      child.unref?.();
      unrefResource(child.stdout as Unrefable | null | undefined);
      unrefResource(child.stderr as Unrefable | null | undefined);
    }

    const append = (kind: string, chunk: Buffer) => {
...
function buildHookEnv(source: NodeJS.ProcessEnv, allowlist: unknown = []): NodeJS.ProcessEnv {
  const env: NodeJS.ProcessEnv = { ...source };
  const allowed = new Set(Array.isArray(allowlist) ? allowlist.map(String) : []);
