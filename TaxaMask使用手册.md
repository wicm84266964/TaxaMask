...
- 在整只体数据中用同一观察方向的 ROI 关键切片矩形定位目标部位，例如 head；确认 ROI 后生成 part volume。
- 进入 part volume 后，再用矩形关键切片或手绘轮廓编辑真正的 part mask。
...
### 10.7 Part ROI、关键切片外壳、part volume 与 part mask

TIF/CT 路线中，建议优先只对目标部位工作，而不是全量重切整个 TIFF。

这里有两层操作，不要混在一起：

| 层级 | 你在哪里操作 | 目的 | 结果 |
|---|---|---|---|
| 部位定位 ROI | 整只 full volume 的切片复核里 | 找到要切出来的部位范围 | `确认 ROI` 后生成 part volume |
| 部位 mask 编辑 | 已生成的 part volume 里 | 细修真正的 part mask | `预览自动填充` / `接受部位 mask` 后写入 part mask |

#### 10.7.1 在整只体数据里定位部位 ROI

目标部位提取的第一步是在整只体数据中定位，例如 head：

1. 选中 specimen 的 `Full volume`。
2. 在 `切片复核` 模式下找到目标部位出现的方向和范围。
3. 点击 `手动框选 ROI / Draw ROI`。
4. 在同一个观察方向的多张关键切片上画矩形。比如你从头顶往下看，可以在头部刚出现、最大截面、快结束的位置分别画不同大小的外接矩形。
5. 程序会把这些矩形连成一个随切片变化的 ROI 外壳，并同时维护一个包含所有矩形的父级 bbox 容器。
6. `保存 ROI 草稿 / Save ROI draft` 是暂存点：它保存当前 ROI 定位状态，防止中途退出或继续调整时丢失。
7. 保存草稿后继续画框，会自动更新同一个草稿；确认 ROI 时使用当前最新 ROI 状态。
8. `确认 ROI / Confirm ROI` 会让你输入 part ID / display name，然后直接生成对应的 part volume。

当前同一个 ROI 外壳只使用一个观察方向。如果你已经在 Z 方向画了关键切片，后来切到 Y 或 X 方向继续画，程序会提示你切回原方向，或另存一个独立 ROI。这样做是为了避免把不同投影方向的矩形混在一起，导致生成的外壳没有明确几何含义。

旧代码里仍保留了 `Create part / 新建部位` 的后端函数，但当前界面定位流程里这个按钮不作为推荐入口，按钮也保持禁用。正常流程就是 `Draw ROI`，必要时 `Save ROI draft`，最后 `Confirm ROI`。

`确认 ROI` 不是单纯“保存草稿”。它会：

- 裁出一个包含全部 ROI 关键切片的规则 part image 容器。
- 在 part 下写入 `image.ome.zarr`、`mask.ome.zarr`、`contours.json` 和 `extraction.json`。
- 把 ROI 记录标记为已经关联到新 part。
- 如果 ROI 使用了多张关键切片矩形，会用这个 ROI 外壳初始化 part mask 草稿。

这里仍会保留一个外接 bbox 作为存储容器，因为体数据文件需要一个规则的数组边界。但这个 bbox 不是目标部位本身；真正减少无关结构干扰的是 ROI 外壳初始化出来的 part mask。换句话说，part image 可以是规则长方体，part mask 可以随切片收缩或扩张。

#### 10.7.2 进入 part volume 后编辑真正的 part mask

确认 ROI 后，左侧 specimen 树会出现新的 part。选中这个 part 以后，才进入第二层：编辑这个部位自己的 mask。

常用流程是：

1. 在 part volume 中回到 `切片复核`。
2. 如果只是快速起步，可以点 `添加矩形关键切片 / Add rectangular key slice`，它会在当前 Z 切片上放一个默认矩形关键帧。
3. 如果要按真实轮廓修，可以点 `手绘轮廓 / Draw contour`，在当前 Z 切片上画闭合轮廓。
4. 在多张关键切片上重复上述操作。
5. 点 `预览自动填充 / Preview auto fill`，程序只根据当前 part 的 mask 关键切片生成预览。
6. 检查预览覆盖是否合理。
7. 点 `接受部位 mask / Accept part mask`，把预览写入 part 的 mask sidecar。

这一层的矩形关键切片和手绘轮廓，才是后续 part mask 编辑材料。它们保存在 part 的 `contours.json` 中。定位 ROI 阶段的关键切片不会再作为 part mask 关键帧写入 `contours.json`，避免你进入 part 后被旧定位框干扰。

#### 10.7.3 旧项目里残留的 ROI 外壳怎么处理
...
| Draw ROI | 在整只体数据切片上画部位定位矩形 | 当前 ROI 草稿和 bbox |
| Save ROI draft | 暂存当前 ROI 定位状态 | specimen 的 part ROI 记录，不生成 part |
| Confirm ROI | 用当前 ROI 生成 part volume | part image / part mask / extraction 记录 |
| Cancel ROI | 取消未关联 part 的 ROI 草稿 | ROI 草稿状态 |
| Add rectangular key slice | 在 part volume 当前 Z 切片添加 mask 矩形关键帧 | part 的 `contours.json` |
| Draw contour | 在 part volume 当前 Z 切片手绘 mask 轮廓 | part 的 `contours.json` |
| Preview auto fill | 根据 part mask 关键切片生成自动填充预览 | 当前 part mask 预览 |
| Accept part mask | 接受预览并写入 part mask | part 的 `mask.ome.zarr` |
| Clear key slices | 清空当前 part 的 mask 关键切片和预览 | part contours 与 mask 草稿，不删 part image |
...
### 15.4 标准流程模板 D：TIF/CT 中从整只体数据定位 ROI、生成 part volume 并复核 part mask

这个模板适合：

- 你手里是一批 TIF stack 或 CT 派生体数据
- 你要从整只个体中切出 head、brain 或其他内部/局部结构
- 你希望用关键切片矩形逐步贴合目标部位，而不是只切一个固定长方体
...
- 不要把 ROI 定位框当成进入 part 后的正式 mask 关键帧。

**步骤 5：确认 ROI 并生成 part volume**

1. 检查 ROI 外壳是否覆盖你要切出的目标部位。
...
- 一个由 ROI 外壳初始化的 part mask 草稿，前提是你画了 ROI 关键切片；这个 mask 可以随切片变化，不是把整个长方体都当作目标。
- ROI 与 part 的关联记录。
- 后续 Local Axis Reslice 可使用的 part 节点。

**步骤 6：进入 part volume 后再编辑真正的 part mask**

1. 在左侧树中选中新生成的 part。
2. 回到 `切片复核`。
3. 用 `Add rectangular key slice` 添加矩形关键切片，或用 `Draw contour` 手绘真实轮廓。
4. 在多张 Z 切片上保存关键帧。
5. 点击 `Preview auto fill` 生成自动填充预览。
6. 预览合理后点击 `Accept part mask`。

这一层才是正式 part mask 编辑。整只体数据里画的 ROI 只是定位和初始化，不会再污染 part 的 `contours.json`。

**步骤 7：如果旧项目里有残留 ROI 外壳**
...
**步骤 8：需要统一方向时再做 Local Axis Reslice**
...
2. **part mask**：在 part 内复核过的目标结构 mask。
3. **Local Axis reslice**：在统一局部坐标系下导出的重切片序列和 metadata。

这三者不要混成一个动作。ROI 负责定位，part mask 负责分割，Local Axis 负责方向标准化。
