 ...
 * @param {string} mode
 */
export function applyPermissionMode(session: Record<string, unknown>, mode: string) {
 ...
 * @param {Record<string, any>} session
 */
export function permissionModeSummary(session: Record<string, unknown>) {
 ...
 */
export function buildApprovalPreview(request: Record<string, unknown> = {}) {
  const toolName = request.toolName ?? "unknown";
  const input = asRecord(sanitizeSensitiveValue(request.input ?? {}));
  const decision = asRecord(request.decision);
  const definition = asRecord(request.definition);
  ...
  if (toolName === "mcp_call") {
    return [
      ...
      decision.targetPath ? `路径：${decision.targetPath}` : null,
      `参数：${truncate(JSON.stringify(input.arguments ?? {}), 220)}`
    ].filter(Boolean);
  }
  if (definition.risk === "network") {
    return [
...
export function sanitizeSensitiveValue(value: unknown, options: Record<string, unknown> = {}) {
  ...
  function visit(current: unknown, depth: number, key = ""): unknown {
    ...
    if (Array.isArray(current)) {
      const output: unknown[] = current.slice(0, maxEntries).map((item) => visit(item, depth + 1));
...
function redactTokenLikeText(value: unknown) {
  return String(value ?? "")
...
function isSensitiveKey(value: unknown) {
