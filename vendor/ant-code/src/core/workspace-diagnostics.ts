 ...
 * @param {string} cwd
 */
export async function diagnoseWorkspace(cwd: string) {
  ...
  const runtimeDataDirs = entries
    ...
    .map((entry) => entry.name)
    .sort((a, b) => a.localeCompare(b));
  const nonRuntimeEntries = names.filter((name: string) => !RUNTIME_DATA_DIRS.has(name));
 ...
 */
export function formatWorkspaceDiagnosticLines(diagnostic: Awaited<ReturnType<typeof diagnoseWorkspace>>, options: { includeCwd?: boolean } = {}) {
  if (!diagnostic) {
...
function workspaceWarning({ runtimeDataOnly, unreadableOrEmpty }: { runtimeDataOnly?: unknown; unreadableOrEmpty?: unknown }) {
  if (runtimeDataOnly) {
