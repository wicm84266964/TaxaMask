...
type PlanSectionKey = "requirementsDoc" | "taskPlanDoc" | "executionChecklist";
 ...
 */
export function createPlanPackageStore(options: { cwd: string }) {
  const root = path.join(options.cwd, PLAN_ROOT);
  return {
    root,
    async writePlanPackage(input: Record<string, unknown> = {}) {
 ...
 */
export function extractPlanPackage(output: unknown, options: { parsed?: unknown } = {}) {
 ...
 * @param {unknown} value
 */
export function normalizePlanPackage(value: unknown) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return null;
  }
  const item = asRecord(value);
  const source = isRecord(item.planPackage) ? item.planPackage : item;
  const documents = isRecord(source.documents) ? source.documents : {};
...
function parseJsonFromText(output: unknown) {
...
function parseMarkdownPlanPackage(output: unknown) {
  const text = String(output ?? "").trim();
  if (!text) {
    return null;
  }
...
function extractMarkdownSections(text: string): Partial<Record<PlanSectionKey, string>> {
  const lines = String(text ?? "").split(/\r?\n/);
  const headings: Array<{ key: PlanSectionKey; index: number }> = [];
  ...
  const result: Partial<Record<PlanSectionKey, string>> = {};
  for (let i = 0; i < headings.length; i += 1) {
    const current = headings[i];
    const next = headings[i + 1];
    if (!current || result[current.key]) {
      continue;
    }
...
function classifyHeading(value: unknown): PlanSectionKey | null {
...
function firstMarkdown(source: Record<string, unknown>, documents: Record<string, unknown>, keys: string[]) {
...
function ensureMarkdown(title: string, value: unknown) {
...
function generatePlanId(now: unknown) {
...
function relativePath(cwd: string, target: string) {
