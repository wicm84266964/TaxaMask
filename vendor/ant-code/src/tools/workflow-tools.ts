 ...
 */
export function todoReadTool(input: { workflow: ReturnType<typeof createWorkflowState> }) {
 ...
 */
export function todoWriteTool(input: { workflow: ReturnType<typeof createWorkflowState>; items: Array<Record<string, unknown>> }) {
 ...
 */
export function planUpdateTool(input: { workflow: ReturnType<typeof createWorkflowState>; explanation?: string; steps: Array<Record<string, unknown>> }) {
  input.workflow.plan = {
 ...
 */
export function syncWorkflowCompletionOnFinal(workflow: ReturnType<typeof createWorkflowState>, finalText: string) {
 ...
 * @param {string} text
 */
export function addTodo(workflow: ReturnType<typeof createWorkflowState>, text: string) {
 ...
 * @param {string} status
 */
export function updateTodoStatus(workflow: ReturnType<typeof createWorkflowState>, number: number, status: string) {
 ...
 * @param {ReturnType<typeof createWorkflowState>} workflow
 */
export function clearTodos(workflow: ReturnType<typeof createWorkflowState>) {
  workflow.todos = [];
  return [];
}
 ...
 * @param {string[]} titles
 */
export function setPlanSteps(workflow: ReturnType<typeof createWorkflowState>, titles: string[]) {
  workflow.plan = {
    explanation: workflow.plan?.explanation ?? "",
    steps: normalizeItems(titles.map((title: string) => ({ title, status: "pending" })), "step")
  };
 ...
 * @param {ReturnType<typeof createWorkflowState>} workflow
 */
export function clearPlan(workflow: ReturnType<typeof createWorkflowState>) {
 ...
 * @param {ReturnType<typeof createWorkflowState>} workflow
 */
export function summarizeWorkflow(workflow: ReturnType<typeof createWorkflowState>) {
 ...
 * @param {ReturnType<typeof createWorkflowState>} workflow
 */
export function formatWorkflowContext(workflow: ReturnType<typeof createWorkflowState>) {
 ...
 */
export function recordFileChange(workflow: ReturnType<typeof createWorkflowState>, change: Record<string, unknown>) {
  const next = {
    ...
    diffBytes: typeof change.diffBytes === "number" && Number.isFinite(change.diffBytes) ? change.diffBytes : 0,
 ...
 */
export function recordValidation(workflow: ReturnType<typeof createWorkflowState>, validation: Record<string, unknown>) {
  const exitCode = typeof validation.exitCode === "number" && Number.isFinite(validation.exitCode) ? validation.exitCode : null;
  ...
  const next: WorkflowValidation = {
    ...
    passed,
    timedOut,
    durationMs: typeof validation.durationMs === "number" && Number.isFinite(validation.durationMs) ? validation.durationMs : null,
 ...
 * @param {ReturnType<typeof createWorkflowState>} workflow
 */
export function cloneWorkflowState(workflow: ReturnType<typeof createWorkflowState>) {
 ...
 * @param {string} prefix
 */
function normalizeItems(items: unknown, prefix: string): WorkflowItem[] {
  ...
  return items.slice(0, MAX_ITEMS).map((item, index) => {
    ...
    const text = truncateText(String(source.content ?? source.text ?? source.title ?? source.task ?? source.description ?? source.label ?? source.name ?? source.value ?? ""), MAX_TEXT_CHARS);
    return {
...
function normalizeItemSource(item: unknown): Record<string, unknown> {
  if (typeof item === "string" || typeof item === "number" || typeof item === "boolean") {
    return { content: String(item), status: "pending" };
  }
  if (Array.isArray(item)) {
    return { content: item.map((part: unknown) => String(part ?? "")).filter(Boolean).join(" "), status: "pending" };
  }
 ...
 */
function inferItemStatus(source: Record<string, unknown>) {
 ...
 * @param {string} status
 */
function normalizeStatusToken(status: unknown) {
...
function hasStrongCompletionSignal(text: string) {
...
function hasIncompleteSignal(text: string) {
  const value = String(text ?? "");
 ...
 */
function completeWorkflowItems(items: WorkflowItem[] | undefined, options: { includePending?: boolean } = {}): { items: WorkflowItem[]; completed: number } {
 ...
 * @param {number} max
 */
function truncateText(text: string, max: number = MAX_TEXT_CHARS) {
 ...
 * @param {Array<Record<string, any>>} items
 */
function summarizeStatuses(items: WorkflowItem[]) {
 ...
 */
function summarizeValidations(validations: WorkflowValidation[]) {
  const summary = {
 ...
 * @param {Array<Record<string, any>>} validations
 */
function recentFailedValidations(validations: WorkflowValidation[]) {
 ...
 */
function formatStatusSummary(summary: Record<string, unknown>) {
 ...
 * @param {number} index
 */
function formatValidationFailure(validation: WorkflowValidation, index: number) {
 ...
 * @param {Record<string, any>} validation
 */
function buildFailureContext(validation: Record<string, unknown>) {
  return {
 ...
 * @param {unknown} value
 */
function buildRedactedExcerpt(value: unknown) {
 ...
 */
function classifyValidationCommand(command: string) {
 ...
 * @param {string} value
 */
function redactValidationText(value: string) {
  return value
    .replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g, "")
