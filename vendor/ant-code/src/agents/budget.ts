...
const BUILTIN_BUDGETS: Record<string, BudgetFields> = Object.freeze({
...
type AgentsBudgetConfig = {
  ...
  maxRounds?: unknown;
  vision?: { enabled?: unknown; model?: unknown };
...
function agentsConfig(config: Record<string, unknown>): AgentsBudgetConfig {
  return asRecord(config.agents) as AgentsBudgetConfig;
}
 ...
 */
export function resolveAgentBudget(options: { config?: Record<string, unknown>; profile?: Record<string, unknown>; routeDecision?: Record<string, unknown>; difficulty?: string; risk?: string } = {}) {
  ...
  const budgetConfig = asRecord(agents.budgets);
  ...
  const keys = [
    ...
    `${profileName}.${risk}`,
    ...aliases.flatMap((alias: unknown) => [`${alias}.${difficulty}`, `${alias}.${risk}`]),
    profileName,
    ...aliases
  ];
  ...
  if (Number.isInteger(agents.maxRounds) && Number(agents.maxRounds) > 0) {
    budget.maxRounds = Number(agents.maxRounds);
  }

  return normalizeBudget(budget, DEFAULT_AGENT_BUDGET) as ResolvedBudget;
}
 ...
 */
export function resolveAgentModel(config: Record<string, unknown>, routeDecision: Record<string, unknown> = {}, profile: Record<string, unknown> = {}) {
  ...
  if (isVisionProfile(profile) || tier === "vision") {
    ...
    const visionModel = vision.enabled === false ? "" : String(vision.model ?? "").trim();
    if (visionModel) {
      return visionModel;
    }
  }
  const modelTiers = asRecord(agents.modelTiers);
  const model = modelTiers[tier];
  return typeof model === "string" && model.trim() ? model.trim() : config.modelAlias;
}

function isVisionProfile(profile: Record<string, unknown> = {}) {
  if (profile.name === "visual-verifier" || profile.purpose === "visual") {
    return true;
  }
  return Array.isArray(profile.aliases) && profile.aliases.some((alias: unknown) => /^(?:vision|vision-verifier|visual-reviewer|screenshot-reviewer)$/i.test(String(alias)));
}

/**
 * @param {ReturnType<typeof resolveAgentBudget>} budget
 */
export function createBudgetTracker(budget: ReturnType<typeof resolveAgentBudget>) {
  return {
    budget,
    startedAt: Date.now(),
    toolCalls: 0,
 ...
 */
export function checkBudget(tracker: ReturnType<typeof createBudgetTracker>, state: { round?: number; pendingToolCalls?: number } = {}) {
  const budget = tracker.budget;
  const round = state.round;
  const maxRounds = budget.maxRounds;
  const maxToolCalls = budget.maxToolCalls;
  if (typeof maxRounds === "number" && Number.isInteger(maxRounds) && maxRounds > 0 && typeof round === "number" && Number.isInteger(round) && round >= maxRounds) {
    return exceeded("maxRounds", `已达到子任务模型-工具循环上限 ${maxRounds} 轮。`, maxRounds);
  }
  if (typeof maxToolCalls === "number" && Number.isInteger(maxToolCalls) && maxToolCalls > 0 && tracker.toolCalls + (state.pendingToolCalls ?? 0) > maxToolCalls) {
    return exceeded("maxToolCalls", `继续执行将超过工具调用上限 ${maxToolCalls} 次。`, maxToolCalls);
  }
 ...
 */
export function recordBudgetToolResult(
  tracker: ReturnType<typeof createBudgetTracker>,
  execution: { ok?: boolean; blocked?: boolean; error?: Record<string, unknown>; decision?: { decision?: unknown } },
...
function normalizeBudget(value: unknown, fallback: BudgetFields = {}): BudgetFields {
  const item = asRecord(value);
  const result: BudgetFields = { ...fallback };
  for (const key of BUDGET_KEYS) {
    const raw = item[key];
    const number = Number(raw);
    if (Number.isInteger(number) && number > 0) {
      result[key] = number;
    } else if ((key === "maxRounds" || key === "maxToolCalls") && raw === null) {
      result[key] = null;
    }
  }
  return result;
...
function exceeded(kind: string, message: string, limit: unknown) {
  return {
    kind,
    message,
    limit
