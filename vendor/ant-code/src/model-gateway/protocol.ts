...
export type NormalizedGatewayResponse = {
  id: string | null;
  model: string | null;
  content: GatewayTextBlock[];
  text: string;
  thinkingText?: string;
...
type GatewayRequestCapabilities = {
  tools: boolean;
  toolResults: boolean;
  streaming: boolean;
  images?: boolean;
...
export function createGatewayRequest(input: {
  ...
  const capabilities: GatewayRequestCapabilities = {
    tools: true,
    toolResults: true,
...
function requestIncludesImages(messages: unknown = []) {
  ...
  return messages.some((message) => {
    ...
    return Array.isArray(content) && content.some((item) => item && typeof item === "object" && "type" in item && item.type === "image");
  });
}
 ...
 */
export function normalizeGatewayResponse(raw: unknown): NormalizedGatewayResponse {
  if (!raw || typeof raw !== "object") {
    return emptyResponse(raw);
  }

  const value = raw as Record<string, unknown>;
  const content = normalizeContent(value.content ?? value.output ?? objectProperty(value.message, "content"));
  ...
  return {
    ...
    content,
    text,
    toolCalls: normalizeToolCalls(value.toolCalls ?? value.tool_calls ?? objectProperty(value.message, "toolCalls")),
 ...
 * @param {unknown} raw
 */
export function emptyResponse(raw: unknown = null): NormalizedGatewayResponse {
  return {
    id: null,
    model: null,
    content: [],
 ...
 * @returns {TextBlock[]}
 */
export function normalizeContent(value: unknown): GatewayTextBlock[] {
...
function normalizeToolCalls(value: unknown) {
  ...
  return value
    .filter((item): item is Record<string, unknown> & { name: string } => isPlainObject(item) && typeof item.name === "string")
    .map((item, index) => ({
