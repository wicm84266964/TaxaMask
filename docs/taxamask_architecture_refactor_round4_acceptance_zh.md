...
- [ ] 关闭并重开同一项目，刚才的修改仍存在。
- [ ] 运行图片导入、SAM、parent/child 训练、批量预测、VLM 或导出时尝试打开另一项目，程序应提示任务仍在运行并阻止切换。
...
- [ ] 子专家训练入口、进度和停止按钮行为正常。
- [ ] 子专家训练完成后，新模型 route 只登记在发起训练的原项目；切换到同名部位的新项目时不能自动串入。
...
- [ ] 从 TIF 或 STL 项目进入 Agent/Start Center 后关闭并重开，最近项目仍恢复为原 TIF/STL 项目。

## 7. 验收结论

- [ ] TaxaMask 全功能已逐项完成本卡人工测试。
- [x] 主要交互项验收通过，第四轮候选标记为 `accepted with recorded manual coverage gaps`。（2026-07-12：用户说明未完整测试 TaxaMask 全部内容，但已测试主要交互项，暂未发现问题）
- [ ] 存在问题，需要记录具体项目、操作步骤、预期和实际结果。

未勾选的功能项表示“本轮没有逐项人工覆盖”，不表示已发现失败。它们由 1,149 条自动化测试提供基础回归证据，并作为后续日常使用中的残余人工覆盖清单保留。
