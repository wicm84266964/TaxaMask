...
> **Version**: TaxaMask developer preview branch `codex/antscan-stl-tif-rearchitecture` (current handoff synced June 23, 2026: public preview docs + TIF/CT Local Axis Reslice + long Chinese manual restored)
> **Purpose**: Up-to-date architectural, workflow, and governance context for implementation and maintenance.

---

## 0) Developer Preview Branch Current State (2026-06-23)

### 0.0 Public branch and documentation status
- Current public developer-preview branch: `codex/antscan-stl-tif-rearchitecture`.
- GitHub remote: `https://github.com/wicm84266964/TaxaMask.git`.
- The branch is intended to expose the newer TIF/CT route without pretending it is the stable `main` line.
- `README.md` and `README_zh.md` now describe the developer preview rather than the older 2D/PDF-only public posture.
- `TaxaMask使用手册.md` has been restored to the long onboarding manual scale and updated with a current developer-preview front section.
- `LLM_CONTEXT_DETAILED.md` is intentionally tracked again. It is part of the Ant-Code / LLM handoff surface, not a disposable private draft.
- Internal design drafts, old handoff folders, local Agent sessions, private runtime config, model weights, CT stacks, project JSON, databases, and generated outputs should remain ignored or removed from public tracking.

### 0.1 Drishti / volume preview provenance wording
- A static source comparison against the local Drishti checkout found no evidence that TaxaMask copied Drishti source code.
- TaxaMask TIF/CT 3D preview is an independent PySide6 / PyOpenGL implementation centered in `AntSleap/ui/tif_gpu_volume_canvas.py` and embedded through `AntSleap/ui/tif_workbench.py`.
- Public wording should be careful: TaxaMask uses common GPU volume-rendering ideas and is informed by established scientific volume-visualization tools such as Drishti. Do not overstate this as a Drishti port or vendored integration.

### 0.2 Current TIF/CT product stance
- The TIF/CT route is now a visible developer-preview workflow, not just an internal experiment.
- It has been developed and tested mainly with AntScan ant CT data.
- Data structures and UI wording should avoid hard-coding "brain" as the module identity. Use **Local Axis Reslice / 局部轴重切片**.
- `head` / brain-oriented work remains the first practical template, but the underlying model is generic: specimen -> part volume -> source Z reference -> editable output Z axis -> roll reference point pair -> local frame -> reslice output.
- Do not present broad multi-taxon validation as complete.

### 0.3 Current Local Axis Reslice behavior
- Main user flow:
  - import/open a TIFF stack as a TIF specimen
  - create a target part with ROI and key-slice contours
  - accept/build the part mask and crop a part volume
  - inspect the part in 3D and multi-direction slices
  - show locked source Z-axis
  - copy it to editable output Z-axis
  - drag output-axis endpoints or axis body in the 3D preview
  - use observation-side clip/section plane for roll reference point picking
  - export `image.tif` plus `metadata.json`, and optional `mask.tif` when a part mask exists
- Output is stored below the selected part:
  - `specimens/<specimen>/parts/<part>/reslices/<reslice_id>/image.tif`
  - `metadata.json`
  - optional `mask.tif`
  - project record in `part.metadata.local_axis_reslices[]`
- Selecting a reslice item must load the exported reslice image, not the original part volume.
- Reslice direction is strictly the editable output Z-axis. Roll reference points standardize in-plane orientation and support review/training; they are not a second slicing axis.
- Grayscale reslicing uses linear interpolation. Mask/label reslicing uses nearest-neighbor interpolation.
- Export must not mutate source TIFF, part volume, source mask, contours, or extraction metadata.

### 0.4 TIF/CT GPU and renderer notes
- The user has a known Windows setup where NVIDIA GPU selection depends on the Python executable path and environment name.
- `启动TaxaMask.bat` now prefers the `taxamask` Conda environment before the older `antsleap` fallback.
- If `TAXAMASK_PYTHON_EXE` or NVIDIA Control Panel still points to an old environment, OpenGL may run on the integrated GPU.
- The TIF workbench status line should be used to verify the active OpenGL renderer. NVIDIA is expected for high-performance TIF/CT work.
- Windows scripts can help select the intended Python path, but they cannot reliably override every Windows/OpenGL GPU routing decision. Public docs should tell users to set the active `python.exe` to High performance in Windows Graphics settings or NVIDIA Control Panel.
- Observation-side clipping/section rendering is display-only and should not mutate saved data.
- The section plane is intentionally texture-like and color-integrated with the current volume rendering so it can be used to inspect fine local structure without looking like a detached CT slice panel.

### 0.5 Current AI stance for TIF/CT
- AI automation remains strategically important because AntScan-scale work may involve thousands of specimens.
- The current first-version product stance is data capture and contract readiness, not a fully integrated model.
- Keep recording manual ROI, part mask, output axis, roll references, local frame, reslice parameters, outputs, and provenance as trainable material.
- Future training/inference should follow the 2D external-backend style:
  - external backend prepares/trains/predicts through contract JSON
  - prediction outputs are proposals/drafts
  - proposals require researcher review before they can drive formal reslice export
  - no backend should directly overwrite confirmed manual truth or final outputs

### 0.6 Main branch synchronization requirement
- The user explicitly asked that `TaxaMask使用手册.md` and `LLM_CONTEXT_DETAILED.md` be restored on both the current developer-preview branch and `main`.
- Keep `main` public and stable, but it should still retain the long Chinese manual and the long LLM context file because they are part of user onboarding and Agent handoff.
- If updating `main`, avoid copying unrelated developer-preview code changes unless explicitly requested. Prefer synchronizing these two documentation/context files only.
...
- `<taxamask-python> -m py_compile AntSleap\main.py AntSleap\core\project.py`
...
- TaxaMask:
  - `<taxamask-python> -m py_compile AntSleap\main.py AntSleap\ui\taxamask_agent_panel.py`
...
- Focused validation used:
  - `<taxamask-python> -m unittest tests.test_vlm_preannotation`
  - `<taxamask-python> -m unittest tests.test_gui_smoke.GuiSmokeTests.test_vlm_result_refreshes_current_canvas_with_project_relative_path tests.test_gui_smoke.GuiSmokeTests.test_vlm_result_repaints_canvas_while_progress_dialog_is_open tests.test_gui_smoke.GuiSmokeTests.test_vlm_result_reloads_workbench_after_file_list_refresh_blocks_selection_signal`
  - `<taxamask-python> -m unittest tests.test_gui_smoke.GuiSmokeTests.test_vlm_preannotation_progress_ui_tracks_steps`
...
- Last full focused validation reported for this milestone:
  - `<taxamask-python> -m unittest tests.test_tif_project tests.test_tif_stack_import tests.test_tif_export tests.test_tif_prediction_import tests.test_tif_gpu_volume_canvas tests.test_tif_workbench`
  - Result: 55 tests, OK.
...
- Verified:
  ...
  - `unittest` in `<taxamask-python>` for GUI/Blink-related tests
...
- Validation used `<taxamask-python>`.
...
- Full validation on 2026-05-16:
  - `<taxamask-python> -m unittest discover tests`
  - 193 tests passed.
- Compile validation:
  - `<taxamask-python> -m compileall -q AntSleap tests`
- Real AMIRA sample path still resolves:
  - `<local-amira-data-dir>`
...
### 0.0 2026-04-02 stable-release frontend alignment imported the accepted dual-theme workbench into the main development workspace
- The stable development branch now carries the approved frontend theme work from the separate test branch.
...
- Real-PDF spot checks from `<local-pdf-output-dir>` showed that external caption/body text leakage was reduced substantially, while whole triptych figures remained generally usable.
