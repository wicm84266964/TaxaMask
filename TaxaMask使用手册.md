...
> 适用版本：`codex/antscan-stl-tif-rearchitecture` 开发预览分支：Agent Center / PDF 文献证据闭环 / 2D-STL 大项目标注 / SAM 与 VLM 草稿复核 / 父部位与子部位标注 / 模型方案与 Blink 后端切换 / TIF-CT 体预览 / part volume 提取 / Local Axis Reslice / 数据集导出
>
...
## 开发预览分支先读：TIF/CT 与局部轴重切片

如果你使用的是 `codex/antscan-stl-tif-rearchitecture` 分支，请先按下面方式安装或下载：
...
这个分支在原有 PDF、2D/STL 和 Agent Center 之外，重点加入了 TIF/CT 内部结构路线。它当前主要用 AntScan 蚂蚁 CT 数据验证，目标是处理原始切片方向、标本体态和目标结构方向差异很大的体数据。程序结构没有把 TIF 工作流写死为蚂蚁专用，但目前还不能宣称已经完成广泛跨类群验证。

TIF/CT 当前最重要的新能力是：

- 导入 TIFF stack 作为 specimen。
- 使用 ROI 和关键切片轮廓创建 part volume，例如 head。
...
TIF Volume Workbench 是独立于 2D/STL 的体数据工作流。它适合连续切片、AMIRA/Avizo 标注数据、micro-CT、confocal 或其他能表示为 stack 的体数据。当前开发预览分支还加入了 AntScan / Local Axis Reslice 路线，用于先提取目标 part volume，再对该部位体进行局部坐标系重切片。

最重要的一点是：

> TIF 的训练单位不是单张 polygon 图片，而是一个带 material-ID label volume 的 specimen 体数据。
>
> 当前 TIF 路径属于开发预览扩展，灵感来自 AntScan 一类体数据集需求。它已经具备项目结构、导入、切片查看、材料表、part ROI、part volume、3D 体预览、局部轴重切片和导出/后端契约，但稳定性和验证程度仍低于 2D/STL 主标注路线。
...
### 10.7 Part ROI、关键切片轮廓与 part volume

TIF/CT 路线中，建议优先只对目标部位工作，而不是全量重切整个 TIFF。

目标部位提取的典型流程是：

1. 在 full volume 或多方向切片中找到目标部位大致范围。
2. 使用 ROI 或关键切片轮廓标出目标 part，例如 head。
3. 使用关键切片插值生成 preview mask。
4. 检查 preview mask 是否覆盖目标部位。
5. 接受 part mask。
6. 使用 `crop_volume_to_part(...)` 一类后端能力生成 part volume。
