...
import { collectDeclaredDependencies, readPackageJson, type NpmLockfile, type PackageJsonLike } from "./dependency-audit-common.ts";
import { collectLockfileConsistencyFailures } from "./lockfile-consistency.ts";
 ...
 */
async function verifyLockfileDependencyBoundaries(declaredDependencyCount: number) {
  ...
  try {
    lock = JSON.parse(text) as NpmLockfile;
  } catch (error) {
...
/**
 * @param {string} name
 * @param {PackageJsonLike} rootPackage
 */
function isReviewedBuildInstallScript(name: string, rootPackage: PackageJsonLike) {
  return reviewedBuildInstallScriptAllowlist.has(name) &&
    Boolean(rootPackage.devDependencies && Object.hasOwn(rootPackage.devDependencies, name)) &&
    !Boolean(rootPackage.dependencies && Object.hasOwn(rootPackage.dependencies, name));
}
