...
class TifBackendTests(unittest.TestCase):
    ...
    def test_train_run_emits_progress_and_can_cancel_command(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(RuntimeError, "tif_backend_train_cancelled"):
                runner.run_action("train", part_refs=[{"specimen_id": "01-0101-11", "part_id": "brain", "reslice_id": "brain_axis_001"}], progress_callback=on_progress, cancel_check=cancel_check)
    ...
    def test_tif_trainer_adapter_prepare_dataset_writes_part_nnunet_layout(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            manifest_path = result["result"]["provenance"]["dataset_manifest"]
    ...
    def test_tif_trainer_adapter_train_writes_model_manifest(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = runner.run_action("train", part_refs=[{"specimen_id": "01-0101-11", "part_id": "brain", "reslice_id": "brain_axis_001"}])

            manifest_path = Path(result["result"]["provenance"]["model_manifest"])
    ...
    def test_tif_trainer_adapter_supports_top_level_volume_loop(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(Path(prepare["result"]["provenance"]["dataset_manifest"]).exists())

            train = runner.run_action("train", specimen_ids=["top-001"], input_scope="top_level_volume")
            manifest_path = Path(train["result"]["provenance"]["model_manifest"])
            with open(manifest_path, "r", encoding="utf-8") as handle:
                manifest = json.load(handle)
    ...
    def test_tif_trainer_adapter_smoke_closes_review_retrain_loop(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            train = runner.run_action("train", part_refs=[ref])
            manifest_path = Path(train["result"]["provenance"]["model_manifest"])
            self.assertTrue(manifest_path.exists())
            with open(manifest_path, "r", encoding="utf-8") as handle:
            ...
            exported_manifest = retrain_prepare["result"]["provenance"]["dataset_manifest"]
