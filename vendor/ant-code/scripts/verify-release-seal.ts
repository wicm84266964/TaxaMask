...
import { ROOT, collectFiles, rel } from "./audit-common.ts";
...
const REVIEWED_RUNTIME_DEPENDENCIES = new Map([
  ["@vscode/ripgrep", {
    ...
    license: "MIT",
    provenanceMarker: "open_source: vscode-ripgrep wrapper, MIT licensed, public npm package"
  }],
  ["ink", {
    versionSpec: "^6.8.0",
...
async function verifyRequiredAttestation() {
  await requireDocMarkers("docs/audit/source-provenance-release-attestation.md", [
...
async function verifyRuntimeSourceMarkers() {
  ...
  const allowlist = new Set([
    "scripts/verify-forbidden-endpoints.ts",
    "scripts/verify-forbidden-endpoints.js",
    "scripts/verify-release-seal.ts",
...
async function verifyReviewedRuntimeDependencies() {
  ...
  for (const [name, reviewed] of REVIEWED_RUNTIME_DEPENDENCIES) {
    if (!Object.hasOwn(runtimeDependencies, name)) {
      continue;
    }
    await verifyRuntimeDependencyEvidence(name, reviewed);
  }
}
...
async function verifyRuntimeDependencyEvidence(name: string, reviewed: { versionSpec: string; installedVersion: string; license: string; provenanceMarker: string }) {
  const lock = await readJson<LockFileJson>("package-lock.json");
 ...
 */
async function requireDocMarkers(relativePath: string, markers: string[]) {
  const fullPath = path.join(ROOT, relativePath);
  const text = await fs.readFile(fullPath, "utf8").catch(() => null);
  if (text === null) {
    failures.push(`${relativePath} is missing`);
