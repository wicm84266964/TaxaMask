...
import { AUDIT_DIR, ROOT, collectFiles, ensureAuditDir, rel } from "./audit-common.ts";

const pkg = JSON.parse(await fs.readFile(path.join(ROOT, "package.json"), "utf8"));
const sourceFiles = await collectFiles(["src"], { extensions: new Set([".ts", ".js"]) });
const testFiles = await collectFiles(["tests"], { extensions: new Set([".ts", ".js"]) });
...
const sourceProvenanceAttestationPath = path.join(AUDIT_DIR, "source-provenance-release-attestation.md");
...
const currentDashboardAcceptancePath = path.join(ROOT, "docs", "deployment", "v1.3.0-dashboard-hardening-acceptance.md");
...
const sourceProvenanceAttestationExists = await exists(sourceProvenanceAttestationPath);
...
const currentDashboardAcceptanceExists = await exists(currentDashboardAcceptancePath);
...
const lines = [
  `# Ant Code ${pkg.version} Release Audit Report`,
  "",
  ...
  "## Scope",
  "",
  `- Ant Code lab-local coding agent ${pkg.version} controlled internal release.`,
  ...
  `- Source provenance release attestation: ${sourceProvenanceAttestationExists ? rel(sourceProvenanceAttestationPath) : "missing"}`,
  ...
  `- Current Dashboard acceptance record: ${currentDashboardAcceptanceExists ? rel(currentDashboardAcceptancePath) : "missing"}`,
  `- Historical internal v3.0 Dashboard acceptance record: ${v30AcceptanceExists ? rel(v30AcceptancePath) : "missing"}`,
  ...
  "```",
  "",
  "The default compatibility check uses the local mock gateway. `node scripts/verify-gateway-compat.ts --live` uses only the configured lab gateway endpoints and should be run before broad rollout.",
  "",
  ...
  "- Release seal checks validate independent attestation material, runtime source markers, public identity, package privacy, reviewed runtime dependency evidence, and local tool/model-adapter boundary evidence.",
