...
import { loadProjectMemory } from "../memory/loader.ts";
import { formatSkillContextLines } from "../skills/registry.ts";
import { getToolDefinitions } from "../tools/definitions.ts";
import { diagnoseWorkspace, formatWorkspaceDiagnosticLines } from "../core/workspace-diagnostics.ts";
import { resolveMaxParallelReadonlyAgentRuns } from "../agents/orchestration-config.ts";

const MAX_MEMORY_CHARS = 12_000;

/**
 * @param {{ cwd: string; config: import("../config/load-config.ts").LabAgentConfig; env?: NodeJS.ProcessEnv; clientSurface?: string }} options
 */
export async function buildInitialContext(options: { cwd: string; config: import("../config/load-config.ts").LabAgentConfig; env?: NodeJS.ProcessEnv; clientSurface?: string }) {
  ...
  return {
    system: [
      ...
      "- For local code search, prefer rg_search/rg_files/rg_files_with_matches/rg_count for broad or regex/glob/context searches; keep legacy grep as a simple fallback when rg is unavailable or a substring scan is enough.",
      "- For TypeScript and JavaScript code understanding, use ts_symbols, ts_find_definition, ts_find_references, and ts_diagnostics to confirm symbols, call impact, definitions, references, and type/syntax errors instead of relying only on text matches.",
...
function formatParallelReadonlyBudget(value: unknown) {
  return `${value} at once`;
}

function normalizeClientSurface(value: unknown) {
...
function clientSurfaceInstructions(surface: unknown) {
 ...
 */
function formatAgentContextLines(config: Record<string, unknown>, cwd: string) {
  return listAgentProfiles(config, { cwd, includeHidden: false }).map((profile) => {
    ...
    const outputContract = profile.outputContract;
    const contractType = outputContract && typeof outputContract === "object" && "type" in outputContract
      ? outputContract.type
      : "";
    const contract = contractType ? ` output=${contractType}` : "";
 ...
 */
function formatProjectMemory(cwd: string, memories: Array<{ path: string; content: string }>) {
